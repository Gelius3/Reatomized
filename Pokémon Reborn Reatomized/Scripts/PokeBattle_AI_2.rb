...
class PokeBattle_AI
	...
	def checkZMoves
		...
		for move in @attacker.moves
			...
			if (move.id == PBMoves::FLORALHEALING) && @attacker.item == PBItems::FAIRIUMZ2
				zmove = PokeBattle_ZMoves2.new(move, @attacker.item, @battle, @attacker)
				break 
			end
			if (move.id == PBMoves::CLANGINGSCALES) && @attacker.item == PBItems::KOMMONIUMZ2
				zmove = PokeBattle_ZMoves2.new(move, @attacker.item, @battle, @attacker)
				break 
			end
		...
		for moveindex in 0...battler.moves.length
			...
			if @battle.doublebattle
				...
				when PBTargets::AllNonUsers
					...
					if mondata.trainer && (mondata.trainer.trainertype == PBTrainers::UMBTITANIA || mondata.trainer.trainertype == PBTrainers::UMBAMARIA) && @battle.doublebattle
					...
					elsif (move.pbType(battler) == PBTypes::FIRE && (battler.pbPartner.ability == PBAbilities::FLASHFIRE || battler.pbPartner.ability == PBAbilities::WELLBAKEDBODY)) ||
						...
						(move.pbType(battler) == PBTypes::GRASS && battler.pbPartner.ability == PBAbilities::SAPSIPPER) || (move.pbType(battler) == PBTypes::GROUND && battler.pbPartner.ability == PBAbilities::EARTHEATER) ||
						...
						(move.pbType(battler) == PBTypes::NUCLEAR && battler.pbPartner.ability == PBAbilities::LEADSKIN) || (move.pbType(battler) == PBTypes::FAIRY && battler.pbPartner.ability == PBAbilities::DISENCHANT)
						scoremult*=2
	...
	def coordinateActions #changes some scores doesn't choose
		...
		# one wants to use decorate
		if PBMoves::DECORATE == bestmove1.id || PBMoves::DECORATE == bestmove2.id
			if PBMoves::DECORATE == bestmove1.id && bestmove2.basedamage == 0
				@aimondata[1].scorearray.map! {|a| a.map!.with_index {|b,i| i==bestindex1 ? 0 : b }}
			elsif PBMoves::DECORATE == bestmove2.id && bestmove1.basedamage == 0
				@aimondata[3].scorearray.map! {|a| a.map!.with_index {|b,i| i==bestindex2 ? 0 : b }}
			end
		end

		#both want to use the same move that affects the battlefield
		if bestmove1.id == bestmove2.id && [PBMoves::STEALTHROCK, PBMoves::STICKYWEB, PBMoves::TAILWIND, PBMoves::GRAVITY, PBMoves::LIGHTSCREEN, PBMoves::REFLECT, PBMoves::AURORAVEIL, PBMoves::SOLARARRAY,
	...
	def threatAssesment
		...
		@battle.battlers.each_with_index {|opp,i|
			...
			# Opp is Nuclear
			threatscore[i]*=2.0 if (opp.type1 == PBTypes::NUCLEAR || opp.type2 == PBTypes::NUCLEAR)
			# Abilities
	...
	def getMoveScore(initialscores=[],scoreindex=-1)
		...
		if @mondata.skill>=MEDIUMSKILL && !moldBreakerCheck(@attacker) &&
			...
			if 	(@move.pbType(@attacker) == PBTypes::FIRE && @opponent.nullsFire?) || (@move.pbType(@attacker) == PBTypes::GRASS && @opponent.nullsGrass?) || (@move.pbType(@attacker) == PBTypes::GROUND && @opponent.nullsGround?) || 
				(@move.pbType(@attacker) == PBTypes::WATER && @opponent.nullsWater?) || (@move.pbType(@attacker) == PBTypes::ELECTRIC && @opponent.nullsElec?) || (@move.pbType(@attacker) == PBTypes::COSMIC && @opponent.nullsNuclearCosmic?) || (@move.pbType(@attacker) == PBTypes::NUCLEAR && @opponent.nullsNuclearCosmic?)
		...
		if @move.pbIsPriorityMoveAI(@attacker) && @attacker != @opponent.pbPartner
			...
			score*=0 if @opponent.ability == PBAbilities::DAZZLING || @opponent.ability == PBAbilities::QUEENLYMAJESTY || @opponent.pbPartner.ability == PBAbilities::DAZZLING || @opponent.pbPartner.ability == PBAbilities::QUEENLYMAJESTY || @opponent.ability == PBAbilities::ARMORTAIL || @opponent.pbPartner.ability == PBAbilities::ARMORTAIL
		...
		if @mondata.skill>=HIGHSKILL && @opponent.index!=@attacker.index
			for j in getAIMemory(@opponent)
				...
				shieldcheck = true if j.id==PBMoves::SPIKYSHIELD || j.id==PBMoves::KINGSSHIELD ||  j.id==PBMoves::BANEFULBUNKER ||  j.id==PBMoves::SILKTRAP
			end
			if @battle.doublebattle
		...
		if hasgreatmoves()
			case @move.type
				...
				when PBTypes::NUCLEAR
					case @attacker.item
					when PBItems::NUCLEARGEM then score*=0.9
					end
				when PBTypes::COSMIC
					case @attacker.item
					when PBItems::COSMICGEM then score*=0.9
					end
			end
			score*=0.9 if @attacker.item == PBItems::POWERHERB && (PBStuff::TWOTURNMOVE + PBStuff::CHARGEMOVE).include?(@move.id)
		end
		#Contact move checks
		if @move.isContactMove? && !(@attacker.item == PBItems::PROTECTIVEPADS || @attacker.item == PBItems::PUNCHINGGLOVE && move.isPunchingMove?)  && @attacker.ability != PBAbilities::LONGREACH
			contactscore=1.0
			...
			when PBAbilities::MUMMY,PBAbilities::LINGERINGAROMA
			...
			contactscore*=0.7 if checkAImoves([PBMoves::SILKTRAP]) && !PBStuff::RATESHARERS.include?(@opponent.lastMoveUsed) && @attacker.pbCanReduceStatStage?(PBStats::SPEED)
		...
		if @opponent.effects[PBEffects::TwoTurnAttack]>0 && @mondata.skill>=HIGHSKILL
			...
			if (@move.accuracy>0 || @move.function==0xA5 || @move.zmove && @move.basedamage > 0 || @move.id == PBMoves::WHIRLWIND) && (PBStuff::TWOTURNMOVE.include?(invulmove) || @opponent.effects[PBEffects::SkyDrop]) &&
					pbAIfaster?(@move,nil,@attacker,@opponent) && @attacker.ability != PBAbilities::NOGUARD && @opponent.ability != PBAbilities::NOGUARD && !(@attacker.ability==PBAbilities::FAIRYAURA && @battle.FE == PBFields::FAIRYTALEF) || (@move.accuracy>0 || @move.function==0xA5 || @move.zmove && @move.basedamage > 0 || @move.id == PBMoves::WHIRLWIND) && (PBStuff::TWOTURNMOVE.include?(invulmove) || @opponent.effects[PBEffects::SkyDrop]) &&
					pbAIfaster?(@move,nil,@attacker,@opponent) && @attacker.ability != PBAbilities::ALOLANTECHNIQUESA && @opponent.ability != PBAbilities::ALOLANTECHNIQUESA && !(@attacker.ability==PBAbilities::FAIRYAURA && @battle.FE == PBFields::FAIRYTALEF)
				miss = true
		...
		if @opponent.effects[PBEffects::Commander]
			$ai_log_data[@attacker.index].final_score_moves.push(0)
			return 0
		end
		...
		case @move.function
			when 0x00 # No effect
				if @mondata.skill >= BESTSKILL && @battle.FE != PBFields::NONE
					...
					when PBFields::PSYCHICT
						if @move.id==PBMoves::EXPANDINGFORCE
			...
			when 0x0F # Flinch, Dark Pulse, Bite, Rolling Kick, Air Slash, Astonish, Needle Arm, Hyper Fang, Headbutt, Extrasensory, Zen Headbutt, Heart Stamp, Rock Slide, Iron Head, Waterfall, Zing Zap
                miniscore = flinchcode()
			when 0x282 && move.id == PBMoves::NEEDLEGUN # Needle Gun
				miniscore = flinchcode()
			...
			when 0x12 # Fake Out
				if @attacker.turncount==0 && !(@opponent.effects[PBEffects::Substitute] > 0 || @opponent.ability == PBAbilities::INNERFOCUS || secondaryEffectNegated?())
					...
					score*=1.5 if (@opponent.type1 == PBTypes::NUCLEAR || @opponent.type2 == PBTypes::NUCLEAR)
			...
			when 0x28 # Growth
				...
				if @mondata.skill >= BESTSKILL
					statarray = [2,0,0,2,0,0,0] if @battle.FE == PBFields::GRASSYT || @battle.FE == PBFields::FORESTF || @battle.pbWeather==PBWeather::SUNNYDAY || @attacker.ability ==PBAbilities::HEATGENERATOR || (@battle.field.counter<=2 && @battle.FE == PBFields::FLOWERGARDENF)
			...
			when 0x800 # Groom Guard
				...
				miniscore = selfstatboost(statarray)
			when 0x225 # Victory Dance
				statarray = [1,1,1,0,0,0,0]
				statarray = [2,2,2,0,0,0,0] if @mondata.skill >= BESTSKILL && @battle.FE == PBFields::BIGTOPA
				miniscore = selfstatboost(statarray)
			...
			when 0x39 # Tail Glow
				miniscore = selfstatboost([0,0,0,3,0,0,0])
			...
			when 0x257 # Fillet Away
				statarray = [2,0,2,2,0,0,0]
				miniscore = selfstatboost(statarray) ** 1.4 #More extreme scoring
				miniscore *= 0.3 if !@attacker.moves.any?{|moveloop| moveloop!=nil && moveloop.basedamage > 0 && moveloop.pbIsPriorityMoveAI(@attacker)} && !pbAIfaster?()
				miniscore *= 1.2 if @attacker.turncount<1
				miniscore = 1 if (@attacker.hp.to_f)/@attacker.totalhp <= 0.5
			when 0x3b # Superpower
				...
				if @attacker.ability == PBAbilities::CONTRARY || @attacker.ability == PBAbilities::ALOLANTECHNIQUESB
					miniscore = selfstatboost(statarray)
				else
			...
			when 0x3e # Hammer Arm, Ice Hammer
				...
				if @attacker.ability == PBAbilities::CONTRARY || @battle.trickroom > 2 || @attacker.ability == PBAbilities::ALOLANTECHNIQUESB
					miniscore = selfstatboost(statarray)
				else
					miniscore = selfstatdrop(statarray,score)
				end
			...
			when 0x96 # Natural Gift
				score*=0 if !pbIsBerry?(@attacker.item) || !@attacker.itemWorks? || @opponent.ability == PBAbilities::UNNERVE
			...
			when 0x9e # Water Sport
				...
				if @battle.FE == PBFields::SUPERHEATEDF
					...
					if pbPartyHasType?(PBTypes::GRASS) || pbPartyHasType?(PBTypes::BUG)
						...
						miniscore*=3 if @battle.FE == PBFields::FLOWERGARDENF && @battle.field.counter<4 && @battle.pbWeather!=PBWeather::RAINDANCE
						miniscore*=3 if @opponent.hasType?(:FIRE)
					end
				end
			...
			when 0xab # Quick Guard
				if (@opponent.ability == PBAbilities::GALEWINGS && @opponent.hp >= (@opponent.totalhp*0.75).floor) || (@opponent.ability == PBAbilities::PRANKSTER && @attacker.pbHasType?(:DARK)) || checkAIpriority() 
			...
			when 0xac # Wide Guard
				if getAIMemory().any? {|moveloop| moveloop!=nil && (moveloop.target == PBTargets::AllOpposing || moveloop.target == PBTargets::AllNonUsers)}
					...
					if @battle.FE == PBFields::CORROSIVEMISTF
						miniscore*=2 if checkAImoves([PBMoves::HEATWAVE,PBMoves::LAVAPLUME,PBMoves::ERUPTION,PBMoves::MINDBLOWN,PBMoves::CHLOROBLAST])
					end
					if @battle.FE == PBFields::CAVE
			...
			when 0xc4 # Solar Beam, Solar Blade
				...
				if @battle.pbWeather!=PBWeather::SUNNYDAY && @attacker.pbHasMove?(PBMoves::SUNNYDAY) && !(@battle.pbCheckGlobalAbility(:AIRLOCK) || @battle.pbCheckGlobalAbility(:CLOUDNINE) || @battle.pbCheckGlobalAbility(:DELTASTREAM) ||
					@battle.pbCheckGlobalAbility(:DESOLATELAND) || @battle.pbCheckGlobalAbility(:PRIMORDIALSEA) || @attacker.item == PBItems::POWERHERB || @attacker.ability == PBAbilities::HEATGENERATOR || @battle.FE == PBFields::UNDERWATER || @battle.FE == PBFields::NEWW || @battle.FE == PBFields::RAINBOWF)
					miniscore = 0.3
				else
			...
			when 0xc8 # Skull Bash
				...
				miniscore *= weaselslashcode()
			when 0xc9 # Fly
				if @attacker.ability == PBAbilities::NOGUARD || @opponent.ability == PBAbilities::NOGUARD || (@attacker.ability==PBAbilities::FAIRYAURA && @battle.FE == PBFields::FAIRYTALEF) || @attacker.ability == PBAbilities::ALOLANTECHNIQUESA || @opponent.ability == PBAbilities::ALOLANTECHNIQUESA
					miniscore = weaselslashcode()
				else
			...
			when 0xd2 # Outrage, Petal Dance, Thrash
				...
				if @mondata.skill>=BESTSKILL
					if @battle.FE == PBFields::SUPERHEATEDF && @attacker.ability != (PBAbilities::OWNTEMPO || PBAbilities::FLOWERPOWER) # Superheated Field
						miniscore*=0.5
					end
			...
			when 0xd8 # Synthesis, Moonlight, Morning Sun
				...
				recoveramount = @attacker.totalhp*0.66 if @battle.pbWeather == PBWeather::SUNNYDAY || @attacker.ability == PBAbilities::HEATGENERATOR
			...
			when 0xec # Dragon Tail, Circle Throw
				miniscore = phasecode() if @opponent.effects[PBEffects::Substitute]<=0 && !(@opponent.effects[PBEffects::Ingrain] || @opponent.ability == PBAbilities::SUCTIONCUPS || @opponent.ability == PBAbilities::GUARDDOG || @opponent.pbNonActivePokemonCount==0 || @opponent.effects[PBEffects::PerishSong]>0 || @opponent.effects[PBEffects::Yawn]>0)
			...
			when 0x103, 0x221 # Spikes / Ceaseless Edge
			...
			when 0x105, 0x220 # Stealth Rocks / Stone Axe
				if !@attacker.pbOpposingSide.effects[PBEffects::StealthRock]
					miniscore = hazardcode()
					miniscore*=1.05 if @attacker.moves.any? {|moveloop| moveloop!=nil && (moveloop.id==PBMoves::SPIKES || moveloop.id==PBMoves::CEASELESSEDGE)}
					if @mondata.skill>=BESTSKILL
            ...
            when 0x262 # Snowscape
				miniscore = snowymountaincode()
			...
			when 0x201, 0x255 # Steel Roller, Ice Spinner
				miniscore*=1.5 if @battle.field.duration > 0 && (@battle.FE == PBFields::GRASSYT || @battle.FE == PBFields::ELECTRICT || @battle.FE == PBFields::MISTYT || @battle.FE == PBFields::PSYCHICT)
			when 0x206 # Terrain Pulse (Temporary)
				miniscore*=0.5 if @battle.FE == PBFields::NONE || @battle.FE == PBFields::INDOORA || @battle.FE == PBFields::INDOORB || @battle.FE == PBFields::INDOORC || @battle.FE == PBFields::CITY || @battle.FE == PBFields::CITYNEW || @battle.FE == PBFields::GUFIELD || @battle.FE == PBFields::DWORLD
			when 0x255 # Ice Spinner
				miniscore*=1.5 if @battle.field.duration > 0 && (@battle.FE == PBFields::GRASSYT || @battle.FE == PBFields::ELECTRICT || @battle.FE == PBFields::MISTYT || @battle.FE == PBFields::PSYCHICT)
			when 0x803 # Big Bang (Infinity)
				miniscore*=1.5 if @battle.field.duration > 0 && (@battle.FE == PBFields::GRASSYT || @battle.FE == PBFields::ELECTRICT || @battle.FE == PBFields::MISTYT || @battle.FE == PBFields::PSYCHICT)
			when 0x163 # Gear Up
			...
			when 0xffe # Z-Conversion / splash
				...
				if !@battle.opponent.is_a?(Array)
					if (@battle.opponent.trainertype==PBTrainers::AMARIA1) && move.id == PBMoves::SPLASH
						score+=200
					end
				end
			when 0x266 # Last Respects
				miniscore*=2 if @attacker.pbFaintedPokemonCount > 2
				miniscore*=3 if @attacker.pbFaintedPokemonCount > 3
			when 0x278 # Gigaton Hammer
				miniscore = hammercode()
			when 0x279 # Revival Blessing
				miniscore*=2 if @attacker.pbFaintedPokemonCount > 1
				miniscore*=3 if @attacker.pbFaintedPokemonCount > 2
				miniscore*=4 if @attacker.pbFaintedPokemonCount > 3
			when 0x281 # Blood Moon
				miniscore = mooncode()
        end
		score*=miniscore
		score=score.to_i
...
#All functions here return a modifier to the original score, similar to miniscore
	def sleepcode
		...
		miniscore*=0.2 if @opponent.ability == PBAbilities::GOODASGOLD && @move.basedamage=0
	...
	def paracode
		...
		if !pbAIfaster?() && (pbRoughStat(@opponent,PBStats::SPEED)/2.0)<@attacker.pbSpeed && @battle.trickroom==0
			if hasbadmoves(40)
				miniscore+=25 if @move.addlEffect == 100 && !@opponent.item == PBItems::COVERTCLOAK # help nuzzle 
			end
		end
		return miniscore
	end
	...
	def flinchcode
		...
		miniscore*= 1.3 if @attacker.pbOpposingSide.effects[PBEffects::LightScreen] || @attacker.pbOpposingSide.effects[PBEffects::Reflect] || @attacker.pbOpposingSide.effects[PBEffects::AuroraVeil] || @attacker.pbOpposingSide.effects[PBEffects::SolarArray]
	...
	def selfstatboost(stats)
		...
		for i in 1...stats.length
			...
			case i
				...
				when PBStats::EVASION
					tank = true
					miniscore*=0.2 if @opponent.ability == PBAbilities::NOGUARD || @attacker.ability == PBAbilities::NOGUARD || checkAIaccuracy() || (@attacker.ability==PBAbilities::FAIRYAURA && @battle.FE == PBFields::FAIRYTALEF) || @opponent.ability == PBAbilities::ALOLANTECHNIQUESA || @attacker.ability == PBAbilities::ALOLANTECHNIQUESA
		...
		miniscore*=2 if @opponent.effects[PBEffects::GlaiveRush]>0
		miniscore*=2 if @attacker.pbHasMove?(PBMoves::STOREDPOWER)
        miniscore*=1.5 if @attacker.pbHasMove?(PBMoves::BODYPRESS)
		...
		if move.basedamage>0
			...
			miniscore*=0 if @attacker.ability == PBAbilities::CONTRARY || @attacker.ability == PBAbilities::ALOLANTECHNIQUESB
	...
	def oppstatboost(stats)
		...
		if @opponent.index != @attacker.pbPartner.index
			...
			miniscore*=0 if @opponent.ability == PBAbilities::CONTRARY || @opponent.ability == PBAbilities::ALOLANTECHNIQUESB
	...
	def oppstatdrop(stats)
		...
		return @move.basedamage > 0 ? 1 : 0 if @opponent.ability == PBAbilities::CLEARBODY || @opponent.ability == PBAbilities::WHITESMOKE || @opponent.ability == PBAbilities::GOODASGOLD
		#stats should be an array of the stat boosts like so: [ATK,DEF,SPE,SPA,SPD,ACC,EVA] with nils in unaffected stats
		#coil, for example, would be [1,1,0,0,0,1,0]
		stats.unshift(0) #this is required to make the next line work correctly
		...
		if stats[PBStats::SPEED]>0  #speed stuff
			...
			miniscore*=0.2 if @opponent.ability == PBAbilities::COMPETITIVE || @opponent.ability == PBAbilities::DEFIANT || @opponent.ability == PBAbilities::CONTRARY || @opponent.ability == PBAbilities::ALOLANTECHNIQUESB
		else    #non-speed stuff
			miniscore*=0.1 if @opponent.ability == PBAbilities::COMPETITIVE || @opponent.ability == PBAbilities::DEFIANT || @opponent.ability == PBAbilities::CONTRARY || @opponent.ability == PBAbilities::ALOLANTECHNIQUESB
			miniscore*=1.1 if @mondata.partyroles.any? {|roles| roles.include?(PBMonRoles::SWEEPER)}
		end
	...
	def defogcode
		...
		miniscore*=3 if @opponent.pbOwnSide.effects[PBEffects::SolarArray]>0
	...
	def entraincode(score)
		...
		if @opponent.index != @attacker.pbPartner.index
			...
			case @opponent.ability
				...
				when PBAbilities::QUICKSTART then score +=50
			end
		end
		return score
	end
	...
	def skillswapcode
		...
		return 0 if (PBStuff::FIXEDABILITIES).include?(@attacker.ability) && @attacker.ability != PBAbilities::RAGINGBLAZE
		return 0 if (PBStuff::FIXEDABILITIES).include?(@opponent.ability) && @opponent.ability != PBAbilities::RAGINGBLAZE
	...
	def ohkode
		...
		return 3.5 if @opponent.effects[PBEffects::LockOn]>0 || @opponent.ability==PBAbilities::NOGUARD || @attacker.ability==PBAbilities::NOGUARD || (@attacker.ability==PBAbilities::FAIRYAURA && @battle.FE == PBFields::FAIRYTALEF) || @opponent.ability==PBAbilities::ALOLANTECHNIQUESA || @attacker.ability==PBAbilities::ALOLANTECHNIQUESA
		return 0.7
	end
	...
	def screencode
		...
		return 0 if @attacker.pbOwnSide.effects[PBEffects::SolarArray]>0 && @move.function == 0x919
		return 0 if @attacker.pbOwnSide.effects[PBEffects::SolarArray]>3
		miniscore=1.2
		miniscore*=0.2 if @attacker.pbOwnSide.effects[PBEffects::AuroraVeil]>0
		miniscore*=0.2 if @attacker.pbOwnSide.effects[PBEffects::SolarArray]>0
		...
		if @attacker.index == 2 # for partners to guess if the player will use solar array
			miniscore *= 0.3 if @attacker.pbPartner.pbHasMove?(PBMoves::SOLARARRAY)
		end
	...
	def nevermisscode(score)
		...
		return miniscore if @attacker.ability == PBAbilities::NOGUARD || @opponent.ability == PBAbilities::NOGUARD || (@attacker.ability==PBAbilities::FAIRYAURA && @battle.FE == PBFields::FAIRYTALEF) || @attacker.ability == PBAbilities::ALOLANTECHNIQUESA || @opponent.ability == PBAbilities::ALOLANTECHNIQUESA
	...
	def lockoncode
		return 0 if @opponent.effects[PBEffects::LockOn]>0 || @opponent.effects[PBEffects::Substitute]>0 || @attacker.ability == PBAbilities::NOGUARD && @opponent.ability == PBAbilities::NOGUARD || (@attacker.ability==PBAbilities::FAIRYAURA && @battle.FE == PBFields::FAIRYTALEF) || @attacker.ability == PBAbilities::ALOLANTECHNIQUESA && @opponent.ability == PBAbilities::ALOLANTECHNIQUESA
		miniscore=1.0
	...
	def forecode5me #after doing hundreds of these this is how i survive
		...
		if @opponent.pbHasType?(:GHOST)
			miniscore*=1.5
			miniscore*=5 if !(@attacker.ability = PBAbilities::SCRAPPY || @attacker.ability = PBAbilities::MINDSEYE) && !@attacker.moves.any? {|moveloop| moveloop!=nil && moveloop.basedamage > 0 && moveloop.pbType(@attacker) != PBTypes::NORMAL && moveloop.pbType(@attacker) != PBTypes::FIGHTING}
		end
		return miniscore
	end
	...
	def protectcode
		...
		miniscore*=4 if @attacker.ability == PBAbilities::SLOWSTART && @attacker.turncount<4
		miniscore*=4 if @attacker.ability == PBAbilities::QUICKSTART && @attacker.turncount>4
	...
	def hammercode
		return 0 if @attacker.effects[PBEffects::GigatonHammer] && @attacker.lastMoveUsed == PBMoves::GIGATONHAMMER
		miniscore = 1.0
		return miniscore
	end
	...
	def twoturncode
		...
		miniscore*=0.7 if @attacker.pbOwnSide.effects[PBEffects::Tailwind]>0 || @attacker.pbOwnSide.effects[PBEffects::Reflect]>0 || @attacker.pbOwnSide.effects[PBEffects::LightScreen]>0 || @attacker.pbOwnSide.effects[PBEffects::AuroraVeil]>0 || @attacker.pbOwnSide.effects[PBEffects::SolarArray]>0
	...
	def endurecode
		...
		return 0 if (@battle.pbWeather==PBWeather::HAIL && !@attacker.pbHasType?(:ICE)) || (@battle.pbWeather==PBWeather::SANDSTORM && !(@attacker.pbHasType?(:ROCK) || @attacker.pbHasType?(:GROUND) || @attacker.pbHasType?(:STEEL))) || (@battle.pbWeather==PBWeather::NUCLEARFALLOUT && !(@attacker.pbHasType?(:NUCLEAR) || @attacker.pbHasType?(:STEEL)))
	...
	def phasecode
		return 0 if @opponent.effects[PBEffects::Ingrain] || @opponent.ability == PBAbilities::SUCTIONCUPS || @opponent.ability == PBAbilities::GUARDDOG || @opponent.pbNonActivePokemonCount==0
		...
		miniscore*=0.7 if @opponent.ability == PBAbilities::QUICKSTART
		...
		miniscore*=0.7 if @opponent.ability == PBAbilities::PETRIFY # Uranium
		miniscore*=0.7 if @opponent.ability == PBAbilities::SUPERSWEETSYRUP # Paldea DLC
		miniscore*=0.7 if @opponent.ability == PBAbilities::INTERFERENCE # Multicore
	...
	def pivotcode
		...
		miniscore*=5   if @attacker.species==PBSpecies::PALAFIN && @attacker.form!=1 && (@battle.FE != PBFields::WATERS || @battle.FE != PBFields::WATERS) && @move.id == PBMoves::FLIPTURN
		...
		miniscore*=1.1 if @attacker.ability == PBAbilities::PETRIFY # Uranium
		miniscore*=1.1 if @attacker.ability == PBAbilities::SUPERSWEETSYRUP# Paldea DLC
		miniscore*=1.1 if @attacker.ability == PBAbilities::INTERFERENCE # Multicore
		...
		miniscore*=1.5 if @mondata.partyroles.any? {|role| role.include?(PBMonRoles::SWEEPER)} && (@move.id == PBMoves::PARTINGSHOT || @move.id == PBMoves::SHEDTAIL || @move.id == PBMoves::CHILLYRECEPTION)
		miniscore*=1.2 if @mondata.partyroles.any? {|role| role.include?(PBMonRoles::SWEEPER)} && (@move.id == PBMoves::UTURN || @move.id == PBMoves::VOLTSWITCH || @move.id == PBMoves::FLIPTURN) && !pbAIfaster?()
	...
	def suncode
		...
		if @attacker.ability == PBAbilities::NOVABURNER || @attacker.ability == PBAbilities::UBERSTRAT
			miniscore*=2
			miniscore*=2 if notOHKO?(@attacker, @opponent, true)
			miniscore*=3 if seedProtection?(@attacker)
		end
...
miniscore*=5   if (@battle.FE == PBFields::GRASSYT && @attacker.species == PBSpecies::ROTOM) && @battle.pbWeather != PBWeather::RAINDANCE # Madame Meganium battle
	...
	def sandcode
		...
		miniscore = 1.0
		miniscore*=0.2 if @attacker.ability == PBAbilities::FORECAST && [:WATER,:GRASS,:FIGHTING].any? {|type| @opponent.pbHasType?(type) }
	...
	def nuclearcode	#! I think the AI wants to switch out...
		return 0 if @battle.pbWeather==PBWeather::NUCLEARFALLOUT
		miniscore = 1.0
		#miniscore*=1.3 if (@mondata.attitemworks && @attacker.item == PBItems::SMOOTHROCK) #todo item that extends duration
		miniscore*=2 if @battle.pbWeather!=0 && @battle.pbWeather!=PBWeather::NUCLEARFALLOUT #? AI switches for triggering weather
		miniscore*= (@attacker.pbHasType?(:NUCLEAR)|| @attacker.pbHasType?(:STEEL)) ? 1.3 : 0.7 #? matching tyoe so it doesn't suffer
		miniscore*=1.5 if @attacker.pbHasType?(:NUCLEAR) #? because these get a boost in here
		#if @attacker.ability == PBAbilities::SANDRUSH #?abilities that get bonuses
		#	miniscore*=2
		#	miniscore*=2 if notOHKO?(@attacker, @opponent, true) #! this one has the notOHKO, meaning apply the speed bonus only if the target isn't dead
		#	miniscore*=3 if seedProtection?(@attacker)
		#end
		miniscore*=1.3 if @attacker.ability == PBAbilities::SANDVEIL #?abilities that do get more bonuses
		miniscore*=0.5 if @attacker.pbHasMove?(PBMoves::MOONLIGHT) || @attacker.pbHasMove?(PBMoves::SYNTHESIS) || @attacker.pbHasMove?(PBMoves::MORNINGSUN) || @attacker.pbHasMove?(PBMoves::GROWTH) || @attacker.pbHasMove?(PBMoves::SOLARBEAM) || @attacker.pbHasMove?(PBMoves::SOLARBLADE) #? avoid if you have these moves
		miniscore*=1.5 if @attacker.pbHasMove?(PBMoves::SHOREUP) #? trigger the field this if you have these moves
		miniscore*=1.5 if @attacker.ability == PBAbilities::SANDFORCE #? ability that gets bonuses
		return miniscore
	end
	...
	def hailcode
		...
		miniscore*=2 if @attacker.pbHasMove?(PBMoves::SOLARARRAY)
	...
	def suckercode
		...
		if @attacker.lastMoveUsed==PBMoves::SUCKERPUNCH # Sucker Punch last turn
			if checkAImoves(PBStuff::PROTECTMOVE)
				return miniscore
			else
	...
	def doubleshockcode
		return 0 if !@attacker.pbHasType?(:ELECTRIC)
		miniscore= (1-@opponent.pbNonActivePokemonCount*0.05)
		if @initial_scores[@score_index]<100
			miniscore*=0.9
			miniscore*=0.5 if getAIMemory().any? {|moveloop| moveloop!=nil && moveloop.isHealingMove?}
		...
		miniscore*=0.5 if @initial_scores.length>0 && hasgreatmoves()
		miniscore*=0.7 if @attacker.pbNonActivePokemonCount==0 && @opponent.pbNonActivePokemonCount!=0
		effcheck = PBTypes.getCombinedEffectiveness(@opponent.type1,(PBTypes::ELECTRIC),(PBTypes::ELECTRIC))
		miniscore*=1.5 if effcheck > 4
		miniscore*=0.5 if effcheck < 4
    	effcheck = PBTypes.getCombinedEffectiveness(@opponent.type2,(PBTypes::ELECTRIC),(PBTypes::ELECTRIC))
		miniscore*=1.5 if effcheck > 4
		miniscore*=0.5 if effcheck < 4
		effcheck = PBTypes.getCombinedEffectiveness(checkAIbestMove().pbType(@opponent),(PBTypes::ELECTRIC),(PBTypes::ELECTRIC))
		miniscore*=1.5 if effcheck > 4
		miniscore*=0.5 if effcheck < 4
		return miniscore
	end
	...
	def moldbreakeronalaser
		...
		case @opponent.ability
			...
			when PBAbilities::FLASHFIRE, PBAbilities::WELLBAKEDBODY
			...
			when PBAbilities::WINDRIDER
				miniscore*=3 if (PBStuff::WINDMOVE).include?(id) && damcount==1
			when PBAbilities::LEVITATE,PBAbilities::EARTHEATER
			...
			when PBAbilities::LEADSKIN
				miniscore*=3 if @move.pbType(@attacker)==PBTypes::NUCLEAR && damcount==1
			when PBAbilities::DISENCHANT
				miniscore*=3 if @move.pbType(@attacker)==PBTypes::FAIRY && damcount==1
			when PBAbilities::SNOWCLOAK
				miniscore*=1.1 if @battle.pbWeather!=PBWeather::HAIL
			when PBAbilities::FURCOAT, PBAbilities::HEAVYMETAL
				miniscore*=1.5 if @attacker.attack>@attacker.spatk
			when PBAbilities::FLUFFY
				miniscore*=1.5
				miniscore*=0.5 if @move.pbType(@attacker)==PBTypes::FIRE
			when PBAbilities::PURIFYINGSALT
				miniscore*=1.3 if @move.pbType(@attacker)==PBTypes::GHOST
    ...
    def saltcurecode
	    return 0 if @opponent.effects[PBEffects::SaltCure]
	    miniscore=1.0
	    miniscore*=4 if @opponent.pbHasType?(:WATER)
		miniscore*=3 if @opponent.pbHasType?(:STEEL)
		return miniscore
	end
	...
	def brickbreakcode(attacker=@attacker)
		...
		miniscore*=5.0 if attacker.pbOpposingSide.effects[PBEffects::SolarArray]>0
		return miniscore
	end
		...
		if @mondata.skill>=BESTSKILL
			if !@battle.pbIsWild?
				...
				for key in oppparty.keys
					movecheck = true if oppparty[key].any? {|moveloop|moveloop!=nil && (moveloop.id==PBMoves::DEFOG || move.id ==PBMoves::RAPIDSPIN || move.id ==PBMoves::TIDYUP)}
				end
	...
	def snowymountaincode
		return 0 if @battle.FE == PBFields::SNOWYM || @battle.FE == PBFields::UNDERWATER || @battle.FE == PBFields::NEWW
		miniscore=1.0
		miniscore*= getFieldDisruptScore(@attacker,@opponent)
		miniscore*=2 if pbPartyHasType?(PBTypes::ICE)
		miniscore*=2 if !@attacker.pbHasType?(:ICE) && @opponent.pbHasType?(:DRAGON)
		miniscore*=0.5 if @attacker.pbHasType?(:FIRE)
		miniscore*=0.5 if @opponent.pbHasType?(:ICE)
		miniscore*=2 if @attacker.pbHasType?(:ICE) && @opponent.attack>@opponent.spatk
		miniscore*=2 if @mondata.attitemworks && @attacker.item == PBItems::AMPLIFIELDROCK
		return miniscore
	end
	...
	def flowershieldcode(score)
		...
		if (@battle.FE == PBFields::FLOWERGARDENF && @battle.field.counter>0) || @battle.FE == PBFields::FAIRYTALEF
			...
			score=0 if @attacker.ability == PBAbilities::CONTRARY || @opponent.ability == PBAbilities::ALOLANTECHNIQUESB
		end
		return score
	end
	...
	def flingcode
		return 0 if @attacker.item == 0 || @battle.pbIsUnlosableItem(@attacker,@attacker.item) || !@attacker.itemWorks? || (pbIsBerry?(@attacker.item) && @opponent.ability == PBAbilities::UNNERVE)
		miniscore=1.0
		case @attacker.item
			...
			when PBItems::SAIYANITE then miniscore*=1.3 if @opponent.pbCanParalyze?(false) && @opponent.ability != PBAbilities::QUICKFEET
	...
	def recyclecode
		...
		miniscore*=2 if @attacker.ability == PBAbilities::CHEEKPOUCH
	...
	def embarcode(opponent=@opponent)
		return 0 if opponent.item==0 || opponent.effects[PBEffects::Substitute]>0 || !opponent.itemWorks?
		miniscore = 1.1
	...
	def perishcode
		...
		return 0 if @attacker.pbNonActivePokemonCount==0
		miniscore=1.0
		miniscore*=1.5 if @attacker.pbHasMove?(PBMoves::UTURN) || @attacker.pbHasMove?(PBMoves::VOLTSWITCH) || @attacker.pbHasMove?(PBMoves::PARTINGSHOT) || @attacker.pbHasMove?(PBMoves::CHILLYRECEPTION) || @attacker.pbHasMove?(PBMoves::SHEDTAIL) || @attacker.pbHasMove?(PBMoves::FLIPTURN)
		...
		miniscore*=0.1 if (@opponent.ability == PBAbilities::SHADOWTAG || @attacker.effects[PBEffects::MeanLook]>0) && !(@attacker.pbHasMove?(PBMoves::UTURN) || @attacker.pbHasMove?(PBMoves::CHILLYRECEPTION) || @attacker.pbHasMove?(PBMoves::SHEDTAIL) || @attacker.pbHasMove?(PBMoves::FLIPTURN) || @attacker.pbHasMove?(PBMoves::VOLTSWITCH) || @attacker.pbHasMove?(PBMoves::PARTINGSHOT))
	...
	def outragecode(score)
		return 1.3 if @attacker.ability == PBAbilities::OWNTEMPO || @attacker.ability == PBAbilities::FLOWERPOWER
		miniscore=1.0
	...
	def spectralthiefcode
		...
		miniscore*=(-1) if @attacker.ability == PBAbilities::CONTRARY || @attacker.ability == PBAbilities::ALOLANTECHNIQUESB
	...
	def spotlightcode
		...
		if @opponent.ability == PBAbilities::FLASHFIRE || @opponent.ability == PBAbilities::WELLBAKEDBODY
		...
		elsif @opponent.ability == PBAbilities::EARTHEATER
			miniscore*=3 if bestmove1.pbType(@attacker.pbOpposing1) ==PBTypes::GROUND || bestmove2.pbType(@attacker.pbOpposing2) ==PBTypes::GROUND
		elsif @opponent.ability == PBAbilities::LEADSKIN
			miniscore*=3 if bestmove1.pbType(@attacker.pbOpposing1) ==PBTypes::NUCLEAR || bestmove2.pbType(@attacker.pbOpposing2) ==PBTypes::NUCLEAR
		elsif @opponent.ability == PBAbilities::DISENCHANT
			miniscore*=3 if bestmove1.pbType(@attacker.pbOpposing1) ==PBTypes::FAIRY || bestmove2.pbType(@attacker.pbOpposing2) ==PBTypes::FAIRY
		end
		miniscore*=2 if (bestmove1.isContactMove? || bestmove2.isContactMove?) && checkAImoves([PBMoves::KINGSSHIELD, PBMoves::BANEFULBUNKER, PBMoves::SPIKYSHIELD, PBMoves::SILKTRAP])
...
######################################################
	...
	def pbGetMonRoles(targetmon=nil)
		...
		for mon in party
			...
			monRoles.push(PBMonRoles::TANK) 		if (mon.ev[0]>251 && !healingmove) || mon.item==(PBItems::ASSAULTVEST) || (mon.species==(PBSpecies::LILLIGANT) && mon.form == 7)
			...
			monRoles.push(PBMonRoles::WEATHERSETTER)if weathermove || (mon.ability == PBAbilities::CHERNOBYL) || (mon.ability == PBAbilities::DROUGHT) || (mon.ability == PBAbilities::ORICHALCUMPULSE) || (mon.ability == PBAbilities::SANDSTREAM) || (mon.ability == PBAbilities::DRIZZLE) || (mon.ability == PBAbilities::SNOWWARNING) || (mon.ability == PBAbilities::PRIMORDIALSEA) || (mon.ability == PBAbilities::DESOLATELAND) || (mon.ability == PBAbilities::DELTASTREAM) || (mon.ability == PBAbilities::STORMBRINGER) || (mon.ability == PBAbilities::SANDSPIT)
			monRoles.push(PBMonRoles::FIELDSETTER) 	if fieldmove || (mon.ability == PBAbilities::GRASSYSURGE) || (mon.ability == PBAbilities::ELECTRICSURGE) || (mon.ability == PBAbilities::MISTYSURGE) || (mon.ability == PBAbilities::PSYCHICSURGE) || (mon.ability == PBAbilities::GLITCHYSURGE) || (mon.ability == PBAbilities::TRICKSTER) || (mon.ability == PBAbilities::ABSTRACT) || mon.item==(PBItems::AMPLIFIELDROCK) || (mon.ability == PBAbilities::HADRONENGINE)
	...
	def pbSereneGraceCheck(miniscore)
		...
		if @move.addlEffect != 100
			...
			addedeffect*=2 if @attacker.ability == PBAbilities::SERENEGRACE || @battle.FE == PBFields::RAINBOWF || @attacker.ability == PBAbilities::STARFALLGRACE
	...
	def pbRoughAccuracy(move,attacker,opponent)
		...
		# Override accuracy
		return 100 if attacker.ability == PBAbilities::NOGUARD || opponent.ability == PBAbilities::NOGUARD || (attacker.ability == PBAbilities::FAIRYAURA && @battle.FE == PBFields::FAIRYTALEF) && @mondata.skill>=MEDIUMSKILL || attacker.ability == PBAbilities::ALOLANTECHNIQUESA || opponent.ability == PBAbilities::ALOLANTECHNIQUESA 
...
=end
		if @mondata.skill>=MEDIUMSKILL
			baseaccuracy=50 if (@battle.pbWeather==PBWeather::SUNNYDAY || @attacker.ability == PBAbilities::HEATGENERATOR) && (move.function==0x08 || move.function==0x15) # Thunder, Hurricane
	  	end
		# Accuracy stages
		...
		if @mondata.skill>=MEDIUMSKILL
			...
			#! here's the foggy weather reducing the fuck out of your accuracies
			accuracy*=0.9 if @battle.pbWeather==PBWeather::FOGGYXOXO && ( ( ((attacker.ability!=PBAbilities::COMPOUNDEYES) || (attacker.ability!=PBAbilities::KEENEYE) || (attacker.ability!=PBAbilities::WONDERSKIN) || (attacker.ability!=PBAbilities::VICTORYSTAR)) && !moldBreakerCheck(opponent) ) || ((attacker.item!=PBItems::WIDELENS) || (attacker.item!=PBItems::ZOOMLENS) || (attacker.item!=PBItems::SCOPELENS) || (attacker.item!=PBItems::ZOOMLENS) || (attacker.item!=PBItems::TECHGLASSES) || (attacker.item!=PBItems::SAFETYGOGGLES) || (attacker.item!=PBItems::BLACKGLASSES) || (attacker.item!=PBItems::WISEGLASSES)) ) && attacker.ability!=PBAbilities::HUSTLE

			accuracy*=0.2 if @battle.pbWeather==PBWeather::FOGGYXOXO && attacker.ability==PBAbilities::HUSTLE #? just switch out man
		end
		evasion = 100 if attacker.ability == PBAbilities::KEENEYE || attacker.ability == PBAbilities::MINDSEYE
    	evasion = 100 if @mondata.skill>=BESTSKILL && @battle.FE == PBFields::ASHENB && [PBAbilities::OWNTEMPO,PBAbilities::FLOWERPOWER,PBAbilities::INNERFOCUS,PBAbilities::PUREPOWER,PBAbilities::SANDVEIL,PBAbilities::STEADFAST].include?(attacker.ability) && opponent.ability != PBAbilities::UNNERVE
	...
	def pbAIfaster?(attackermove=nil, opponentmove=nil, attacker=@attacker, opponent=@opponent)
		...
		for battler in [attacker, opponent]
			...
			pri -= 1 if battler.ability == PBAbilities::MYCELIUMMIGHT && battlermove.basedamage==0 # Is status move
			pri += 1 if @battle.FE == PBFields::GRASSYT && battlermove.id == PBMoves::GRASSYGLIDE
			pri += 1 if battler.ability == PBAbilities::GALEWINGS && battlermove.type==PBTypes::FLYING && ((battler.hp >= (battler.totalhp*0.75).floor) || ((@battle.FE == PBFields::MOUNTAIN || @battle.FE == PBFields::SNOWYM) && @battle.pbWeather == PBWeather::STRONGWINDS))
	...
	def hpGainPerTurn(attacker=@attacker)
		...
		if @battle.FE != 0
			...
			healing += 0.0625 if (@battle.FE == PBFields::DESERTF || @battle.FE == PBFields::CAVE) && attacker.ability == PBAbilities::EARTHEATER
	...
	def pbTypeModNoMessages(type=@move.type,attacker=@attacker,opponent=@opponent,move=@move,skill=@mondata.skill)
		...
		if !moldBreakerCheck(attacker)
			case opponent.ability
				...
				when PBAbilities::PLOTARMOR 		then return 0 if type == ((PBTypes::COSMIC || PBTypes::NUCLEAR) || (secondtype==PBTypes::NUCLEAR || secondtype==PBTypes::COSMIC))
				...
				when PBAbilities::EARTHEATER 	then return -1 if type == PBTypes::GROUND || secondtype==PBTypes::GROUND && opponent.effects[PBEffects::HealBlock]==0
				...
				when PBAbilities::WINDRIDER 	then return 0  if (PBStuff::WINDMOVE).include?(id)
				when PBAbilities::WELLBAKEDBODY then return -1 if type == PBTypes::FIRE || secondtype==PBTypes::FIRE
				when PBAbilities::LEADSKIN      then return -1 if type == PBTypes::NUCLEAR || secondtype==PBTypes::NUCLEAR
				when PBAbilities::DISENCHANT    then return -1 if type == PBTypes::FAIRY || secondtype==PBTypes::FAIRY
		...
		typemod *= 4 if id == PBMoves::FREEZEDRY && opponent.pbHasType?(PBTypes::WATER) && !@battle.typesInverted?
		typemod *= 4 if id == PBMoves::INFERNALBLADE && opponent.pbHasType?(PBTypes::FAIRY) && !@battle.typesInverted?
		...
		if @battle.FE == PBFields::DRAGONSD && opponent.ability == PBAbilities::MULTISCALE && !moldBreakerCheck(attacker) && # Dragons Den Multiscale
	...
	def pbChangeMove(move,attacker)
		...
		case move.id
			when PBMoves::WEATHERBALL
				...
				move.type=PBTypes::FIRE if (weather==PBWeather::SUNNYDAY && !attacker.hasWorkingItem(:UTILITYUMBRELLA)) || @attacker.ability == PBAbilities::HEATGENERATOR
				...
				move.type=PBTypes::NUCLEAR if weather==PBWeather::NUCLEARFALLOUT
				move.type=PBTypes::ELECTRIC if weather==PBWeather::THUNDERSTORM
				move.type=PBTypes::FLYING if weather==PBWeather::FOGGYXOXO
	...
	def getAbilityDisruptScore(attacker,opponent)
		...
		case opponent.ability
			...
			when PBAbilities::PLOTARMOR
				for i in attacker.moves
					next if i.nil?
					nuclearmove=i if i.pbType(attacker) == PBTypes::NUCLEAR
					cosmicmove=i if i.pbType(attacker) == PBTypes::COSMIC
				end
				if attacker.moves.any? {|moveloop| moveloop!=nil && moveloop.pbType(attacker) == (PBTypes::NUCLEAR || PBTypes::COSMIC)}
					abilityscore*=3 if attacker.moves.all? {|moveloop| moveloop!=nil && moveloop.pbType(attacker) == (PBTypes::NUCLEAR || COSMIC)}
					abilityscore*=2 if pbTypeModNoMessages(nuclearmove.pbType(attacker),attacker,opponent,nuclearmove)>4
					abilityscore*=2 if pbTypeModNoMessages(cosmicmove.pbType(attacker),attacker,opponent,cosmicmove)>4
				end
			when PBAbilities::LEVITATE,PBAbilities::EARTHEATER
				for i in attacker.moves
					next if i.nil?
			...
			when PBAbilities::SERENEGRACE
				abilityscore*=1.3
			when PBAbilities::STARFALLGRACE
				abilityscore*=1.3
			when PBAbilities::PUREPOWER, PBAbilities::HUGEPOWER, PBAbilities::PUREFOCUS
				abilityscore*=2
			when PBAbilities::SOUNDPROOF
			...
			when PBAbilities::TECHNICIAN
				abilityscore*=1.3
			when PBAbilities::TROUBLESHOOTER
				abilityscore*=1.3
			...
			when PBAbilities::SHEERFORCE
				abilityscore*=1.2
			when PBAbilities::CONTRARY || PBAbilities::ALOLANTECHNIQUESB
				abilityscore*=1.4
			...
			when PBAbilities::PARENTALBOND
				abilityscore*=3
			when PBAbilities::HYDRABOND #HYDRABOND
				abilityscore*=4
	...
	def getFieldDisruptScore(attacker=@attacker,opponent=@opponent,fieldeffect=@battle.FE, violent=false)
		...
		case fieldeffect
			...
			when PBFields::BIGTOPA # Big Top field
				...
				fieldscore*=0.5 if pbPartyHasType?(PBTypes::FIGHTING ||PBTypes::FG ||PBTypes::GFS ||PBTypes::PFD)
				...
				fieldscore*=1.5 if checkAImoves([PBMoves::SING,PBMoves::DRAGONDANCE,PBMoves::QUIVERDANCE,PBMoves::VICTORYDANCE],aimem)
			...
			when PBFields::RAINBOWF # Rainbow field
				...
				fieldscore*=0.5 if pbPartyHasType?(PBTypes::NORMAL) || pbPartyHasType?(PBTypes::NPG)
			...
			when PBFields::CORROSIVEMISTF # Corromist field
				if violent
					if !PBStuff::INVULEFFECTS.any? {|eff| opponent.effects[eff] == true } && !(PBStuff::TWOTURNMOVE.include?(opponent.effects[PBEffects::TwoTurnAttack]) && pbAIfaster?(nil,nil,attacker,opponent)) && opponent.ability != PBAbilities::FLASHFIRE && opponent.ability != PBAbilities::WELLBAKEDBODY
			...
			when PBFields::HOLYF # Holy field
				...
				fieldscore*=0.6 if pbPartyHasType?(PBTypes::NORMAL) || pbPartyHasType?(PBTypes::FAIRY) || pbPartyHasType?(PBTypes::NPG)
			...
			when PBFields::INVERSEF # Inverse field
				...
				fieldscore*=0.3 if pbPartyHasType?(PBTypes::NORMAL) || pbPartyHasType?(PBTypes::NPG)
...
################################################################################
	...
	def getItemScore
		...
		return if @attacker.effects[PBEffects::Embargo]>0 || @attacker.underAncientPressure?
		...
		for i in items
			...
			itemscore*=0.8 if @attacker.pbOwnSide.effects[PBEffects::SolarArray]>0
...
################################################################################
	...
	def getSwitchInScoresParty(hard_switch)
		...
		for partyindex in 0...party.length
			...
			if !i.moves.any? {|moveloop| moveloop != nil && moveloop.id != 0 && moveloop.id != PBMoves::LASTRESPECTS}
				partyScores.push(-9000)
				PBDebug.log(sprintf("Last Respects later- Score: -9000\n")) if $INTERNAL
				$ai_log_data[@attacker.index].switch_scores.push(-9000)
				$ai_log_data[@attacker.index].switch_name.push(PBSpecies.getName(i.pokemon.species))
				next
			end
			...
			if @mondata.skill >= HIGHSKILL
				if theseRoles.include?(PBMonRoles::SWEEPER)
					...
					rolescore-=200 if party.length>2 && @attacker.pbHasMove?(PBMoves::LASTRESPECTS)
				end
				...
				if theseRoles.include?(PBMonRoles::WEATHERSETTER)
					rolescore+=30 if @battle.weather!=PBWeather::NUCLEARFALLOUT
					rolescore-=60 if @battle.weather==PBWeather::NUCLEARFALLOUT
					if (i.ability == PBAbilities::DROUGHT) || (nonmegaform.ability == PBAbilities::DROUGHT) || i.pbHasMove?(PBMoves::SUNNYDAY)
						rolescore+=60 if @battle.weather!=PBWeather::SUNNYDAY && @battle.weather!=PBWeather::NUCLEARFALLOUT
					elsif (i.ability == PBAbilities::DRIZZLE) || (nonmegaform.ability == PBAbilities::DRIZZLE) || i.pbHasMove?(PBMoves::RAINDANCE)
						rolescore+=60 if @battle.weather!=PBWeather::RAINDANCE && @battle.weather!=PBWeather::NUCLEARFALLOUT
					elsif (i.ability == PBAbilities::SANDSTREAM) || (nonmegaform.ability == PBAbilities::SANDSTREAM) || i.pbHasMove?(PBMoves::SANDSTORM) || (i.ability == PBAbilities::SANDSPIT)
						rolescore+=60 if @battle.weather!=PBWeather::SANDSTORM && @battle.weather!=PBWeather::NUCLEARFALLOUT
					elsif (i.ability == PBAbilities::SNOWWARNING) || (nonmegaform.ability == PBAbilities::SNOWWARNING) || i.pbHasMove?(PBMoves::HAIL)
						rolescore+=60 if @battle.weather!=PBWeather::HAIL && @battle.weather!=PBWeather::NUCLEARFALLOUT
						...
						rolescore+=60 if @battle.weather!=PBWeather::NUCLEARFALLOUT
					elsif (i.ability == PBAbilities::CHERNOBYL) || (nonmegaform.ability == PBAbilities::CHERNOBYL)
						rolescore+=600
					end
				end
			end
			...
			case @battle.weather
				...
				when PBWeather::SUNNYDAY
					...
					weatherscore+=80 if (i.ability == PBAbilities::NOVABURNER)
					weatherscore+=80 if (i.ability == PBAbilities::UBERSTRAT)
				...
				when PBWeather::NUCLEARFALLOUT
					weatherscore+=100 if (i.ability == PBAbilities::MAGICGUARD) || (i.ability == PBAbilities::BULLETPROOF) || i.hasType?(:NUCLEAR) || i.hasType?(:STEEL)
				when PBWeather::THUNDERSTORM
					weatherscore+=25 if (i.ability == PBAbilities::LIGHTNINGROD) || i.hasType?(:ELECTRIC)
					weatherscore-=80 if i.hasType?(:FLYING)
				when PBWeather::FOGGYXOXO
					weatherscore+=80 if ( ([PBAbilities::COMPOUNDEYES, PBAbilities::KEENEYE, PBAbilities::WONDERSKIN, PBAbilities::VICTORYSTAR].include?(i.ability)) || ([PBItems::WIDELENS, PBItems::ZOOMLENS, PBItems::SCOPELENS, PBItems::TECHGLASSES, PBItems::SAFETYGOGGLES, PBItems::BLACKGLASSES, PBItems::WISEGLASSES].include?(i.item)) ) && i.ability!=PBAbilities::HUSTLE
					weatherscore-=80 if i.ability==PBAbilities::HUSTLE
				end
			if @battle.trickroom>0 #* why here?
			...
			if @mondata.skill >= HIGHSKILL
				case i.ability
					...
					when PBAbilities::DROUGHT,PBAbilities::DESOLATELAND
						...
						abilityscore+=40 if ((@opponent.ability == PBAbilities::PRIMORDIALSEA) || (@opponent.pbPartner.ability == PBAbilities::PRIMORDIALSEA)) && @battle.state.effects[PBEffects::HeavyRain]
						for specificmemory in [aimem,aimem2]
					...
					when PBAbilities::DRIZZLE,PBAbilities::PRIMORDIALSEA
						...
						abilityscore+=40 if ((@opponent.ability == PBAbilities::DESOLATELAND) || (@opponent.pbPartner.ability == PBAbilities::DESOLATELAND)) && @battle.state.effects[PBEffects::HarshSunlight]
						for specificmemory in [aimem,aimem2]
					...
					when PBAbilities::POISONHEAL,PBAbilities::TOXICBOOST,PBAbilities::IMMUNITY
						abilityscore+=20 if checkAImoves(PBStuff::POISONMOVE,aimem)
					when PBAbilities::THERMALEXCHANGE,PBAbilities::WELLBAKEDBODY
						abilityscore+=20 if [PBTypes::FIRE].include?(checkAIbestMove().pbType(@opponent)) || [PBTypes::FIRE].include?(checkAIbestMove(@opponent.pbPartner).pbType(@opponent.pbPartner))
					when PBAbilities::WINDRIDER,PBAbilities::WINDPOWER
						abilityscore+=20 if checkAImoves([PBStuff::WINDMOVE],aimem)
					when PBAbilities::MAGICGUARD
						abilityscore+=20 if checkAImoves([PBMoves::LEECHSEED],aimem)
						abilityscore+=20 if checkAImoves([PBMoves::WILLOWISP],aimem)
						abilityscore+=20 if checkAImoves(PBStuff::POISONMOVE,aimem)
					when PBAbilities::WATERBUBBLE,PBAbilities::WATERVEIL,PBAbilities::FLAREBOOST,PBAbilities::THERMALEXCHANGE
					...
					when PBAbilities::INTIMIDATE,PBAbilities::FURCOAT,PBAbilities::STAMINA,PBAbilities::AUTOMATRON,PBAbilities::HEAVYMETAL
					...
					when PBAbilities::EFFECTSPORE,PBAbilities::STATIC,PBAbilities::POISONPOINT,PBAbilities::ROUGHSKIN,PBAbilities::IRONBARBS,PBAbilities::FLAMEBODY,PBAbilities::CUTECHARM,PBAbilities::MUMMY,PBAbilities::AFTERMATH,PBAbilities::GOOEY,PBAbilities::FLUFFY,PBAbilities::LINGERINGAROMA
					...
					when PBAbilities::TRACE 
						if [PBAbilities::WATERABSORB,PBAbilities::VOLTABSORB,PBAbilities::STORMDRAIN,PBAbilities::MOTORDRIVE,PBAbilities::FLASHFIRE,PBAbilities::LEVITATE,PBAbilities::LIGHTNINGROD,PBAbilities::WELLBAKEDBODY,PBAbilities::EARTHEATER,PBAbilities::LEADSKIN,PBAbilities::DISENCHANT,
							PBAbilities::SAPSIPPER,PBAbilities::DRYSKIN,PBAbilities::SLUSHRUSH,PBAbilities::SANDRUSH,PBAbilities::SWIFTSWIM,PBAbilities::CHLOROPHYLL,PBAbilities::NOVABURNER,PBAbilities::UBERSTRAT,PBAbilities::SPEEDBOOST,
							PBAbilities::WONDERGUARD,PBAbilities::PRANKSTER].include?(@opponent.ability) || 
							(pbAIfaster?() && ((@opponent.ability == PBAbilities::ADAPTABILITY) || (@opponent.ability == PBAbilities::DOWNLOAD) || (@opponent.ability == PBAbilities::PROTEAN))) || (@opponent.ability == PBAbilities::PETRIFY) ||
					...
					when PBAbilities::PURIFYINGSALT
						abilityscore+=30 if PBTypes::GHOST ==checkAIbestMove().pbType(@opponent) || PBTypes::GHOST == checkAIbestMove(@opponent.pbPartner).pbType(@opponent.pbPartner)
					when PBAbilities::LIQUIDOOZE
						for j in aimem
					...
					when PBAbilities::SCRAPPY,PBAbilities::MINDSEYE
					...
					when PBAbilities::FLAMESOFRUIN
						abilityscore+=30 if PBTypes::DARK == checkAIbestMove().pbType(@opponent) || PBTypes::DARK == checkAIbestMove(@opponent.pbPartner).pbType(@opponent.pbPartner)
					when PBAbilities::FLAMESOFRUIN
						abilityscore+=30 if PBTypes::GHOST == checkAIbestMove().pbType(@opponent) || PBTypes::GHOST == checkAIbestMove(@opponent.pbPartner).pbType(@opponent.pbPartner)
					when PBAbilities::FLAMESOFRUIN
						abilityscore+=30 if PBTypes::ICE == checkAIbestMove().pbType(@opponent) || PBTypes::ICE == checkAIbestMove(@opponent.pbPartner).pbType(@opponent.pbPartner)
					when PBAbilities::FLAMESOFRUIN
						abilityscore+=30 if PBTypes::DRAGON == checkAIbestMove().pbType(@opponent) || PBTypes::DRAGON == checkAIbestMove(@opponent.pbPartner).pbType(@opponent.pbPartner)
					when PBAbilities::FLAMESOFRUIN
						abilityscore+=30 if PBTypes::FAIRY == checkAIbestMove().pbType(@opponent) || PBTypes::FAIRY == checkAIbestMove(@opponent.pbPartner).pbType(@opponent.pbPartner)
					...
					when PBAbilities::IRONBARBS,PBAbilities::ROUGHSKIN
						...
						abilityscore+=30 if (@opponent.item == PBItems::LOADEDDICE) 
						abilityscore+=30 if (@opponent.pbPartner.hasWorkingItem(:LOADEDDICE))
					when PBAbilities::PRANKSTER
					...
					when PBAbilities::WINDRIDER
						abilityscore+=40 if (PBStuff::WINDMOVE).include?(checkAIbestMove().id) || (PBStuff::WINDMOVE).include?(checkAIbestMove(@opponent.pbPartner).id)
					when PBAbilities::AURABREAK
						abilityscore+=50 if (@opponent.ability == PBAbilities::FAIRYAURA) || (@opponent.ability == PBAbilities::DARKAURA) || (@opponent.ability == PBAbilities::CELESTIALAURA)
						abilityscore+=50 if (@opponent.pbPartner.ability == PBAbilities::FAIRYAURA) || (@opponent.pbPartner.ability == PBAbilities::DARKAURA) || (@opponent.pbPartner.ability == PBAbilities::CELESTIALAURA)
					when PBAbilities::PROTEAN
					...
					when PBAbilities::DAZZLING,PBAbilities::QUEENLYMAJESTY,PBAbilities::ARMORTAIL
                    ...
                    when PBAbilities::SUPREMEOVERLORD
						abilityscore+= 50 if @attacker.pbFaintedPokemonCount > 3
				end
			end
			if transformed  #pokemon has imposter ability. because we copy pokemon, we can use i to see ability opponent
				abilityscore+=50 if (i.ability == PBAbilities::PUREPOWER) || (i.ability == PBAbilities::HUGEPOWER) || (i.ability == PBAbilities::MOXIE) || (i.ability == PBAbilities::SPEEDBOOST) || (i.ability == PBAbilities::BEASTBOOST) || (i.ability == PBAbilities::SOULHEART) || (i.ability == PBAbilities::WONDERGUARD) || (i.ability == PBAbilities::PROTEAN) || (i.ability == PBAbilities::PUREFOCUS) || (i.ability == PBAbilities::LIBERO)
			...
			if @mondata.skill>=HIGHSKILL
				if (i.item == PBItems::ROCKYHELMET)
					...
					itemscore+=30 if (@opponent.item == PBItems::LOADEDDICE)
					itemscore+=30 if (@opponent.pbPartner.hasWorkingItem(:LOADEDDICE))
				...
				if (((i.item == PBItems::FOCUSSASH) || (@battle.FE == PBFields::CHESSB && i.pokemon.piece==:PAWN) || ((i.ability == PBAbilities::STURDY)))) && i.hp == i.totalhp
					if 	(((@battle.weather==PBWeather::SANDSTORM && !(i.hasType?(:ROCK) || i.hasType?(:GROUND) || i.hasType?(:STEEL)))  || (@battle.weather==PBWeather::NUCLEARFALLOUT && !(i.hasType?(:NUCLEAR) || i.hasType?(:STEEL)))  || (@battle.weather==PBWeather::HAIL && !(i.hasType?(:ICE)))) && !((i.ability == PBAbilities::OVERCOAT)))  || @attacker.pbOwnSide.effects[PBEffects::StealthRock] ||
			...
			if @mondata.skill>=BESTSKILL
			  case @battle.FE
				...
				when PBFields::BURNINGF
				  ...
				  fieldscore+=30 if (i.ability == PBAbilities::THERMALEXCHANGE)
				  fieldscore+=30 if (i.ability == PBAbilities::WELLBAKEDBODY)
				...
				when PBFields::FACTORYF
				  ...
				  fieldscore+=20 if (i.ability == PBAbilities::INNERFLAME)
				  fieldscore+=25 if (i.ability == PBAbilities::DOWNLOAD)
				  fieldscore+=25 if (i.ability == PBAbilities::TECHNICIAN)
				  fieldscore+=25 if (i.ability == PBAbilities::TROUBLESHOOTER)
				  fieldscore+=25 if (i.ability == PBAbilities::GALVANIZE)
				...
				when PBFields::ASHENB
				  ...
				  fieldscore+=15 if ((i.ability == PBAbilities::OWNTEMPO || i.ability == PBAbilities::FLOWERPOWER))
				...
				when PBFields::WATERS
				  ...
				  fieldscore+=50 if (i.ability == PBAbilities::SURGESURFER)
				  fieldscore+=80 if (i.species == PBSpecies::PALAFIN)
				...
				when PBFields::UNDERWATER
				  ...
				  fieldscore+=50 if (i.ability == PBAbilities::SWIFTSWIM)
				  fieldscore+=50 if (i.ability == PBAbilities::SURGESURFER)
				  fieldscore+=50 if (i.species == PBSpecies::PALAFIN)
				  mod1=PBTypes.getEffectiveness(PBTypes::WATER,i.type1)
				  mod2=(i.type1==i.type2) ? 2 : PBTypes.getEffectiveness(PBTypes::WATER,i.type2)
				  fieldscore-=50 if mod1*mod2>4
				when PBFields::CAVE
				...
				when PBFields::HOLYF
				  fieldscore+=20 if i.hasType?(:NORMAL)
				  fieldscore+=20 if (i.ability == PBAbilities::FLAMESOFRUIN)
				when PBFields::MIRRORA
				  fieldscore+=25 if (i.ability == PBAbilities::SANDVEIL)
				  fieldscore+=25 if (i.ability == PBAbilities::SNOWCLOAK)
		...
		if survivors.none? {|lives| lives == true}
			# aftermath
			# intimidate
	...
	def shouldSwitch?
	    ...
	    return  1000 if @attacker.species == PBSpecies::PALAFIN && @attacker.form!=1 && (@battle.FE != PBFields::WATERS || @battle.FE != PBFields::WATERS) && !pbAIfaster?
		count = 0
		...
		for i in @attacker.moves
			...
			notnorm = true if i.type != ((PBTypes::NORMAL) && (PBTypes::NPG))
		...
		for oppmon in [@opponent,@opponent.pbPartner]
			...
			#punishing loaded-dice multi-hit contact moves
			if oppmon.hasWorkingItem(:LOADEDDICE) 
				if getAIMemory(oppmon).any? {|moveloop| moveloop!=nil && moveloop.function==0xC0 && moveloop.isContactMove?}
					for i in @mondata.party
						next if i.nil? || i.hp == 0 || @mondata.party.index(i) == @attacker.pokemonIndex
						if (i.item == PBItems::ROCKYHELMET) || (i.ability == PBAbilities::ROUGHSKIN) || (i.ability == PBAbilities::IRONBARBS)
			...
			#Flames of Ruin switch vs dark attack moves
			bestmove=checkAIbestMove()
			if bestmove.pbType(@opponent) == PBTypes::DARK && @attacker.ability != PBAbilities::FLAMESOFRUIN
				for i in @mondata.party
					next if i.nil? || i.hp == 0 || @mondata.party.index(i) == @attacker.pokemonIndex
					if i.ability==PBAbilities::FLAMESOFRUIN
						specialscore+=70
						break
					end
				end
			end
			#Flames of Ruin switch vs ghost attack moves
			bestmove=checkAIbestMove()
			if bestmove.pbType(@opponent) == PBTypes::GHOST && @attacker.ability != PBAbilities::FLAMESOFRUIN
				for i in @mondata.party
					next if i.nil? || i.hp == 0 || @mondata.party.index(i) == @attacker.pokemonIndex
					if i.ability==PBAbilities::FLAMESOFRUIN
						specialscore+=70
			...
			#Flames of Ruin switch vs dragon attack moves
			bestmove=checkAIbestMove()
			if bestmove.pbType(@opponent) == PBTypes::DRAGON && @attacker.ability != PBAbilities::FLAMESOFRUIN
				for i in @mondata.party
					next if i.nil? || i.hp == 0 || @mondata.party.index(i) == @attacker.pokemonIndex
					if i.ability==PBAbilities::FLAMESOFRUIN
						specialscore+=70
			...
			#Flames of Ruin switch vs ice attack moves
			bestmove=checkAIbestMove()
			if bestmove.pbType(@opponent) == PBTypes::ICE && @attacker.ability != PBAbilities::FLAMESOFRUIN
				for i in @mondata.party
					next if i.nil? || i.hp == 0 || @mondata.party.index(i) == @attacker.pokemonIndex
					if i.ability==PBAbilities::FLAMESOFRUIN
						specialscore+=70
			...
			#Flames of Ruin switch vs fairy attack moves
			bestmove=checkAIbestMove()
			if bestmove.pbType(@opponent) == PBTypes::FAIRY && @attacker.ability != PBAbilities::FLAMESOFRUIN
				for i in @mondata.party
					next if i.nil? || i.hp == 0 || @mondata.party.index(i) == @attacker.pokemonIndex
					if i.ability==PBAbilities::FLAMESOFRUIN
						specialscore+=70
		...
		if pbAIfaster?(nil,nil,@attacker,@opponent) && pbAIfaster?(nil,nil,@attacker,@opponent.pbPartner)
			betterswitchscore+=90 if @attacker.pbHasMove?(PBMoves::VOLTSWITCH) || @attacker.pbHasMove?(PBMoves::UTURN) || @attacker.pbHasMove?(PBMoves::FLIPTURN)
		end
	...
	def pbStatChangingSwitch(mon)
		...
		#Contrary
		if mon.ability==PBAbilities::CONTRARY || mon.ability == PBAbilities::ALOLANTECHNIQUESB
	...
	def shouldHardSwitch?(attacker,switch_in_index)
		...
		#return true if attacker.species == PBSpecies::PALAFIN && attacker.form !=1 && (@battle.FE != PBFields::WATERS || @battle.FE != PBFields::WATERS) && !pbAIfaster?
...
######################################################
	def pbRoughDamage(move=@move,attacker=@attacker,opponent=@opponent)
		...
		basedamage*=1.33 if ((attacker.effects[PBEffects::HydraBond] && move.pbNumHits(attacker)>=1)) 
		...
		elsif move.function==0x121 # Body Press
		...
		if @mondata.skill>=MEDIUMSKILL
		  ############ ATTACKER ABILITY CHECKS ############
			...
			#Troubleshooter
				...
				basedamage=(basedamage*1.5).round if (basedamage>60) || (@battle.FE == PBFields::FACTORYF && basedamage>40)
			# Iron Fist
			elsif (attacker.ability == PBAbilities::IRONFIST || attacker.item == PBItems::PUNCHINGGLOVE)
			basedamage=(basedamage*1.4).round if move.isPunchingMove?
			# Enchanted Cannon
			elsif attacker.ability == PBAbilities::ENCHANTEDCANNON
				basedamage=(basedamage*1.5).round if (PBStuff::BEAMMOVE).include?(move.id)
			# Sharpness
			elsif attacker.ability == PBAbilities::SHARPNESS
				basedamage=(basedamage*1.5).round if (PBStuff::SLASHMOVE).include?(move.id) 
			# Injection
			elsif attacker.ability == PBAbilities::INJECTION
				basedamage=(basedamage*1.5).round if (PBStuff::STINGMOVE).include?(move.id) 
			# ACCELERATION
			elsif attacker.ability == PBAbilities::ACCELERATION 
				basedamage=(basedamage*1.5).round if [26,108,180,252,278,306,310,314,526,550,836,955,963,989].include?(move.id)
			# Strong Jaw
			...
			# Mega Launcher
				...
				if move.id == PBMoves::AURASPHERE || move.id == PBMoves::DRAGONPULSE || move.id == PBMoves::DARKPULSE || move.id == PBMoves::WATERPULSE || move.id == PBMoves::ORIGINPULSE || move.id == PBMoves::TERRAINPULSE
					basedamage=(basedamage*1.5).round
				end
			# Sand Force
			...
			#Defeatist
				...
				atk=(atk*0.5).round if attacker.hp<=(attacker.totalhp/4.0).floor
			#Pure/Huge Power
			...
			#Pure Focus
			elsif attacker.ability == PBAbilities::PUREFOCUS && move.pbIsSpecial?(type)
				atk=(atk*2.0).round
			#Solar Power
			...
			if attacker.turncount<4 && move.pbIsPhysical?(type)
				atk=(atk*0.75).round
			end
			#Fast Start
			elsif attacker.ability == PBAbilities::QUICKSTART
				if attacker.turncount<2 && move.pbIsPhysical?(type)
					atk=(atk*2).round
				elsif attacker.turncount>2 && move.pbIsPhysical?(type)
					atk=(atk*0.5).round
				end
			#Fast Start
			elsif attacker.ability == PBAbilities::QUICKSTART
				if attacker.turncount<2 && move.pbIsSpecial?(type)
					spatk=(spatk*2).round
				elsif attacker.turncount>2 && move.pbIsSpecial?(type)
					spatk=(spatk*0.5).round
				end
		elsif attacker.ability == PBAbilities::HADRONENGINE
			if (@battle.FE == PBFields::ELECTRICT || @battle.FE == PBFields::FACTORYF) && move.pbIsSpecial?(type)
				atk=(atk*1.3).round
			end
		elsif attacker.ability == PBAbilities::ORICHALCUMPULSE
			if @battle.weather==PBWeather::SUNNYDAY && move.pbIsPhysical?(type)
				atk=(atk*1.3).round
			end
			# Type Changing Abilities
				...
				# Flower Surge
				elsif attacker.ability == PBAbilities::FLOWERPOWER
					if @mondata.skill>=BESTSKILL
						if @battle.FE == PBFields::FLOWERGARDENF || @battle.FE == PBFields::FORESTF || @battle.FE == PBFields::GRASSYT # Flower Garden or Forest Field or Grassy Terrain
							basedamage=(basedamage*1.5).round
						else
							basedamage=(basedamage*1.2).round
						end
				...
				# Furious Spirit
				elsif attacker.ability == PBAbilities::FURIOUSSPIRIT
					if @mondata.skill>=BESTSKILL
						if @battle.FE == PBFields::BIGTOPA || @battle.FE == PBFields::ASHENB # Big Top Arena or Ashen Beach
							basedamage=(basedamage*1.5).round
						else
							basedamage=(basedamage*1.2).round
						end
				...
				# Raging Blaze
				elsif attacker.ability == PBAbilities::RAGINGBLAZE
					if @mondata.skill>=BESTSKILL
						if @battle.FE == PBFields::BIGTOPA || @battle.FE == PBFields::ASHENB # Big Top Arena or Ashen Beach
							basedamage=(basedamage*1.5).round
						else
				...
				# Dragon Sword
					...
					if @mondata.skill>=BESTSKILL
						if @battle.FE == PBFields::HOLYF || @battle.FE == PBFields::MOUNTAIN || @battle.FE == PBFields::FAIRYTALEF || @battle.FE == PBFields::DRAGONSD # Holy Field , Mountain , Fairy Tale Field or Dragon's Den
							basedamage=(basedamage*1.2).round
						else
		  ...
		  ############ OPPONENT ABILITY CHECKS ############
			...
			if attitemworks #don't bother with this if it doesn't work
				#Type-boosting items
				case type
					when (PBTypes::NORMAL || PBTypes::NPG)
						case attacker.item
							when PBItems::SILKSCARF then basedamage=(basedamage*1.2).round
							when PBItems::NORMALGEM then basedamage=(basedamage*1.5).round
						end
					when (PBTypes::FIGHTING || PBTypes::FG || PBTypes::GFS || PBTypes::PFD)
						case attacker.item
							when PBItems::BLACKBELT,PBItems::FISTPLATE then basedamage=(basedamage*1.2).round
							when PBItems::FIGHTINGGEM then basedamage=(basedamage*1.5).round
						end
					when PBTypes::FLYING
						case attacker.item
							when PBItems::SHARPBEAK,PBItems::SKYPLATE then basedamage=(basedamage*1.2).round
							when PBItems::FLYINGGEM then basedamage=(basedamage*1.5).round
						end
					when PBTypes::POISON
						case attacker.item
					...
					when PBTypes::ELECTRIC
						case attacker.item
							...
							when PBItems::ELECTRICGEM then basedamage=(basedamage*1.5).round
						end
					when PBTypes::PSYCHIC
						case attacker.item
							when PBItems::TWISTEDSPOON,PBItems::MINDPLATE,PBItems::ODDINCENSE then basedamage=(basedamage*1.2).round
							when PBItems::PSYCHICGEM then basedamage=(basedamage*1.5).round
						end
					when PBTypes::ICE
						case attacker.item
							when PBItems::NEVERMELTICE,PBItems::ICICLEPLATE then basedamage=(basedamage*1.2).round
							when PBItems::ICEGEM then basedamage=(basedamage*1.5).round
						end
					when PBTypes::DRAGON
						case attacker.item
							when PBItems::DRAGONFANG,PBItems::DRACOPLATE then basedamage=(basedamage*1.2).round
							when PBItems::DRAGONGEM then basedamage=(basedamage*1.5).round
						end
					when PBTypes::DARK
						case attacker.item
							when PBItems::BLACKGLASSES,PBItems::DREADPLATE then basedamage=(basedamage*1.2).round
							when PBItems::DARKGEM then basedamage=(basedamage*1.5).round
						end
					when PBTypes::FAIRY
						case attacker.item
							when PBItems::PIXIEPLATE then basedamage=(basedamage*1.2).round
							when PBItems::FAIRYGEM then basedamage=(basedamage*1.5).round
						end
					when PBTypes::NUCLEAR
						case attacker.item
							when PBItems::ATOMICPLATE then basedamage=(basedamage*1.2).round
							when PBItems::NUCLEARGEM then basedamage=(basedamage*1.5).round
						end
					when PBTypes::COSMIC
						case attacker.item
							when PBItems::COSMICPLATE then basedamage=(basedamage*1.2).round
							when PBItems::COSMICGEM then basedamage=(basedamage*1.5).round
						end
				end
				# Muscle Band
			...
			# Dark Aura or Celestial Aura/Aurabreak
			elsif type == PBTypes::DARK
				if @battle.battlers.any? {|battler| battler.ability == (PBAbilities::DARKAURA || PBAbilities::CELESTIALAURA)}
			...
			# Celestial Aura/Aurabreak
			elsif type == PBTypes::FLYING
				if @battle.battlers.any? {|battler| battler.ability == PBAbilities::CELESTIALAURA}
					basedamage*= @battle.battlers.any? {|battler| battler.ability == PBAbilities::AURABREAK} ? (2.0/3) : (4.0/3)
				end
			end
			#Battery
		...
		if @mondata.skill>=BESTSKILL
			...
			atk=(atk*0.5).round if @battle.FE == PBFields::UNDERWATER && move.pbIsPhysical?(type) && type != PBTypes::WATER && attacker.ability!=PBAbilities::STEELWORKER
		end
		...
		if @mondata.skill>=MEDIUMSKILL
            ...
            elsif opponent.ability == PBAbilities::MAGMAARMOR && !moldBreakerCheck(attacker)
				defense=(defense*1.5).round if type == PBTypes::WATER	
			elsif opponent.ability == PBAbilities::HEAVYMETAL 
				defense=(defense*1.5).round if move.pbIsPhysical?(type) && !moldBreakerCheck(attacker)
			elsif opponent.ability == PBAbilities::PURIFYINGSALT && !moldBreakerCheck(attacker)
				defense=(defense*2.0).round if type == PBTypes::GHOST
		...
		if @mondata.skill>=MEDIUMSKILL
			case @battle.pbWeather
				when PBWeather::SUNNYDAY
					...
					if (type == PBTypes::FIRE || move.id == 1031)
						damage=(damage*1.5).round
					elsif (type == PBTypes::WATER && !(move.id == 1031))
				...
				when PBWeather::RAINDANCE
					...
					if @battle.state.effects[PBEffects::Storm] && type == PBTypes::ELECTRIC # Uranium
						damage=(damage*1.5).round
					end
					if type == PBTypes::FIRE
						damage=(damage*0.5).round
		...
		if @mondata.skill>=MEDIUMSKILL
			...
			elsif attacker.ability == PBAbilities::INNERFLAME && type == PBTypes::FIRE
				if @battle.FE == PBFields::SUPERHEATEDF || @battle.FE == PBFields::DRAGONSD # Superheated Field || Dragon's Den
					damage=(damage*2).round
				else
					damage=(damage*1.5).round
				end
		...
		if @mondata.skill>=HIGHSKILL
			if move.pbIsPhysical?(type)
				...
				if attacker.pbOwnSide.effects[PBEffects::SolarArray]>0
		...
		if @mondata.skill>=MEDIUMSKILL
			...
			# Plot Armor
			damage=(damage*2.0).round if attacker.ability == PBAbilities::PLOTARMOR && typemod<4 #PLOTARMOR ADDED
			# Neuroforce
	...
	def pbBetterBaseDamage(move=@move,attacker=@attacker,opponent=@opponent)
		...
		case move.function
			...
			when 0x6B # Dragon Rage
				return 40
			when 0x6C # Super Fang, Ruination
				...
				elsif (move.id == PBMoves::RUINATION) && @battle.FE == PBFields::NEWW
					return (opponent.hp*0.66).floor
				end
			...
			when 0x6E # Endeavor
				...
				return 20
			when 0x181 # Fishious Rend
				return move.basedamage*2 if pbAIfaster?()
			when 0x76 # Earthquake
			...
			when 0x20 # Rising Voltage
				return move.basedamage*2 if @battle.FE == PBFields::ELECTRICT
			when 0x201 # Steel Roller
				if @battle.FE == PBFields::ELECTRICT || @battle.FE == PBFields::GRASSYT || @battle.FE == PBFields::MISTYT || @battle.FE == PBFields::PSYCHICT
					return move.basedamage
				else
					return 0
				end
			when 0x266 # Last Respects
				party=@battle.pbPartySingleOwner(attacker.index).find_all {|mon| mon && !mon.isEgg? && mon.hp<=0}
				return basedamage + (50*party.length)
            ...
            when 0x251 # Population Bomb
				return move.basedamage*10	
	...
	def mirrorNeverMiss
		return (@attacker.stages[PBStats::ACCURACY] < 0 || @opponent.stages[PBStats::EVASION] > 0 || @opponent.item == PBItems::BRIGHTPOWDER || 
			@opponent.item == PBItems::LAXINCENSE || accuracyWeatherAbilityActive?(@opponent) || @opponent.vanished) &&
			 @opponent.ability != PBAbilities::NOGUARD && @attacker.ability != PBAbilities::NOGUARD && !(@attacker.ability == PBAbilities::FAIRYAURA && @battle.FE == PBFields::FAIRYTALEF) && @opponent.ability != PBAbilities::ALOLANTECHNIQUESA && @attacker.ability != PBAbilities::ALOLANTECHNIQUESA
	end
	...
	def moveSuccesful?(move,attacker,opponent)
		if move.pbIsPriorityMoveAI(attacker)
			...
			return false if opponent.ability == PBAbilities::DAZZLING || opponent.ability == PBAbilities::QUEENLYMAJESTY || opponent.ability == PBAbilities::ARMORTAIL
			return false if opponent.pbPartner.ability == PBAbilities::DAZZLING || opponent.pbPartner.ability == PBAbilities::QUEENLYMAJESTY || opponent.pbPartner.ability == PBAbilities::ARMORTAIL
			return false if opponent.ability == PBAbilities::PRANKSTER && move.pbIsStatus?
            return false if @battle.FE == PBFields::GRASSYT && move.id == PBMoves::GRASSYGLIDE
		end
		return true
	end
...
#####################################################
	...
	def moldBreakerCheck(battler)
		return battler.ability==PBAbilities::MOLDBREAKER || battler.ability==PBAbilities::TERAVOLT || battler.ability==PBAbilities::TURBOBLAZE || (battler.ability==PBAbilities::MYCELIUMMIGHT && move.basedamage==0)
	end
	...
	def notOHKO?(attacker,opponent, immediate = false)
		...
		return false if @battle.pbWeather == PBWeather::NUCLEARFALLOUT && !(attacker.pbHasType?(:NUCLEAR) || attacker.pbHasType?(:STEEL)) && !immediate
		return false if attacker.hp != attacker.totalhp
		return false if attacker.ability == PBAbilities::PARENTALBOND || attacker.ability == PBAbilities::SKILLLINK || attacker.ability == PBAbilities::HYDRABOND
	...
	def canGroundMoveHit?(battler)
		...
		return false if battler.pbHasType?(:FLYING) && battler.effects[PBEffects::Roost]==false && !@battle.typesInverted?
	...
	def secondaryEffectNegated?(move = @move, attacker = @attacker, opponent = @opponent)
		return move.basedamage > 0 && (opponent.ability == PBAbilities::SHIELDDUST || attacker.ability == PBAbilities::SHEERFORCE || opponent.hasWorkingItem(:COVERTCLOAK))
	end

	def seedProtection?(battler = @attacker)
		return battler.effects[PBEffects::KingsShield] || battler.effects[PBEffects::BanefulBunker] || battler.effects[PBEffects::SpikyShield] || battler.effects[PBEffects::SilkTrap]
	end
