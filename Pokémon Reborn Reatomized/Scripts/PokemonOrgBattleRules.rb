def pbBaseStatTotal(species)
  bst = 0
  for i in $cache.pkmn_dex[species][:BaseStats]
    bst += i
  end
  return bst
end

def pbBalancedLevel(species)
  return (113-(pbBaseStatTotal(species)*0.072)).round
end

def pbTooTall?(pokemon,maxHeightInMeters)
  return $cache.pkmn_dex[pokemon.species][:Height]>maxHeightInMeters
end

def pbTooHeavy?(pokemon,maxWeightInKg)
  return $cache.pkmn_dex[pokemon.species][:Weight]>maxWeightInKg
end
...
class LevelAdjustment
  ...
  def getOldLevels(team1,team2)
    ret=[]
    for i in 0...team1.length
      ret.push(team1[i].level)
    end
    return ret
  end
  ...
  def unadjustLevels(team1,team2,adjustments)
    for i in 0...team1.length
      level=adjustments[0][i]
      if level && team1[i].level!=level
        team1[i].level=level
        team1[i].calcStats
      end
    end
  ...
  def adjustLevels(team1,team2)
    adj1=nil
    adj2=nil
    ret=[getOldLevels(team1,team2),getOldLevels(team2,team1)]
...
class LevelBalanceAdjustment < LevelAdjustment
  ...
  def getAdjustment(thisTeam,otherTeam)
    ret=[]
    for i in 0...thisTeam.length
      ret[i]=pbBalancedLevel(thisTeam[i].species)
    end
    return ret
  end
end
...
class StandardRestriction
  def isValid?(pokemon)
    ...
    basestatsum = 0
    for i in $cache.pkmn_dex[pokemon.species][:BaseStats]
      basestatsum += i
    end
    # Surge abilities are banned
    if (pokemon.ability == PBAbilities::GRASSYSURGE) ||
       (pokemon.ability == PBAbilities::MISTYSURGE) ||
       (pokemon.ability == PBAbilities::PSYCHICSURGE) ||
       (pokemon.ability == PBAbilities::ELECTRICSURGE) ||
       (pokemon.ability == PBAbilities::GLITCHYSURGE) ||
       (pokemon.ability == PBAbilities::TRICKSTER)
      return false
    end
    # Ultra Beasts are banned
    if (pokemon.ability == PBAbilities::BEASTBOOST) 
      return false
    end
    # Zygarde Complete is banned
    if (pokemon.ability == PBAbilities::POWERCONSTRUCT)
      return false
    end
    ...
    stonelist = pbGetMegaStoneList
    if stonelist.include?(pokemon.item)
      return false
    end
...
class SoulDewClause
  def isValid?(pokemon)
    return !(pokemon.item == PBItems::SOULDEW)
  end
end
...
class NegativeExtendedGameClause
  def isValid?(pokemon)
    return false if (pokemon.species == PBSpecies::ARCEUS)
    return false if (pokemon.item == PBItems::MICLEBERRY)
    return false if (pokemon.item == PBItems::CUSTAPBERRY)
    return false if (pokemon.item == PBItems::JABOCABERRY)
    return false if (pokemon.item == PBItems::ROWAPBERRY)
  end
end
...
class LittleCupRestriction
  def isValid?(pokemon)
    return false if (pokemon.item == PBItems::BERRYJUICE)
    return false if (pokemon.item == PBItems::DEEPSEATOOTH)
    return false if pokemon.knowsMove?(:SONICBOOM)
    return false if pokemon.knowsMove?(:DRAGONRAGE)
    return false if (pokemon.species == PBSpecies::SCYTHER)
    return false if (pokemon.species == PBSpecies::SNEASEL)
    return false if (pokemon.species == PBSpecies::MEDITITE)
    return false if (pokemon.species == PBSpecies::YANMA)
    return false if (pokemon.species == PBSpecies::TANGELA)
    return false if (pokemon.species == PBSpecies::MURKROW)
    return true
  end
end
...
#  ret.addPokemonRule(BannedSpeciesRestriction.new(:UNOWN))
  ...
  ret.addBattleRule(SoulDewBattleClause.new)
  if double
    ret.setDoubleBattle(double).setNumber(4)
  else
    ret.setDoubleBattle(double).setNumber(3)
  end
  return ret
