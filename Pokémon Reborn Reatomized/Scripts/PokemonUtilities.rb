...
def pbLoadPokemonBitmapSpecies(pokemon, species, back=false)
  ...
  y = pokemon.form*384
  y = 8*384 if (species == PBSpecies::SILVALLY && pokemon.form == 21) #Vulpes abusing her dev privileges
...
def pbNickname(pokemon)
  ...
  if pokemon.isShiny?
  return "" if !Kernel.pbConfirmMessage(_INTL("Would you like to give a nickname to \\c[6]{1}?",speciesname))
  else
...
def pbAddPokemon(pokemon,level=nil,seeform=true)
  ...
  shinyretries=0
  shinyretries+=2 if hasConst?(PBItems,:SHINYCHARM) && $PokemonBag.pbQuantity(:SHINYCHARM)>0 # 3 times as likely
  shinyretries+=1 if hasConst?(PBItems,:SHINYCHARMP) && $PokemonBag.pbQuantity(:SHINYCHARMP)>0 # 2 times as likely
  shinyretries+=2 if hasConst?(PBItems,:SHINYCHARMPP) && $PokemonBag.pbQuantity(:SHINYCHARMPP)>0 # 3 times as likely
  shinyretries+=50 if hasConst?(PBItems,:DEVCHARM) && $PokemonBag.pbQuantity(:DEVCHARM)>0 # 6 times as likely
  ...
  if pokemon.isShiny?
    Kernel.pbMessage(_INTL("The Pokèmon seems to \\c[6]sparkle!"))
    Kernel.pbMessage(_INTL("{1} obtained \\c[6]{2}!\\se[itemlevel]\1",$Trainer.name,speciesname))
  else
