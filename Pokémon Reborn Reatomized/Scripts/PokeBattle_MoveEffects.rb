...
if attacker.pbPartner.isFainted? && @id == PBMoves::HOLDHANDS
...
# / Sludge / Poison Tail / Smog / Poison Sting / Poison Gas / Poison Powder / Noxious Torque)
################################################################################
class PokeBattle_Move_005 < PokeBattle_Move
  ...
  def pbAdditionalEffect(attacker,opponent)
    if @battle.FE == PBFields::WASTELAND && ((@id == PBMoves::GUNKSHOT) || (@id == PBMoves::SLUDGEBOMB) || 
     ...
     !(opponent.ability == PBAbilities::MULTICORE) &&
...
# / Lick / Stun Spore / Body Slam / Glare / Splishy Splash / Buzzy Buzz
# / Wildbolt Storm)
################################################################################
...
# / Will-O-Wisp / Scald / Steam Eruption / Sizzly Slide / Scorching Sands
# / Sandsear Storm / Blazing Torque)
################################################################################
...
# / Heart Stamp / Rock Slide / Iron Head / Waterfall / Zing Zap / Floaty Fall
# / Fiery Wrath / Mountain Gale)
################################################################################
...
################################################################################
# Fails if the target didn't choose a damaging move with priority this round, 
# or has already moved. Flinches if successfull (Upper Hand)
################################################################################
class PokeBattle_Move_289 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    if opponent.effects[PBEffects::SilkTrap]==true
      @battle.pbDisplay(_INTL("But it failed!"))
      return -1
    end
    ...
    if opponent.effects[PBEffects::Bulwark]==true 
      @battle.pbDisplay(_INTL("But it failed!"))
      return -1
    end
    ...
    if ((@battle.choices[opponent.index][2] == nil) || (@battle.choices[opponent.index][2] == 0) || (@battle.choices[opponent.index][2] == -1) || (@battle.choices[opponent.index][2].basedamage == 0) || (@battle.choices[opponent.index][2].priority == 0))
      @battle.pbDisplay(_INTL("But it failed!"))
      return -1
    end
    return super
...
################################################################################
# Cures all party Pokémon of permanent status problems. (Aromatherapy, Heal Bell, Sparkly Swirl)
################################################################################
class PokeBattle_Move_019 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    if @id == PBMoves::AROMATHERAPY || @id == PBMoves::SPARKLYSWIRL
    ...
    for i in @battle.battlers
      ...
      if @id == PBMoves::HEALBELL && i.ability == PBAbilities::SOUNDPROOF && !(i.moldbroken)
        @battle.pbDisplay(_INTL("{1}'s {2} blocks {3}!",i.pbThis,
           PBAbilities.getName(i.ability),@name))
        next
      end
      i.pbCureStatusComplete #New method to reduce redundancy
    ...
    for i in 0...party.length
      ...
      next if @id == PBMoves::HEALBELL && party[i].ability == PBAbilities::SOUNDPROOF
      case party[i].status
...
class PokeBattle_Move_01C < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    elsif (@id == PBMoves::HOWL) && @battle.doublebattle 
      ret=attacker.pbIncreaseStat(PBStats::ATTACK,1,abilitymessage:false)
      ret=attacker.pbPartner.pbIncreaseStat(PBStats::ATTACK,1,abilitymessage:false)
    else
      ret=attacker.pbIncreaseStat(PBStats::ATTACK,1,abilitymessage:false)
    end
    return ret ? 0 : -1
...
################################################################################
# Increases the user's Speed by 1 stage. (Flame Charge, Esper Wing, Aqua Step, Trailblaze)
################################################################################
...
################################################################################
# Increases the user's Special Attack by 1 stage. (Charge Beam, Fiery Dance, Mystical Power, Torch Song)
################################################################################
...
class PokeBattle_Move_028 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    pbShowAnimation(@id,attacker,nil,hitnum,alltargets,showanimation)
    increment = (@battle.pbWeather==PBWeather::SUNNYDAY && !attacker.hasWorkingItem(:UTILITYUMBRELLA)) || attacker.ability == PBAbilities::HEATGENERATOR ? 2 : 1
...
class PokeBattle_Move_03A < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    if attacker.hasWorkingAbility(:CONTRARY) || attacker.hasWorkingAbility(:ALOLANTECHNIQUESB)
...
################################################################################
# Decreases the user's Defense and Special Defense by 1 stage each. (Close Combat, Dragon Ascent, Headlong Rush, Armor Cannon)
################################################################################
...
################################################################################
# Decreases the target's Attack by 1 stage. (Growl, Aurora Beam, Baby-Doll Eyes, Play Nice, Play Rough, Lunge, Trop Kick, Breaking Swipe, Bitter Malice, Springtide Storm, Chilling Water, Wicked Torque)
################################################################################
...
################################################################################
# Decreases the target's Defense by 1 stage. (Tail Whip, Crunch, Rock Smash, Crush Claw, Leer, Iron Tail, Razor Shell, Fire Lash, Liquidation, Shadow Bone, Thunderous Kick, Combat Torque)
################################################################################
...
################################################################################
# Decreases the target's Speed by 1 stage. (Rock Tomb, Electroweb, Low Sweep, Bulldoze, Mud Shot, Glaciate, Icy Wind, Constrict, Bubble Beam, Bubble, Bleakwind Storm, Pounce)
################################################################################
...
# Mystical Fire / Struggle Bug / Mist Ball / Skitter Smack)
################################################################################
...
class PokeBattle_Move_047 < PokeBattle_Move
  ...
  def pbAdditionalEffect(attacker,opponent)
    if @battle.FE == PBFields::WASTELAND && (@id == PBMoves::OCTAZOOKA) && 
      ((!opponent.pbHasType?(:POISON) && !opponent.pbHasType?(:STEEL)) || opponent.corroded) &&
      opponent.ability != PBAbilities::TOXICBOOST && opponent.ability != PBAbilities::MULTICORE && 
      opponent.ability != PBAbilities::POISONHEAL && (opponent.ability != PBAbilities::IMMUNITY && !(opponent.moldbroken))
...
class PokeBattle_Move_049 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    if attacker.pbOpposingSide.effects[PBEffects::SolarArray]>0
      attacker.pbOpposingSide.effects[PBEffects::SolarArray]=0
      if !@battle.pbIsOpposing?(attacker.index)
        @battle.pbDisplay(_INTL("The opposing team's Solar Array wore off!"))
      else
        @battle.pbDisplay(_INTL("Your team's Solar Array wore off!"))
      end
    end
    ...
    if attacker.pbOwnSide.effects[PBEffects::Decrystals] || opponent.pbOwnSide.effects[PBEffects::Decrystals]
      attacker.pbOwnSide.effects[PBEffects::Decrystals]=false
      opponent.pbOwnSide.effects[PBEffects::Decrystals]=false
      if !@battle.pbIsOpposing?(attacker.index)
        @battle.pbDisplay(_INTL("The crystallized stones disappeared from around your opponent's team!"))
      else
        @battle.pbDisplay(_INTL("The crystallized stones disappeared from around your team!"))
      end
    end
  ...
  def pbAdditionalEffect(attacker,opponent)
    ...
    opponent.pbOwnSide.effects[PBEffects::SolarArray] = 0
    opponent.pbOwnSide.effects[PBEffects::Mist] = 0
    opponent.pbOwnSide.effects[PBEffects::Safeguard] = 0
    opponent.pbOwnSide.effects[PBEffects::Spikes] = 0
    opponent.pbOwnSide.effects[PBEffects::StealthRock] = false
...
class PokeBattle_Move_04A < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    opponent.ability == PBAbilities::WHITESMOKE || opponent.ability == PBAbilities::FULLMETALBODY) && !(opponent.moldbroken)
      @battle.pbDisplay(_INTL("{1}'s {2} prevents stat loss!",opponent.pbThis,
        PBAbilities.getName(opponent.ability)))
      return -1
    end
    if opponent.hasWorkingItem(:CLEARAMULET)
      @battle.pbDisplay(_INTL("{1}'s {2} prevents stat loss!",opponent.pbThis,
...
# Acid Spray / Metal Sound / Lumina Crash)
################################################################################
...
################################################################################
# Resets all stat stages for all battlers to 0. (Haze / Freezy Frost)
################################################################################
...
class PokeBattle_Move_054 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    if @battle.FE==PBFields::NEWW && opponent.effects[PBEffects::Substitute]==0
      pbShowAnimation(@id,attacker,opponent,hitnum,alltargets,showanimation)
      olda=attacker.hp
      oldo=opponent.hp
...
class PokeBattle_Move_063 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    if opponent.effects[PBEffects::Substitute]>0 || (PBStuff::FIXEDABILITIES).include?(opponent.ability)
      @battle.pbDisplay(_INTL("But it failed!"))
      return -1
    elsif opponent.hasWorkingItem(:ABILITYSHIELD)
      @battle.pbDisplay(_INTL("{1}'s Ability is protected by its {2}!", opponent.pbThis, PBItems.getName(opponent.item)))
      return -1
    end
    pbShowAnimation(@id,attacker,opponent,hitnum,alltargets,showanimation)
    ...
    opponent.pbBreakIllusionOnAbilityChange #New method to reduce redundancy
    return 0
  end
end
...
class PokeBattle_Move_065 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    if opponent.ability==0 || attacker.ability==opponent.ability ||
      ...
      @battle.pbDisplay(_INTL("{1}'s Ability is protected by its {2}!", attacker.pbThis, PBItems.getName(attacker.item)))
      return -1
    end
    pbShowAnimation(@id,attacker,opponent,hitnum,alltargets,showanimation)
...
class PokeBattle_Move_066 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    if opponent.effects[PBEffects::Substitute]>0
      @battle.pbDisplay(_INTL("But it failed!"))
      return -1
    elsif attacker.ability==0 || attacker.ability==opponent.ability ||
...
class PokeBattle_Move_067 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    if (attacker.ability==0 && opponent.ability==0) ||
       (PBStuff::FIXEDABILITIES - [PBAbilities::ZENMODE] + [PBAbilities::ILLUSION] + [PBAbilities::WONDERGUARD] + [PBAbilities::PLOTARMOR]).include?(attacker.ability) ||
       (PBStuff::FIXEDABILITIES - [PBAbilities::ZENMODE] + [PBAbilities::ILLUSION] + [PBAbilities::WONDERGUARD] + [PBAbilities::PLOTARMOR]).include?(opponent.ability)
      @battle.pbDisplay(_INTL("But it failed!"))
      return -1
    elsif opponent.hasWorkingItem(:ABILITYSHIELD)
      @battle.pbDisplay(_INTL("{1}'s Ability is protected by its {2}!", opponent.pbThis, PBItems.getName(opponent.item)))
...
class PokeBattle_Move_069 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    if attacker.effects[PBEffects::Transform] ||
       ...
       opponent.effects[PBEffects::Commander]
      @battle.pbDisplay(_INTL("But it failed!"))
      return -1
    end
    pbShowAnimation(@id,attacker,opponent,hitnum,alltargets,showanimation)
...
################################################################################
# Halves the target's current HP. (Super Fang / Nature Madness / Ruination)
################################################################################
...
#    return false if opponent.pokemon.piece==:PAWN && @battle.FE == PBFields::CHESSB
    ...
    return true if opponent.level <= attacker.level && (attacker.ability == PBAbilities::NOGUARD || opponent.ability == PBAbilities::NOGUARD || attacker.ability == PBAbilities::ALOLANTECHNIQUESA || opponent.ability == PBAbilities::ALOLANTECHNIQUESA) # no guard OHKO move situation.
...
class PokeBattle_Move_079 < PokeBattle_UnimplementedMove
  ...
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    if hitnum == 1 && attacker.effects[PBEffects::HydraBond] && pbNumHits(attacker)==1
      damage /= 6
    end
    if hitnum == 2 && attacker.effects[PBEffects::HydraBond] && pbNumHits(attacker)==2
      damage /= 6
    end
...
class PokeBattle_Move_07C < PokeBattle_Move
  ...
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ret=super(attacker,opponent,hitnum,alltargets,showanimation)
    if opponent.damagestate.calcdamage>0 && !opponent.damagestate.substitute &&
       opponent.status==PBStatuses::PARALYSIS && !((attacker.ability == PBAbilities::PARENTALBOND && hitnum==0) || (attacker.ability == PBAbilities::HYDRABOND && hitnum==0) || (attacker.ability == PBAbilities::HYDRABOND && hitnum==1))
      opponent.status=0
...
class PokeBattle_Move_07D < PokeBattle_Move
  ...
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ret=super(attacker,opponent,hitnum,alltargets,showanimation)
    if opponent.damagestate.calcdamage>0 && !opponent.damagestate.substitute &&
       opponent.status==PBStatuses::SLEEP && !((attacker.ability == PBAbilities::PARENTALBOND && hitnum==0) || (attacker.ability == PBAbilities::HYDRABOND && hitnum==0) || (attacker.ability == PBAbilities::HYDRABOND && hitnum==1))
      opponent.pbCureStatus
    end
    return ret
  end
...
class PokeBattle_Move_07F < PokeBattle_Move
  def pbBaseDamage(basedmg,attacker,opponent)
    if (opponent.status>0 || opponent.ability == PBAbilities::COMATOSE || @battle.FE == PBFields::FALLOUT) &&
      opponent.effects[PBEffects::Substitute]==0
      return basedmg*2
    end
    return basedmg
  ...
  def pbBaseDamage(basedmg,attacker,opponent)
    ...
    if attacker.itemWorks? 
      case attacker.item
        ...
        when PBItems::NUCLEARGEM then gem = true if movetype == PBTypes::NUCLEAR
        when PBItems::COSMICGEM then gem = true if movetype == PBTypes::COSMIC
      end
    end
...
class PokeBattle_Move_087 < PokeBattle_Move
  def pbBaseDamage(basedmg,attacker,opponent)
    if @battle.pbWeather!=0 || @battle.FE == PBFields::RAINBOWF || attacker.ability == PBAbilities::HEATGENERATOR
      return basedmg*2
    end
    return basedmg
  end
  ...
  def pbType(attacker,type=@type)
    ...
    type=((PBTypes::FIRE) || type) if (weather==PBWeather::SUNNYDAY && !attacker.hasWorkingItem(:UTILITYUMBRELLA)) || attacker.ability == PBAbilities::HEATGENERATOR
    ...
    type=((PBTypes::NUCLEAR) || type) if weather==PBWeather::NUCLEARFALLOUT
    type=((PBTypes::ELECTRIC) || type) if weather==PBWeather::THUNDERSTORM
    type=((PBTypes::FLYING) || type) if weather==PBWeather::FOGGYXOXO
...
class PokeBattle_Move_08B < PokeBattle_Move
  def pbOnStartUse(attacker)
    if @battle.FE == PBFields::CORROSIVEMISTF && @id == PBMoves::ERUPTION
      bearer=@battle.pbCheckGlobalAbility(:DAMP)
      if bearer
  ...
  def pbBaseDamage(basedmg,attacker,opponent)
    return [(basedmg*(attacker.hp.to_f)/attacker.totalhp).floor,1].max
  end
end
...
class PokeBattle_Move_08C < PokeBattle_Move
  def pbBaseDamage(basedmg,attacker,opponent)
    if attacker.pokemon.species == PBSpecies::REGIGIGAS # custom
      return [(200*(opponent.hp.to_f)/opponent.totalhp).floor,1].max
    else
...
class PokeBattle_Move_090 < PokeBattle_Move
  ...
  def pbHiddenPower(user)
    ...
    types = [1,2,3,4,5,6,7,8,10,11,12,13,14,15,16,17,18]
    # Won't be Normal, ???, Nuclear, Cosmic or any of the fusion types
...
class PokeBattle_Move_09F < PokeBattle_Move
  def pbType(attacker,type=@type)
    if ((@id == PBMoves::JUDGMENT) && (attacker.species == PBSpecies::ARCEUS)) || 
      ...
      type = attacker.form % 21
    end
    if attacker.itemWorks? && attacker.form<21
  ...
  def pbShowAnimation(id,attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    if @id == PBMoves::JUDGMENT
      ...
      when PBTypes::GHOST, PBTypes::COSMIC
      ...
      when PBTypes::GRASS, PBTypes::NUCLEAR
    ...
    if @id == PBMoves::MULTIATTACKGLITCH
      ...
      when PBTypes::GHOST, PBTypes::COSMIC
        @battle.pbAnimation(PBMoves::MULTIATTACKGHOST,attacker,opponent,hitnum)
      when PBTypes::STEEL
...
################################################################################
# This attack is always a critical hit, if successful. (Storm Throw / Frost Breath / Zippy Zap / Wicked Blow)
################################################################################
...
# Shadow Punch / Magical Leaf / Swift / Magnet Bomb / Disarming Voice / Smart Strike / Flower Trick / Kowtow Cleave)
################################################################################
...
class PokeBattle_Move_0AD < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    if opponent.pbPartner && !opponent.pbPartner.isFainted? && !opponent.pbPartner.effects[PBEffects::Protect] && !opponent.pbPartner.effects[PBEffects::SpikyShield] &&
      !opponent.pbPartner.effects[PBEffects::KingsShield] && !opponent.pbPartner.effects[PBEffects::Obstruct] && !opponent.pbPartner.effects[PBEffects::SilkTrap]
      opponent.pbPartner.effects[PBEffects::ProtectNegation]=true
    elsif (opponent.pbPartner.effects[PBEffects::Protect] || opponent.pbPartner.effects[PBEffects::SpikyShield] || opponent.pbPartner.effects[PBEffects::KingsShield] || opponent.pbPartner.effects[PBEffects::SilkTrap] || opponent.pbPartner.effects[PBEffects::Obstruct]) &&
...
class PokeBattle_Move_0BC < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    if @battle.FE == PBFields::BIGTOPA
      ...
      opponent.effects[PBEffects::Encore] = 8 if opponent.hasMovedThisRound?

    else
      opponent.effects[PBEffects::Encore] = 3
      opponent.effects[PBEffects::Encore] = 4 if opponent.hasMovedThisRound?
    end
...
################################################################################
# Hits twice. (Gear Grind / Double Hit / Double Kick / Dual Chop
# / Bonemerang / Dual Wingbeat / Twin Beam)
################################################################################
...
class PokeBattle_Move_0C0 < PokeBattle_Move
  ...
  def pbNumHits(attacker)
    ...
    if @id == PBMoves::WATERSHURIKEN && attacker.species == PBSpecies::GRENINJA && !attacker.effects[PBEffects::Transform]
      ret=5 if attacker.form==1
      ret=7 if attacker.form==2
    end
    return ret
  end
end
...
class PokeBattle_Move_0C3 < PokeBattle_Move
  def pbTwoTurnAttack(attacker,checking=false)
    @immediate=false
    if attacker.effects[PBEffects::TwoTurnAttack]==0 # custom
      @immediate=true if @battle.pbWeather==PBWeather::STRONGWINDS
    end
...
class PokeBattle_Move_0C4 < PokeBattle_Move
  def pbTwoTurnAttack(attacker,checking=false)
    ...
    if attacker.effects[PBEffects::TwoTurnAttack]==0
      @immediate=true if (@battle.pbWeather==PBWeather::SUNNYDAY && !attacker.hasWorkingItem(:UTILITYUMBRELLA)) || attacker.ability == PBAbilities::HEATGENERATOR
...
class PokeBattle_Move_0C5 < PokeBattle_Move
  def pbTwoTurnAttack(attacker,checking=false)
    @immediate=false
    if attacker.effects[PBEffects::TwoTurnAttack]==0 # custom
      @immediate=true if @battle.FE == PBFields::ICYF
    end
    if !@immediate && attacker.hasWorkingItem(:POWERHERB)
      @immediate=true
      if !checking
...
class PokeBattle_Move_0CD < PokeBattle_Move
  ...
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    ret=super(attacker,opponent,hitnum,alltargets,showanimation)
    opponent.effects[PBEffects::ProtectNegation]=true if ret>0
    if opponent && !opponent.isFainted? && !opponent.effects[PBEffects::Protect] && !opponent.effects[PBEffects::SpikyShield] && !opponent.effects[PBEffects::SilkTrap] &&
    ...
    elsif (opponent.effects[PBEffects::Protect] || opponent.effects[PBEffects::SpikyShield] || opponent.effects[PBEffects::KingsShield] || opponent.effects[PBEffects::Obstruct]  || opponent.effects[PBEffects::SilkTrap]) &&
          (opponent.pbOwnSide.effects[PBEffects::CraftyShield] || opponent.pbOwnSide.effects[PBEffects::WideGuard] || opponent.pbOwnSide.effects[PBEffects::QuickGuard] ||
          opponent.pbOwnSide.effects[PBEffects::MatBlock])
      opponent.pbOwnSide.effects[PBEffects::CraftyShield]=false
      opponent.pbOwnSide.effects[PBEffects::WideGuard]=false
...
class PokeBattle_Move_0CE < PokeBattle_Move
  ...
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    if opponent.effects[PBEffects::Protect] || opponent.effects[PBEffects::KingsShield] || opponent.effects[PBEffects::SpikyShield] || opponent.effects[PBEffects::Obstruct] || opponent.effects[PBEffects::SilkTrap]
      @battle.pbDisplay(_INTL("But it failed!"))
      attacker.effects[PBEffects::TwoTurnAttack] = 0
      attacker.effects[PBEffects::SkyDroppee] = nil
      return -1
  ...
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    if !opponent.isFainted? && opponent.damagestate.calcdamage>0 &&
      ...
      if opponent.effects[PBEffects::MultiTurn]==0
        ...
        opponent.effects[PBEffects::BindingBand] = attacker.hasWorkingItem(:BINDINGBAND)
        if (@id == PBMoves::BIND)
  ...
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    if opponent.damagestate.calcdamage>0 && attacker.effects[PBEffects::Outrage]==0 && attacker.status!=PBStatuses::SLEEP  #TODO: Not likely what actually happens, but good enough
      if attacker.ability == PBAbilities::PARENTALBOND
        ...
        attacker.effects[PBEffects::Outrage]=6+(@battle.pbRandom(2)*3)
      else
...
class PokeBattle_Move_0D8 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    hpgain=0
    if (@battle.pbWeather==PBWeather::SUNNYDAY && !attacker.hasWorkingItem(:UTILITYUMBRELLA)) || attacker.ability == PBAbilities::HEATGENERATOR
...
# Horn Leech / Mega Drain / Absorb / Parabolic Charge / Bouncy Bubble / Bitter Blade)
################################################################################
...
class PokeBattle_Move_0E2 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    if !prevented && opponent.hasWorkingItem(:CLEARAMULET)
      @battle.pbDisplay(_INTL("{1}'s {2} prevents stat loss!",opponent.pbThis,
        PBItems.getName(opponent.item)))
      prevented=true
    end
  ...
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    for i in 0...4
      if @battle.battlers[i].effects[PBEffects::PerishSong]==0 &&
         (@battle.battlers[i].ability != PBAbilities::SOUNDPROOF || @battle.battlers[i].ability != PBAbilities::GOODASGOLD || @battle.battlers[i].moldbroken)
        failed=false; break
      end
    end
    if failed
...
class PokeBattle_Move_0EB < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    if (opponent.ability == PBAbilities::GUARDDOG) && !(opponent.moldbroken)
      @battle.pbDisplay(_INTL("{1} refused to move away!",opponent.pbThis,PBAbilities.getName(opponent.ability)))
      return -1
    end
...
class PokeBattle_Move_0EC < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    if !attacker.isFainted? && !opponent.isFainted? &&
     ...
     (opponent.ability != PBAbilities::SUCTIONCUPS || opponent.ability != PBAbilities::GUARDDOG || opponent.moldbroken) &&
     !opponent.effects[PBEffects::Ingrain] && !((attacker.ability == PBAbilities::PARENTALBOND && hitnum==0) ||(attacker.ability == PBAbilities::HYDRABOND && hitnum==0) ||(attacker.ability == PBAbilities::HYDRABOND && hitnum==1))
      if !@battle.opponent
        if !(opponent.level>attacker.level)
          @battle.decision=3 # Set decision to escaped
        else
...
class PokeBattle_Move_0EE < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    if !attacker.isFainted? && @battle.pbCanChooseNonActive?(attacker.index) &&
       !@battle.pbAllFainted?(@battle.pbParty(opponent.index)) && !((attacker.ability == PBAbilities::PARENTALBOND && hitnum==0) ||(attacker.ability == PBAbilities::HYDRABOND && hitnum==0) ||(attacker.ability == PBAbilities::HYDRABOND && hitnum==1))

      oppEject = opponent.hasWorkingItem(:EJECTBUTTON) && !opponent.damagestate.substitute && opponent.damagestate.calcdamage>0 && !opponent.isFainted? && @battle.pbCanChooseNonActive?(opponent.index) && !@battle.pbAllFainted?(@battle.pbParty(opponent.index))
      
      attacker.userSwitch = true if !oppEject

      if (pbTypeModifier(@type,attacker,opponent)==0 && !@battle.typesInverted?) || oppEject
...
class PokeBattle_Move_0F0 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    if opponent.damagestate.calcdamage>0 && !opponent.damagestate.substitute && opponent.item!=0
      if opponent.hasWorkingItem(:ROCKYHELMET,true) && attacker.ability != PBAbilities::MAGICGUARD && !(opponent.ability == PBAbilities::STICKYHOLD && !(opponent.moldbroken))
        @battle.scene.pbDamageAnimation(attacker,0)
        attacker.pbReduceHP((attacker.totalhp/6.0).floor)
        @battle.pbDisplay(_INTL("{1} was hurt by the {2}!",attacker.pbThis,PBItems.getName(opponent.item)))
        if attacker.hp<=0
          return ret
        end
      end
      if opponent.ability == PBAbilities::STICKYHOLD && !(opponent.moldbroken)
      ...
      elsif !@battle.pbIsUnlosableItem(opponent,opponent.item) && !((attacker.ability == PBAbilities::PARENTALBOND && hitnum==0) ||(attacker.ability == PBAbilities::HYDRABOND && hitnum==0) || (attacker.ability == PBAbilities::HYDRABOND && hitnum==1))
        item = opponent.item
        ...
        if opponent.item == PBItems::WEAKNESSPOLICY && opponent.damagestate.typemod>4 && opponent.hp > 0
          if opponent.pbCanIncreaseStatStage?(PBStats::ATTACK)
            ...
            @battle.pbDisplay(_INTL("{1}'s {2} sharply raised its Attack!", opponent.pbThis,PBItems.getName(item)))
            opponent.pbDisposeItem(false)
          end
          if opponent.pbCanIncreaseStatStage?(PBStats::SPATK)
            opponent.pbIncreaseStatBasic(PBStats::SPATK,2)
            @battle.pbCommonAnimation("StatUp",opponent,nil)
            @battle.pbDisplay(_INTL("{1}'s {2} sharply raised its Special Attack!", opponent.pbThis,PBItems.getName(item)))
            opponent.pbDisposeItem(false)
          end
        end
        opponent.effects[PBEffects::ChoiceBand]=-1
        if opponent != 0
          # Knocking off the item
...
class PokeBattle_Move_0F2 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    tempEffect=attacker.pokemon.corrosiveGas # essentials
...
class PokeBattle_Move_0F4 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    if !attacker.isFainted? && opponent.damagestate.calcdamage>0 &&
       !opponent.damagestate.substitute && pbIsBerry?(opponent.item) && !((attacker.ability == PBAbilities::PARENTALBOND && hitnum==0) ||(attacker.ability == PBAbilities::HYDRABOND && hitnum==0) || (attacker.ability == PBAbilities::HYDRABOND && hitnum==1)) &&
      ...
      if opponent.ability == PBAbilities::STICKYHOLD && !(opponent.moldbroken)
        ...
        if attacker.ability != PBAbilities::KLUTZ && attacker.effects[PBEffects::Embargo]==0 && !attacker.underAncientPressure?
...
# Double-Edge / Wood Hammer / Wave Crash)
################################################################################
...
################################################################################
# User takes recoil damage equal to 1/2 of the damage this move dealt. (Head Smash)
################################################################################
...
class PokeBattle_Move_0FF < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    if @battle.state.effects[PBEffects::HeavyRain]
      ...
      @battle.pbDisplay(_INTL("No sunlight gets through these dark clouds!"))
      return -1
...
################################################################################
# If attack misses, user takes crash damage of 1/2 of max HP. (High Jump Kick / Jump Kick / Axe Kick)
################################################################################
class PokeBattle_Move_10B < PokeBattle_Move
  ...
  def pbAdditionalEffect(attacker,opponent) # Axe Kick
    if opponent.pbCanConfuse?(false)
      opponent.effects[PBEffects::Confusion]=2+@battle.pbRandom(4)
      @battle.pbCommonAnimation("Confusion",opponent,nil)
      @battle.pbDisplay(_INTL("{1} became confused!",opponent.pbThis))
...
class PokeBattle_Move_110 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    if !attacker.isFainted? && opponent.damagestate.calcdamage>0
      ...
      #!if attacker.effects[PBEffects::Decrystalseed]>=0
        #!attacker.effects[PBEffects::Decrystalseed]=-1
      #!end
      if attacker.pbOwnSide.effects[PBEffects::StealthRock]
        attacker.pbOwnSide.effects[PBEffects::StealthRock]=false
        @battle.pbDisplay(_INTL("{1} blew away stealth rocks!",attacker.pbThis))
      end
      if attacker.pbOwnSide.effects[PBEffects::Decrystals]
        attacker.pbOwnSide.effects[PBEffects::Decrystals]=false
        @battle.pbDisplay(_INTL("{1} blew away crystallized rocks!",attacker.pbThis))
      end
...
class PokeBattle_Move_11E < PokeBattle_Move
  ...
  def pbAdditionalEffect(attacker,opponent) # custom
    if opponent.pbCanReduceStatStage?(PBStats::ACCURACY,false)
       opponent.pbReduceStat(PBStats::ACCURACY,2,abilitymessage:false, statdropper: attacker)
    end
    return true
...
class PokeBattle_Move_157 < PokeBattle_Move
  ...
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ret=super(attacker,opponent,hitnum,alltargets,showanimation)
    opponent.effects[PBEffects::ProtectNegation]=true if ret>0
    if opponent.pbPartner && !opponent.pbPartner.isFainted? && !opponent.pbPartner.effects[PBEffects::Protect] && !opponent.pbPartner.effects[PBEffects::SpikyShield] && !opponent.pbPartner.effects[PBEffects::SilkTrap] &&
       !opponent.pbPartner.effects[PBEffects::KingsShield] && !opponent.pbPartner.effects[PBEffects::Obstruct]
      opponent.pbPartner.effects[PBEffects::ProtectNegation]=true
    elsif (opponent.pbPartner.effects[PBEffects::Protect] || opponent.pbPartner.effects[PBEffects::SpikyShield] || opponent.pbPartner.effects[PBEffects::KingsShield] || opponent.pbPartner.effects[PBEffects::Obstruct] || opponent.pbPartner.effects[PBEffects::SilkTrap]) &&
          (opponent.pbOwnSide.effects[PBEffects::CraftyShield] || opponent.pbOwnSide.effects[PBEffects::WideGuard] || opponent.pbOwnSide.effects[PBEffects::QuickGuard] ||
...
class PokeBattle_Move_15B < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    if attacker.pbOwnSide.effects[PBEffects::AuroraVeil]>0 || (@battle.pbWeather!=PBWeather::HAIL && ![4,9,13,25,28,30,34].include?(@battle.FE))
      @battle.pbDisplay(_INTL("But it failed!"))
      return -1
    end
    pbShowAnimation(@id,attacker,opponent,hitnum,alltargets,showanimation)
...
class PokeBattle_Move_15E < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    if !attacker.pbHasType?(:FIRE,ignorefusion:true) || attacker.effects[PBEffects::BurnUp]
      @battle.pbDisplay(_INTL("But it failed!"))
      return -1
    end
...
class PokeBattle_Move_16E < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    totalboost = 0
    if pbTypeModifier(@type,attacker,opponent) != 0 || @battle.FE == PBFields::INVERSEF
      for i in [PBStats::ATTACK,PBStats::DEFENSE,PBStats::SPEED,
                PBStats::SPATK,PBStats::SPDEF,PBStats::ACCURACY,PBStats::EVASION]
        if opponent.stages[i]>0
          oppboost = opponent.stages[i]
          oppboost *= -1 if attacker.ability == PBAbilities::CONTRARY
          oppboost *= -1 if attacker.ability == PBAbilities::ALOLANTECHNIQUESB
...
class PokeBattle_Move_175 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    ret=super(attacker,opponent,hitnum,alltargets,showanimation)
    if attacker.ability != PBAbilities::MAGICGUARD && opponent.index == alltargets.last
      attacker.pbReduceHP((attacker.totalhp/2.0).ceil)
      @battle.pbDisplay(_INTL("{1} is damaged by the recoil!",attacker.pbThis))
    end
    return ret
  end
...
class PokeBattle_Move_176 < PokeBattle_Move
  ...
  def pbIsPhysical?(type=@type)
    ...
    if attacker.class == PokeBattle_Battler
      ...
      atkmult *= 0.75 if attacker.ability == PBAbilities::SLOWSTART && attacker.turncount<4
      atkmult *= 2 if attacker.ability == PBAbilities::QUICKSTART && attacker.turncount<2
      atkmult *= 0.5 if attacker.ability == PBAbilities::QUICKSTART && attacker.turncount>2
    ...
    if attacker.class == PokeBattle_Battler
      ...
      spatkmult *= 2 if attacker.ability == (PBAbilities::PUREPOWER && @battle.FE == PBFields::PSYCHICT) || PBAbilities::PUREFOCUS
    end
...
class PokeBattle_Move_17A < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    if pbIsBerry?(attacker.item) && !attacker.pokemon.corrosiveGas
...
class PokeBattle_Move_17F < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    for mon in @battle.battlers
      if pbIsBerry?(mon.item) && !mon.pokemon.corrosiveGas
        ourberry = mon.item
        itemname=PBItems.getName(ourberry)
        mon.item=0
        mon.pokemon.itemRecycle = ourberry
        mon.pokemon.itemInitial=0 if mon.pokemon.itemInitial==ourberry
        @battle.pbDisplay(_INTL("{1} ate its {2}!",mon.pbThis,itemname))
        mon.pbUseBerry(ourberry,true)
      end
    end
    return 0
  end
...
class PokeBattle_Move_180 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    if opponent.effects[PBEffects::Octolock] >= 0
      @battle.pbDisplay(_INTL("But it failed!"))
      return -1
    end
    opponent.effects[PBEffects::Octolock] = attacker.index
...
class PokeBattle_Move_181 < PokeBattle_Move
  def pbBaseDamage(basedmg,attacker,opponent)
    if !opponent.hasMovedThisRound? || @battle.switchedOut[opponent.index] || (@battle.FE == PBFields::ELECTRICT && @id == PBMoves::BOLTBEAK)
      return basedmg*2
    else
      return basedmg
    end
...
class PokeBattle_Move_185 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    if !opponent.pbCanIncreaseStatStage?(PBStats::ATTACK,false) &&
       !opponent.pbCanIncreaseStatStage?(PBStats::SPATK,false)
      @battle.pbDisplay(_INTL("{1}'s stats won't go any higher!",opponent.pbThis))
      return -1
    end
    pbShowAnimation(@id,attacker,opponent,hitnum,alltargets,showanimation)
    for stat in [PBStats::ATTACK,PBStats::SPATK]
      if opponent.pbCanIncreaseStatStage?(stat,false)
        opponent.pbIncreaseStat(stat,1,abilitymessage:false)
      end
    end
    return 0
  end
...
################################################################################
# Heals self and partner by 1/4 max HP (Life Dew)
################################################################################
...
################################################################################
# Works only on terrain. Destroys temporary created terrain (Steel Roller)
################################################################################
...
################################################################################
# Hits 2-5 times, boosts Speed and drops Defense after last hit (Scale Shot)
################################################################################
class PokeBattle_Move_202 < PokeBattle_Move
  ...
  #Stat change part is handled in PokeBattle_Battler
end
...
################################################################################
# Increased priority when on Grassy Terrain (Grassy Glide) 
################################################################################
...
class PokeBattle_Move_206 < PokeBattle_Move
  def pbBaseDamage(basedmg,attacker,opponent)
    if ![0, PBFields::INDOORA, PBFields::INDOORB, PBFields::INDOORC, PBFields::CITY, PBFields::CITYNEW].include?(@battle.field.effect)
      return basedmg*2
    end
    return basedmg
  end
  ...
  def pbType(attacker,type=@type)
    field=@battle.field.effect
    type = PBTypes::NORMAL || 0
    type = (PBTypes::NORMAL || type) if field==PBFields::HOLYF
    type = (PBTypes::ELECTRIC || type) if field==PBFields::ELECTRICT
    type = (PBTypes::FIRE || type) if field==PBFields::BURNINGF || field==PBFields::SUPERHEATEDF
    type = (PBTypes::WATER || type) if field==PBFields::WATERS || field==PBFields::UNDERWATER || field==PBFields::MURKWATERS
    type = (PBTypes::ICE || type) if field==PBFields::ICYF || field==PBFields::MIRRORA || field==PBFields::SNOWYM
    type = (PBTypes::GRASS || type) if field==PBFields::GRASSYT || field==PBFields::FORESTF || field==PBFields::FLOWERGARDENF
    type = (PBTypes::FIGHTING || type) if field==PBFields::ASHENB || field==PBFields::BIGTOPA
    type = (PBTypes::PSYCHIC || type) if field==PBFields::PSYCHICT || field==PBFields::CHESSB
    type = (PBTypes::DARK || type) if field==PBFields::DARKCRYSTALC || field==PBFields::STARLIGHTA
    type = (PBTypes::GHOST || type) if field==PBFields::SHORTCIRCUITF
    type = (PBTypes::STEEL || type) if field==PBFields::FACTORYF
    type = (PBTypes::POISON || type) if field==PBFields::CORROSIVEF || field==PBFields::CORROSIVEMISTF || field==PBFields::WASTELAND
    type = (PBTypes::FLYING || type) if field==PBFields::MOUNTAIN
    type = (PBTypes::GROUND || type) if field==PBFields::CAVE || field==PBFields::DESERTF
    type = (PBTypes::ROCK || type) if field==PBFields::ROCKYF || field==PBFields::CRYSTALC
    type = (PBTypes::DRAGON || type) if field==PBFields::DRAGONSD
    type = (PBTypes::FAIRY || type) if field==PBFields::RAINBOWF || field==PBFields::FAIRYTALEF || field==PBFields::MISTYT
    type = (PBTypes::QMARKS || type) if field==PBFields::GLITCHF || field==PBFields::NEWW
    type = (PBTypes::COSMIC || type) if field==PBFields::GUFIELD || field==PBFields::DWORLD
    type = (PBTypes::NUCLEAR || type) if field==PBFields::FALLOUT
    type = super(attacker,type)
    return type
  end
end
...
class PokeBattle_Move_207 < PokeBattle_Move
  def pbAdditionalEffect(attacker,opponent)
    if opponent.pbCanBurn?(false) && opponent.effects[PBEffects::BurningJealousy]
       opponent.pbBurn(attacker)
    end
  end
end
...
class PokeBattle_Move_209 < PokeBattle_Move
  def pbMoveFailed(attacker,opponent)
    if opponent.item==0 || @battle.pbIsUnlosableItem(opponent,opponent.item) || !opponent.itemWorks?
      return true
    else
...
class PokeBattle_Move_210 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    if opponent.pokemon.corrosiveGas || opponent.ability == PBAbilities::STICKYHOLD || 
    ...
    pbShowAnimation(@id,attacker,opponent,hitnum,alltargets,showanimation)
    opponent.pokemon.corrosiveGas = true
    @battle.pbDisplay(_INTL("{1} corroded {2}'s {3}!",attacker.pbThis,opponent.pbThis(true),PBItems.getName(opponent.item)))
    return 0
  end
end

################################################################################
# Increases the target's Attack and Defense by 1 stage each (Coaching)
################################################################################
class PokeBattle_Move_211 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    for stat in [PBStats::ATTACK,PBStats::DEFENSE]
      if opponent.pbCanIncreaseStatStage?(stat,false)
        boost_amt = @battle.FE == PBFields::ASHENB ? 2 : 1
        opponent.pbIncreaseStat(stat,boost_amt,abilitymessage:false)
      end
    end
    return 0
  end
...
###############################################################################
# Heals self and partner by 1/4 max HP. Also heals status conditions (Jungle Healing / Lunar Blessing)
################################################################################
class PokeBattle_Move_212 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    if attacker.hp == attacker.totalhp && attacker.status == 0 && attacker.pbPartner.hp == attacker.pbPartner.totalhp && attacker.pbPartner.status == 0
      @battle.pbDisplay(_INTL("But it failed!"))
      return -1
    end
    pbShowAnimation(@id,attacker,opponent,hitnum,alltargets,showanimation)
...
class PokeBattle_Move_214 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ret = super(attacker,opponent,hitnum,alltargets,showanimation)
    for i in opponent.moves
      if opponent.lastMoveUsed>0 && i.id==opponent.lastMoveUsed && i.pp>0
...
################################################################################
# User loses 50% of max HP when using the move (Steel Beam / Chloroblast)
################################################################################
...
################################################################################
# Deals more damage on Psychic Terrain and hits both targets (Expanding Force)
################################################################################
class PokeBattle_Move_216 < PokeBattle_Move
  def pbBaseDamage(basedmg,attacker,opponent)
    return basedmg*1.5 if @battle.FE == PBFields::PSYCHICT && !attacker.isAirborne?
    return basedmg
  end
end
...
################################################################################
# Lays stealth rocks when dealing damage. (Stone Axe)
################################################################################
class PokeBattle_Move_220 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ret = super(attacker,opponent,hitnum,alltargets,showanimation)
    if !opponent.pbOwnSide.effects[PBEffects::StealthRock]
      opponent.pbOwnSide.effects[PBEffects::StealthRock]=true
...
################################################################################
# Lays spikes when dealing damage. (Ceaseless Edge)
################################################################################
class PokeBattle_Move_221 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ret = super(attacker,opponent,hitnum,alltargets,showanimation)
    if @battle.FE != PBFields::WATERS && @battle.FE != PBFields::MURKWATERS && !(opponent.pbOwnSide.effects[PBEffects::Spikes]>=3)
      opponent.pbOwnSide.effects[PBEffects::Spikes]+=1
      if @battle.pbIsOpposing?(opponent.index)
        @battle.pbDisplay(_INTL("Spikes were scattered all around the feet of the foe's team!"))
      else
        @battle.pbDisplay(_INTL("Spikes were scattered all around the feet of your team!"))
...
################################################################################
# Deals damage and may inflict poison, paralysis or sleep. (Dire Claw)
################################################################################
...
################################################################################
# Swaps offenses with defenses (retained PLA effect). (Power Shift)
################################################################################
class PokeBattle_Move_224 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    @battle.pbDisplay(_INTL("{1} switched its offensive and defensive stats!",attacker.pbThis))
    return 0
  end
end

######################################################################################
# Raises the user's Attack, Defense, and Speed. (Victory Dance)
######################################################################################
class PokeBattle_Move_225 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    showanim=true
    boost_amount=1
    if @battle.FE == PBFields::BIGTOPA && (@id == PBMoves::VICTORYDANCE)
      boost_amount=2
    end
...
# May also poison the target. (Barb Barrage)
################################################################################
...
################################################################################
# Lowers the target's Defense. May cause it to flinch. (Triple Arrows)
################################################################################
class PokeBattle_Move_229 < PokeBattle_Move
  ...
  def pbSecondAdditionalEffect(attacker,opponent)
    if rand(5)<3 && opponent.ability != PBAbilities::INNERFOCUS && !opponent.damagestate.substitute #We change the 50% chance to a 30%
      opponent.effects[PBEffects::Flinch]=true
      return true
    end
    return false
...
################################################################################
# Power is doubled if the target has a status problem. May burn the target. (Infernal Parade)
################################################################################
...
################################################################################
# Hits 3 times. 100% critical hit chance (handled in superclass). (Surging Strikes)
################################################################################
...
################################################################################
# Heals user's status, and raises Special Attack and Special Defense. (Take Heart)
################################################################################
class PokeBattle_Move_233 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    pbShowAnimation(@id,attacker,nil,hitnum,alltargets,showanimation)
    boost_amount=1
    boost_amount=2 if @battle.FE == PBFields::STARLIGHTA || @battle.FE == PBFields::NEWW || @battle.FE == PBFields::PSYCHICT
    for stat in [PBStats::SPATK,PBStats::SPDEF]
      if attacker.pbCanIncreaseStatStage?(stat,false)
        attacker.pbIncreaseStat(stat,boost_amount,abilitymessage:false)
      end
    end
    attacker.pbCureStatusComplete #New method to reduce redundancy
    return 0
  end
end

################################################################################
# Hits twice. Chance to flinch. (Double Iron Bash)
################################################################################
...
################################################################################
# Deals randomly Fire, Electric, or Ice damage (Laser Pulse)
################################################################################
class PokeBattle_Move_240 < PokeBattle_Move
  def pbType(attacker,type=@type)
    rnd=@battle.pbRandom(3) #0=Fire / 1=Electric/ 2=Ice
      case rnd
        when 0
          return type=PBTypes::FIRE
        when 1
          return type=PBTypes::ELECTRIC          
        when 2
          return type=PBTypes::ICE
        end
  end
end

################################################################################
# Decreases the target's Defense and Special Defense by 2 stage each. (Ionic Strain)
################################################################################
class PokeBattle_Move_241 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    pbShowAnimation(@id,attacker,opponent,hitnum,alltargets,showanimation)
    ret=-1; showanim=true
    if opponent.pbReduceStat(PBStats::DEFENSE,2,abilitymessage:false, statdropper: attacker)
      ret=0; showanim=false
    end
    if opponent.pbReduceStat(PBStats::SPDEF,2,abilitymessage:false, statdropper: attacker)
      ret=0; showanim=false
    end
    return ret
  end
...
class PokeBattle_Move_242 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    if !@battle.canChangeFE?(PBFields::FALLOUT)
      @battle.pbDisplay(_INTL("But it failed!"))
      return -1
    end
    pbShowAnimation(@id,attacker,opponent,hitnum,alltargets,showanimation)
    ...
    @battle.field.duration=5
    @battle.field.duration=8 if (attacker.item == PBItems::AMPLIFIELDROCK)
    @battle.pbDisplay(_INTL("Radioactive energy fills the area!"))
    return 0
  end
end

################################################################################
# Inflicts a fixed 70HP damage. (Blast Wave)
################################################################################
class PokeBattle_Move_243 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    return pbEffectFixedDamage(70,attacker,opponent,hitnum,alltargets,showanimation)
  end
end

################################################################################
# Puts the user and the target to sleep. (Over Dosage)
################################################################################
class PokeBattle_Move_244 < PokeBattle_Move
   def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    if !attacker.pbCanSleep?(true,true,true) || attacker.status==PBStatuses::SLEEP || !opponent.pbCanSleep?(true) || opponent.status==PBStatuses::SLEEP
      @battle.pbDisplay(_INTL("But it failed!"))
      return -1
    end
      pbShowAnimation(@id,attacker,opponent,hitnum,alltargets,showanimation)
      opponent.pbSleep
      @battle.pbDisplay(_INTL("{1} fell unconscious!",opponent.pbThis))
      attacker.pbSleepSelf(3)
      @battle.pbDisplay(_INTL("{1} fell unconscious!",attacker.pbThis))
      return 0
    end
  end

################################################################################
# Increases all stats, cursing user (Meltdown)
################################################################################
class PokeBattle_Move_245 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    if !attacker.pbCanIncreaseStatStage?(PBStats::ATTACK,false) &&
       ...
       !attacker.pbCanIncreaseStatStage?(PBStats::SPDEF,false) &&
       !attacker.pbCanIncreaseStatStage?(PBStats::ACCURACY,false) &&
       !attacker.pbCanIncreaseStatStage?(PBStats::EVASION,false) &&
       !attacker.pbCanIncreaseStatStage?(PBStats::SPEED,false)
      @battle.pbDisplay(_INTL("{1}'s stats won't go any higher!",attacker.pbThis))
      return -1
    end
    pbShowAnimation(@id,attacker,nil,hitnum,alltargets,showanimation)
    boost_amount=1
    for stat in [PBStats::ATTACK,PBStats::DEFENSE,PBStats::SPATK,PBStats::SPDEF,PBStats::ACCURACY,PBStats::EVASION,PBStats::SPEED]
      if attacker.pbCanIncreaseStatStage?(stat,false)
        attacker.pbIncreaseStat(stat,boost_amount,abilitymessage:false)
      end
    end
    @battle.pbDisplay(_INTL("{1}'s Meltdown drains on its health!",attacker.pbThis))
    attacker.pbReduceHP((attacker.totalhp/3.0).floor,true)
    return 0
  end
end

################################################################################
# Decreases the target's Defense and Sp.Defense by 1 stage each. (Caustic Breath)
################################################################################
...
################################################################################
# Silk Trap (protects and reduces Speed on contact)
################################################################################
...
################################################################################
# Spicy Extract (sharply raises Attack but harshly lowers Defense of the target)
################################################################################
class PokeBattle_Move_253 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    if !opponent.pbCanIncreaseStatStage?(PBStats::ATTACK,false) && !opponent.pbCanReduceStatStage?(PBStats::DEFENSE,false)
      @battle.pbDisplay(_INTL("But it failed!"))
      return -1
    end
    pbShowAnimation(@id,attacker,opponent,hitnum,alltargets,showanimation)
...
class PokeBattle_Move_254 < PokeBattle_Move
  def pbAdditionalEffect(attacker,opponent)
    if attacker.pbCanReduceStatStage?(PBStats::SPEED,false)
      attacker.pbReduceStat(PBStats::SPEED,2,abilitymessage:false, statdropper: attacker)
    end
    return true
  end
end
...
################################################################################
# Deals damage. Destroys temporary created terrain if present (Ice Spinner)
################################################################################
...
################################################################################
# Removes trapping moves and Leech Seed on user/user's side. Poisons target. (Mortal Spin)
################################################################################
...
################################################################################
# Sacrifices 50% of max HP and sharply boosts Attack, Special Attack and Speed. (Fillet Away)
################################################################################
class PokeBattle_Move_257 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    if attacker.hp<=(attacker.totalhp/2.0).floor
      @battle.pbDisplay(_INTL("It was too weak to use the move!"))
      return -1
    elsif !attacker.pbCanIncreaseStatStage?(PBStats::ATTACK,false) &&
          !attacker.pbCanIncreaseStatStage?(PBStats::SPATK,false) &&
          !attacker.pbCanIncreaseStatStage?(PBStats::SPEED,false)
      @battle.pbDisplay(_INTL("{1}'s stats won't go any higher!",attacker.pbThis))
      return -1
...
################################################################################
# Type depends on user's form. Removes Reflect and similar. (Raging Bull)
################################################################################
class PokeBattle_Move_258 < PokeBattle_Move
  def pbType(attacker,type=@type)
    if attacker.species == PBSpecies::TAUROS
      type = PBTypes::FIGHTING if attacker.form == 1
      type = PBTypes::FIRE if attacker.form == 2
      type = PBTypes::WATER if attacker.form == 3
    end
    return super(attacker,type)
  end
...
################################################################################
# Removes entry hazards and Substitutes from both user and opponent's sides. Raises Attack and Speed by one stage. (Tidy Up)
################################################################################
class PokeBattle_Move_259 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    tidy = false
    if attacker.pbOwnSide.effects[PBEffects::StealthRock] || attacker.pbOpposingSide.effects[PBEffects::StealthRock]
      attacker.pbOwnSide.effects[PBEffects::StealthRock]=false
      attacker.pbOpposingSide.effects[PBEffects::StealthRock]=false
      @battle.pbDisplay(_INTL("{1} blew away all stealth rocks!",attacker.pbThis))
      tidy = true
    end
    if attacker.pbOwnSide.effects[PBEffects::Decrystals] || attacker.pbOpposingSide.effects[PBEffects::Decrystals]
      attacker.pbOwnSide.effects[PBEffects::Decrystals]=false
      attacker.pbOpposingSide.effects[PBEffects::Decrystals]=false
      @battle.pbDisplay(_INTL("{1} blew away all crystallized rocks!",attacker.pbThis))
      tidy = true
    end
    if attacker.pbOwnSide.effects[PBEffects::Spikes]>0 || attacker.pbOpposingSide.effects[PBEffects::Spikes]>0
      attacker.pbOwnSide.effects[PBEffects::Spikes]=0
      attacker.pbOpposingSide.effects[PBEffects::Spikes]=0
      @battle.pbDisplay(_INTL("{1} blew away all spikes!",attacker.pbThis))
      tidy = true
    end
    if attacker.pbOwnSide.effects[PBEffects::ToxicSpikes]>0 || attacker.pbOpposingSide.effects[PBEffects::ToxicSpikes]>0
      attacker.pbOwnSide.effects[PBEffects::ToxicSpikes]=0
      attacker.pbOpposingSide.effects[PBEffects::ToxicSpikes]=0
      @battle.pbDisplay(_INTL("{1} blew away all poison spikes!",attacker.pbThis))
      tidy = true
    end
    if attacker.pbOwnSide.effects[PBEffects::StickyWeb] || attacker.pbOpposingSide.effects[PBEffects::StickyWeb]
      attacker.pbOwnSide.effects[PBEffects::StickyWeb]=false
      attacker.pbOpposingSide.effects[PBEffects::StickyWeb]=false
      @battle.pbDisplay(_INTL("{1} blew away all of the sticky webbing!",attacker.pbThis))
      tidy = true
    end
    subs = false
    for i in 0...4
      if @battle.battlers[i].effects[PBEffects::Substitute] > 0
        @battle.battlers[i].effects[PBEffects::Substitute] = 0
        @battle.scene.pbUnSubstituteSprite(@battle.battlers[i],@battle.battlers[i].pbIsOpposing?(1))
        subs = true; tidy = true
      end
    end
    @battle.pbDisplay(_INTL("All substitutes were removed!")) if subs

    if tidy
      @battle.pbDisplay(_INTL("Tidying up complete!"))
    else
      @battle.pbDisplay(_INTL("There was nothing to tidy up!"))
    end
    boost = false
    if attacker.pbCanIncreaseStatStage?(PBStats::ATTACK,false)
      attacker.pbIncreaseStat(PBStats::ATTACK,1,abilitymessage:false)
    ...
    return tidy || boost ? 0 : -1
  end
end

################################################################################
# Deals bonus damage if super effective. (Collision Course / Electro Drift)
################################################################################
...
################################################################################
# Creates Hail and switches out. (Chilly Reception)
################################################################################
...
class PokeBattle_Move_262 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    if !@battle.canChangeFE?(PBFields::SNOWYM)
      @battle.pbDisplay(_INTL("But it failed!"))
      return -1
    end
    pbShowAnimation(@id,attacker,opponent,hitnum,alltargets,showanimation)
    @battle.setField(PBFields::SNOWYM,true)
    @battle.field.duration=5
    @battle.field.duration=8 if (attacker.item == PBItems::AMPLIFIELDROCK)
    @battle.pbDisplay(_INTL("The terrain became a Snowscape!"))
    return 0
  end
end
...
class PokeBattle_Move_263 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    if opponent.ability==0 || PBStuff::ABILITYBLACKLIST.include?(opponent.ability) || PBStuff::FIXEDABILITIES.include?(attacker.ability) || PBStuff::FIXEDABILITIES.include?(attacker.pbPartner.ability)
      @battle.pbDisplay(_INTL("But it failed!"))
      return -1
    end
    pbShowAnimation(@id,attacker,opponent,hitnum,alltargets,showanimation)
    ...
    if attacker.pbPartner && !attacker.pbPartner.isFainted?
      @battle.pbDisplay(_INTL("{1} copied {2}'s {3} for their side!",attacker.pbThis,opponent.pbThis(true),abilityname))
      if !attacker.pbPartner.hasWorkingItem(:ABILITYSHIELD)
        attacker.pbPartner.ability = opponent.ability
      else
        @battle.pbDisplay(_INTL("{1}'s Ability is protected by its {2}!", attacker.pbPartner.pbThis, PBItems.getName(attacker.pbPartner.item)))
      end
    else
      @battle.pbDisplay(_INTL("{1} copied {2}'s {3}!",attacker.pbThis,opponent.pbThis(true),abilityname))
    end
    if !attacker.hasWorkingItem(:ABILITYSHIELD)
      attacker.ability = opponent.ability
    else
      @battle.pbDisplay(_INTL("{1}'s Ability is protected by its {2}!", attacker.pbThis, PBItems.getName(attacker.item)))
    end
...
class PokeBattle_Move_264 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    if !attacker.pbHasType?(:ELECTRIC,ignorefusion:true) || attacker.effects[PBEffects::DoubleShock]
      @battle.pbDisplay(_INTL("But it failed!"))
      return -1
    end
    attacker.effects[PBEffects::DoubleShock]=true
    ret=super(attacker,opponent,hitnum,alltargets,showanimation)
    if (attacker.type1 == PBTypes::ELECTRIC)
      if (attacker.type2 == PBTypes::ELECTRIC)
        attacker.type1=(PBTypes::QMARKS) || 0
      else
        attacker.type1=attacker.type2
      end
    ...
    @battle.pbDisplay(_INTL("{1} was overcharged!",attacker.pbThis))
    return ret
  end
end
...
class PokeBattle_Move_265 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    pbShowAnimation(@id,attacker,nil,hitnum,alltargets,showanimation)
    attacker.pbReduceHP((attacker.totalhp/2.0).ceil,false,false)
    attacker.effects[PBEffects::UsingSubstituteRightNow]=true
    @battle.scene.pbAnimation(PBMoves::SUBSTITUTE,attacker,nil)
    attacker.effects[PBEffects::UsingSubstituteRightNow]=false
    attacker.effects[PBEffects::MultiTurn]=0
    attacker.effects[PBEffects::MultiTurnAttack]=0
    @battle.pbDisplay(_INTL("{1} shed its tail to create a decoy!",attacker.pbThis))
    sublife = [(attacker.totalhp/4.0).floor,1].max
    #attacker.effects[PBEffects::Substitute] = sublife #Why bother?
...
################################################################################
# Deals more damage the more party members fainted. (Last Respects)
################################################################################
class PokeBattle_Move_266 < PokeBattle_Move
  ...
  def pbBaseDamage(basedmg,attacker,opponent)
    mons=@participants.length
    return basedmg+(mons*50)
  end
end
...
class PokeBattle_Move_267 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    if opponent.damagestate.calcdamage>0
      ...
      @battle.pbDisplay(_INTL("{1} is vulnerable for attacks!",attacker.pbThis))
    end
    return ret
  end
end
...
class PokeBattle_Move_268 < PokeBattle_Move
  def pbBaseDamage(basedmg,attacker,opponent)
    rageFist = @battle.getBattlerHit(attacker)
    dmg = basedmg + (50*rageFist)
    return dmg
  end
end
...
class PokeBattle_Move_269 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ret=super(attacker,opponent,hitnum,alltargets,showanimation)
    if !opponent.isFainted? && opponent.damagestate.calcdamage>0 && !opponent.damagestate.substitute
      if !opponent.effects[PBEffects::SaltCure]
...
class PokeBattle_Move_270 < PokeBattle_Move
  def pbAdditionalEffect(attacker,opponent)
    return false if !(attacker.effects[PBEffects::Commandee] && attacker.pbPartner.effects[PBEffects::Commander])
    ...
    attacker.pbIncreaseStat(stat,1,abilitymessage:false) if attacker.pbCanIncreaseStatStage?(stat,true)
    return true
  end
end

################################################################################
# Decreases the user's Sp. Attack by 1 stage. (Make It Rain)
################################################################################
...
################################################################################
# Power increases with the user's happiness. Never misses. (Pika Papow / Veevee Volley)
################################################################################
...
################################################################################
# For 5 rounds, lowers power of physical attacks against the user's side. (Baddy Bad)
################################################################################
class PokeBattle_Move_273 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ret = super(attacker,opponent,hitnum,alltargets,showanimation)
    return ret if attacker.pbOwnSide.effects[PBEffects::Reflect]>0
    attacker.pbOwnSide.effects[PBEffects::Reflect]=5
    attacker.pbOwnSide.effects[PBEffects::Reflect]=8 if attacker.hasWorkingItem(:LIGHTCLAY) || @battle.FE == PBFields::MIRRORA
    if !@battle.pbIsOpposing?(attacker.index)
...
################################################################################
# For 5 rounds, lowers power of special attacks against the user's side. (Glitzy Glow)
################################################################################
class PokeBattle_Move_274 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ret = super(attacker,opponent,hitnum,alltargets,showanimation)
    return ret if attacker.pbOwnSide.effects[PBEffects::LightScreen]>0
    attacker.pbOwnSide.effects[PBEffects::LightScreen]=5
    attacker.pbOwnSide.effects[PBEffects::LightScreen]=8 if attacker.hasWorkingItem(:LIGHTCLAY) || @battle.FE == PBFields::MIRRORA
    if !@battle.pbIsOpposing?(attacker.index)
...
################################################################################
# Seeds the target. Seeded Pokémon lose 1/8 of max HP at the end of each
# round, and the Pokémon in the user's position gains the same amount. (Sappy Seed)
################################################################################
class PokeBattle_Move_275 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ret = super(attacker,opponent,hitnum,alltargets,showanimation)
    return ret if opponent.effects[PBEffects::LeechSeed]>=0 || opponent.effects[PBEffects::Substitute]>0 || opponent.pbHasType?(:GRASS)
    #Now handled elsewhere
    opponent.effects[PBEffects::LeechSeed]=attacker.index
    @battle.pbDisplay(_INTL("{1} was seeded!",opponent.pbThis))
    return ret
...
################################################################################
# Deals damage and lowers target's Speed each turn for three turns. (Syrup Bomb)
################################################################################
class PokeBattle_Move_276 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ret=super(attacker,opponent,hitnum,alltargets,showanimation)
    if !opponent.isFainted? && opponent.damagestate.calcdamage>0 && !opponent.damagestate.substitute
      opponent.effects[PBEffects::Syrup] = 4
      @battle.pbDisplay(_INTL("{1} was covered in syrup!",opponent.pbThis))
    end
    return ret
  end
end
...
################################################################################
# User gains half the HP it inflicts as damage. May burn the target.
# (Matcha Gotcha)
################################################################################
...
class PokeBattle_Move_278 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ret=super(attacker,opponent,hitnum,alltargets,showanimation)
    if opponent.damagestate.calcdamage>0
      attacker.effects[PBEffects::GigatonHammer]=2
    end
    return ret
  end
end
...
class PokeBattle_Move_279 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    if revivablePokemon.length==0 || (@battle.pbOwnedByPlayer?(attacker.index) && $game_switches[:Nuzlocke_Mode]==true)
      @battle.pbDisplay(_INTL("But it failed!"))
      return -1
    end
    pbShowAnimation(@id,attacker,nil,hitnum,alltargets,showanimation)
    if !@battle.pbOwnedByPlayer?(attacker.index)
      ...
      pokemon = @battle.scene.pbChooseRevivalBlessingRecipient(attacker.index)
    end
    if pokemon
      pokemon.status=0
      pokemon.hp=1+(pokemon.totalhp/2.0).floor
      @battle.pbDisplay(_INTL("{1} was revived and is ready to fight again!",pokemon.name))
    else
...
################################################################################
# Type depends on user's form. (Ivy Cudgel)
################################################################################
class PokeBattle_Move_280 < PokeBattle_Move
  def pbType(attacker,type=@type)
    if attacker.species == PBSpecies::OGERPON
      type = PBTypes::WATER if attacker.form == 2 || attacker.form == 6
      type = PBTypes::FIRE if attacker.form == 3 || attacker.form == 7
      type = PBTypes::ROCK if attacker.form == 4 || attacker.form == 8
    end
    return super(attacker,type)
  end
end
...
class PokeBattle_Move_281 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ret=super(attacker,opponent,hitnum,alltargets,showanimation)
    if opponent.damagestate.calcdamage>0
      attacker.effects[PBEffects::BloodMoon]=2
    end
    return ret
  end
end
...
################################################################################
# Protects and burns the target on contact (Burning Bulwark)
################################################################################
class PokeBattle_Move_283 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    if @battle.pbRandom(65536)<(65536/attacker.effects[PBEffects::ProtectRate]).floor
      attacker.effects[PBEffects::Bulwark]=true
      attacker.effects[PBEffects::ProtectRate]*=3
      @battle.pbAnimation(@id,attacker,nil)
      @battle.pbDisplay(_INTL("{1} shielded itself against damage!",attacker.pbThis))
      return 0
...
################################################################################
# Hits twice and guaranteed. (Tachyon Cutter)
################################################################################
...
################################################################################
# Ignores target's protections. Does not ignore other moves thi turn. (Hyper Drill / Mighty Cleave)
################################################################################
...
################################################################################
# Increases the allies' critical hit rate. (Dragon Cheer)
################################################################################
class PokeBattle_Move_286 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    if opponent.effects[PBEffects::FocusEnergy]>=2
      @battle.pbDisplay(_INTL("But it failed!"))
      return -1
    end
    pbShowAnimation(@id,attacker,opponent,hitnum,alltargets,showanimation)
    opponent.effects[PBEffects::FocusEnergy]=2
    opponent.effects[PBEffects::FocusEnergy]=3 if (opponent.type1 == PBTypes::DRAGON || opponent.type2 == PBTypes::DRAGON)
...
################################################################################
# Deals damage and prevents healing for two turns (Psychic Noise)
################################################################################
...
################################################################################
# 30% to to double the moves damage on use. (Fickle Beam)
################################################################################
    ...
    @rnd=@battle.pbRandom(3)
      case @rnd
        ...
        when 1
          dmg = basedmg
        when 2
          @battle.pbDisplay(_INTL("{1} went for an all-out attack!",attacker.pbThis))
          dmg = basedmg*2
        end
          return dmg
     end

  def pbShowAnimation(id,attacker,opponent,hitnum=0,alltarget=nil,showanimation=true)
    return if !showanimation
    case @rnd
    when 0
      @battle.pbAnimation(PBMoves::SIMPLEBEAM,attacker,opponent,hitnum)
    when 1
      @battle.pbAnimation(PBMoves::SIMPLEBEAM,attacker,opponent,hitnum)
    when 2
      @battle.pbAnimation(PBMoves::HYPERBEAM,attacker,opponent,hitnum)
    end
  end
end
...
class PokeBattle_Move_804 < PokeBattle_Move
  ...
  def pbAdditionalEffect(attacker,opponent)
    return false if !opponent.pbCanPoison?(false)
    opponent.pbPoison(attacker) #? if you add " ,true " it means you are badly poisoning it
    @battle.pbDisplay(_INTL("{1} was poisoned!",opponent.pbThis))
    return true
  end
end
...
################################################################################
# Needle Gun (Hits 6 times, chance to flinch)
################################################################################
...
################################################################################
# May increase the user's Defense and Special Defense by 1 stage. May also heal 1/16 of HP. (Cosmic Barrage)
################################################################################
class PokeBattle_Move_907 < PokeBattle_Move
  ...
  def pbSecondAdditionalEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    return -1 if attacker.hp==attacker.totalhp
    attacker.pbRecoverHP(((attacker.totalhp+1)/16).floor,true)
    @battle.pbDisplay(_INTL("{1}'s HP was restored.",attacker.pbThis))
    return 0
  end
end
...
################################################################################
# Traps and may burn the target. May also raise Evasion and Accuracy. (Infernal Fireblast)
################################################################################
class PokeBattle_Move_908 < PokeBattle_Move
  ...
  def pbSecondAdditionalEffect(attacker,opponent)
    for stat in [PBStats::EVASION,PBStats::ACCURACY]
      if attacker.pbCanIncreaseStatStage?(stat,false)
        attacker.pbIncreaseStat(stat,1,abilitymessage:false)
      end
    end
...
################################################################################
# Decreases Sp.Atk, heals by 1/4 of the damage dealt, traps and may burn the target. (Infernal Waterblast)
################################################################################
class PokeBattle_Move_909 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    if opponent.damagestate.calcdamage>0
      hpgain=((opponent.damagestate.hplost+1)/4).floor
      if (opponent.abilityWorks?(true) && opponent.ability == PBAbilities::LIQUIDOOZE)
        hpgain*=2 if @battle.FE == PBFields::WASTELAND || @battle.FE == PBFields::MURKWATERS
        attacker.pbReduceHP(hpgain,true)
        @battle.pbDisplay(_INTL("{1} sucked up the liquid ooze!",attacker.pbThis))
...
################################################################################
# May inflict random status. May also increase Evasion. (Ancient Ice-Shard)
################################################################################
class PokeBattle_Move_910 < PokeBattle_Move
  def pbIsMultiHit
    return true
  end

  def pbNumHits(attacker)
    hitchances=[1,2,2,3,3,3]
    ret=hitchances[@battle.pbRandom(hitchances.length)]
    ret=3 if attacker.ability == PBAbilities::SKILLLINK || attacker.hasWorkingItem(:LOADEDDICE)
    return ret
  end

  def pbAdditionalEffect(attacker,opponent)
    rnd=@battle.pbRandom(7)
    case rnd
      when 0
        return false if !opponent.pbCanBurn?(false)
        opponent.pbBurn(attacker)
...
################################################################################
# Infests the opponent and recovers HP. May increase the user's and decrease the opponent's Attack, Defense, Speed, Special Attack and Special Defense
# by 1 stage. (Midnight Photosynthesis)
################################################################################
  ...
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    if attacker.hp < attacker.totalhp
      if (@battle.pbWeather==PBWeather::SUNNYDAY && !attacker.hasWorkingItem(:UTILITYUMBRELLA))
        hpgain=(attacker.totalhp/16.0).floor
      elsif @battle.pbWeather!=0
        hpgain=(attacker.totalhp/12.0).floor
      else
  ...
  def pbSecondAdditionalEffect(attacker,opponent)
    for stat in 1..5
      if opponent.pbCanReduceStatStage?(stat,false)
        opponent.pbReduceStat(stat,1, statdropper: attacker)
      end
    end
    return true
  end
...
# Entry hazard.  Lays stealth rocks on the opposing side. (Decrystallization)
################################################################################
class PokeBattle_Move_913 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    return super(attacker,opponent,hitnum,alltargets,showanimation) if @basedamage>0
    return -1 if !attacker.pbCanReduceStatStage?(PBStats::DEFENSE,true) &&
    !attacker.pbCanReduceStatStage?(PBStats::SPDEF,true) && !attacker.pbCanIncreaseStatStage?(PBStats::SPEED,true)
    pbShowAnimation(@id,attacker,opponent,hitnum,alltargets,showanimation)
    ret=attacker.pbReduceStat(PBStats::DEFENSE,2,abilitymessage:false, statdropper: attacker)
    ret=attacker.pbReduceStat(PBStats::SPDEF,2,abilitymessage:false, statdropper: attacker)
    ret=attacker.pbIncreaseStat(PBStats::SPEED,1,abilitymessage:false)
  ...
  def pbAdditionalEffect(attacker,opponent)
    for stat in [PBStats::DEFENSE,PBStats::SPDEF]
      if attacker.pbCanReduceStatStage?(stat,false)
        attacker.pbReduceStat(stat,2,abilitymessage:false, statdropper: attacker)
      end
    end
    for stat in [PBStats::SPEED]
      if attacker.pbCanIncreaseStatStage?(stat,false)
        attacker.pbIncreaseStat(stat,1,abilitymessage:false)
      end
    end
...
################################################################################
# Raises Speed, Accuracy and Evasion by 1 stage. Lowers Def and Sp. Def by 3 stages. Lays crystallized rocks on the opposing side. (Decrystallization)
################################################################################
class PokeBattle_Move_913 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    if !attacker.pbCanIncreaseStatStage?(PBStats::SPEED,false) &&
    !attacker.pbCanIncreaseStatStage?(PBStats::ACCURACY,false) &&
    !attacker.pbCanIncreaseStatStage?(PBStats::EVASION,false) &&
    attacker.pbOpposingSide.effects[PBEffects::Decrystals]
      @battle.pbDisplay(_INTL("But it failed!"))
      return -1
    end
    ...
    for stat in [PBStats::DEFENSE,PBStats::SPDEF]
      if attacker.pbCanReduceStatStage?(stat,false)
        attacker.pbReduceStat(stat,3,abilitymessage:false, statdropper: attacker) #? your Defenses take a serious hit
      end
    end
    for stat in [PBStats::SPEED,PBStats::ACCURACY,PBStats::EVASION]
      if attacker.pbCanIncreaseStatStage?(stat,false)
        attacker.pbIncreaseStat(stat,1,abilitymessage:false) #? the enemies dread your existence
      end
    end

    return 0 if attacker.pbOpposingSide.effects[PBEffects::Decrystals]
    for i in 0...4
      next if !(attacker.pbIsOpposing?(i))
      if (@battle.battlers[i].ability == PBAbilities::MAGICBOUNCE && !PBStuff::TWOTURNMOVE.include?(@battle.battlers[i].effects[PBEffects::TwoTurnAttack])) ||
      (@battle.battlers[i]).effects[PBEffects::MagicCoat]
        @battle.pbDisplay(_INTL("{1} bounced the crystals back!",(@battle.battlers[i]).pbThis))
        attacker.pbOwnSide.effects[PBEffects::Decrystals]=true
        #!attacker.pbOwnSide.effects[PBEffects::Decrystalseed]=@battle.battlers[i].index
        if @battle.pbIsOpposing?(attacker.index)
          @battle.pbDisplay(_INTL("Pointed crystals float in the air around your foe's team!"))
        else
          @battle.pbDisplay(_INTL("Pointed crystals float in the air around your team!"))
        end
        return 0
      end
    end
    attacker.pbOpposingSide.effects[PBEffects::Decrystals]=true
    #!attacker.pbOpposingSide.effects[PBEffects::Decrystalseed]=attacker.index
    if !@battle.pbIsOpposing?(attacker.index)
      @battle.pbDisplay(_INTL("Pointed crystals float in the air around your foe's team!"))
    else
      @battle.pbDisplay(_INTL("Pointed crystals float in the air around your team!"))
...
# User halves its own HP and creates a decoy.
# Consecutive uses increase power. (Forest Frenzy)
################################################################################
class PokeBattle_Move_914 < PokeBattle_Move
  def pbBaseDamage(basedmg,attacker,opponent)
    basedmg=basedmg<<(attacker.effects[PBEffects::ForestFrenzy]-1) # can be 1 to 4
    return basedmg
  end
...
################################################################################
# Hits 2 times. Power is multiplied by the hit number. (Big Bang Nuclear)
################################################################################
class PokeBattle_Move_915 < PokeBattle_Move
  def pbIsMultiHit
    return true
  end
...
################################################################################
# May decrease the opponent's Defense by 1 stage. May also flinch. (Weakening Jumpscare)
################################################################################
class PokeBattle_Move_917 < PokeBattle_Move
  def pbIsMultiHit
    return true
  end
...
################################################################################
# Increases Atk, Def and Speed by 1 stage,
# then one stage of a random stat. (Cosmic Dance)
################################################################################
class PokeBattle_Move_918 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    if !attacker.pbCanIncreaseStatStage?(PBStats::ATTACK,false) &&
       !attacker.pbCanIncreaseStatStage?(PBStats::DEFENSE,false) &&
       !attacker.pbCanIncreaseStatStage?(PBStats::SPEED,false)
    ...
    showanim=true
    boost_amount=1
    boost_amount=2 if @battle.FE == PBFields::NEWW
    for stat in [PBStats::ATTACK,PBStats::DEFENSE,PBStats::SPEED]
      if attacker.pbCanIncreaseStatStage?(stat,false)
        attacker.pbIncreaseStat(stat,boost_amount,abilitymessage:false)
      end
    ...
    for i in [PBStats::ATTACK,PBStats::DEFENSE,PBStats::SPEED,
              PBStats::SPATK,PBStats::SPDEF,PBStats::ACCURACY,PBStats::EVASION]
      array.push(i) if attacker.pbCanIncreaseStatStage?(i,false)
    end
...
############################################################################################################
# For 5 rounds if drought, increases power of physical & special attacks from the user's side. (Solar Array)
############################################################################################################
class PokeBattle_Move_919 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    if attacker.pbOwnSide.effects[PBEffects::SolarArray]>0 ||
    (@battle.pbWeather!=PBWeather::SUNNYDAY &&
    ![PBFields::RAINBOWF,PBFields::MIRRORA].include?(@battle.FE)) #Temporary
      @battle.pbDisplay(_INTL("But it failed!"))
      return -1
    end
    pbShowAnimation(@id,attacker,opponent,hitnum,alltargets,showanimation)
    if @battle.FE!=PBFields::MIRRORA
      attacker.pbOwnSide.effects[PBEffects::SolarArray]=5
      attacker.pbOwnSide.effects[PBEffects::SolarArray]=8 if attacker.hasWorkingItem(:LIGHTCLAY)
      if !@battle.pbIsOpposing?(attacker.index)
        @battle.pbDisplay(_INTL("A Flare is empowering your team!"))
      else
        @battle.pbDisplay(_INTL("A Flare is empowering the opposing team!"))
      end
    else
      attacker.pbOwnSide.effects[PBEffects::SolarArray]=8
      if !@battle.pbIsOpposing?(attacker.index)
        @battle.pbDisplay(_INTL("A Flare is empowering your team!"))
      else
        @battle.pbDisplay(_INTL("A Flare is empowering the opposing team!"))
      ...
      if attacker.pbCanIncreaseStatStage?(PBStats::ACCURACY,false)
        attacker.pbIncreaseStat(PBStats::ACCURACY,1,abilitymessage:false)
      end
    end
    return 0
  end
