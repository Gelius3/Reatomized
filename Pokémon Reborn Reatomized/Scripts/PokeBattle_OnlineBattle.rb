  ...
  def pbPriority(ignorequickclaw = false,megacalc = false)
    ...
    for i in 0..3
      ...
      if @choices[i][0]==1 # Is a move
        ...
        pri += 1 if @battlers[i].ability == PBAbilities::GALEWINGS && @choices[i][2].type==2 && ((@battlers[i].hp >= (@battlers[i].totalhp*0.75).floor) || ((@battle.FE == PBFields::MOUNTAIN || @battle.FE == PBFields::SNOWYM) && @weather == PBWeather::STRONGWINDS))
  ...
  def pbStartBattleCore(canlose)
    ...
    loop do   # Now begin the battle loop
      ...
      for i in 0..3
        ...
        @battlers[1].moves[i].battle = nil
        @battlers[2].moves[i].battle = nil if @doublebattle
        @battlers[3].moves[i].battle = nil if @doublebattle
      end
