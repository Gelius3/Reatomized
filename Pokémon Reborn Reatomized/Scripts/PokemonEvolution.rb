class PBEvolution
  ...
  EVONAMES=["Unknown",
     ...
     "BadInfluence","Affection","LevelRain","LevelDay","LevelNight",
     "LandCritical","Alcremie","Poisoned","LevelWind","Gimmi",
     "NuclearEvo","AtkGreaterSpe","SpeGreaterAtk","LevelSand"
  ]
  ...
  EVOPARAM=[0,     # Unknown (do not use)
     ...
     1,1,1,1,1,    # LandCritical, Alcremie, Poisoned, LevelWind, Gimmi
     1,1,1,1       # NuclearEvo, AtkGreaterSpe, SpeGreaterAtk, LevelSand
  ]
end
    ...
    if canceled
      ...
      createSpecies=pbCheckEvolutionEx(@pokemon){|pokemon,evonib,level,poke|
        if evonib==14 # Shedinja
          ...
          next -1
        elsif evonib == PBEvolution::Gimmi
          $PokemonBag.pbDeleteItem(PBItems::GIMMIGHOULCOIN,999) if poke==@newspecies
          next -1
        else
          next -1
        end
...
def pbMiniCheckEvolution(pokemon,evonib,level,poke)
  marowakMaps=[152,552]
  golurkMaps=[15,16,17,18,19,20,21,22,23,25,26,27,30,31,32,33,34,35,199,200,201,202,203,204,205,562,563,564,565,566,567,568] #Rebornian Golurk
  ...
  maneonMaps=[15,16,17,18,19,20,21,22,23,25,26,27,30,31,32,33,34,35,199,200,201,202,203,204,205,206,207,208,536,538,547,553,556,558,562,563,564,565,566,567,568,569,574,575,576,577,578,579,586,601,603,604,605]
  accreteonMaps=[305]
  ametrineLucarioMaps=[439] # Ametrine City
  magneticMaps=[197,198,281]
  tanzanMaps=[371,231]
  birdMaps=[333,334,335]
  dozerMaps=[21,27,33,34,35,199,200,201,202,203,204,205]
  slugMaps=[467,468,469,470,471,472,473,474,475,476,477,478,479,480,481,482,483,484,485,486,487,488,489,490,491,492,493,494,495,496,497,498,499,500,501,502,503,504,505]
  ...
  case evonib
    ...
    when PBEvolution::HappinessDay
      if pokemon.species == PBSpecies::RIOLU && ametrineLucarioMaps.include?($game_map.map_id) # Riolu -> Ametrine Lucario
        pokemon.form=4
      end
    ...
    when PBEvolution::Level
	 ...
	 when 446             # Munchlax to Rebornian Snorlax
        ...
        when 223            # Remoraid -> Octillery forms
          if PBDayNight.isDay?(pbGetTimeNow) || remoraidMaps.include?($game_map.map_id)
            pokemon.form=1
          elsif PBDayNight.isNight?(pbGetTimeNow)
            pokemon.form=0
          end
        when 622            # Golett -> Golurk forms
          if golurkMaps.include?($game_map.map_id)
            pokemon.form=1
          else
            pokemon.form=0
          end
        when 109            # Koffing -> Weezing forms
          if koffingMaps.include?($game_map.map_id) && pokemon.form==0
            pokemon.form=1
          end
         when 627            # Braviary Hisuian
          if birdMaps.include?($game_map.map_id)
            pokemon.form=1
          else
            pokemon.form=0
          end
        when 704             # Sliggoo Hisuian
          if slugMaps.include?($game_map.map_id)
            pokemon.form=1
          else
            pokemon.form=0
          end
        when 712             # Avalugg Hisuian
          if dozerMaps.include?($game_map.map_id)
            pokemon.form=2
          else
            pokemon.form=0
          end
        ...
        when 156             # Quilava -> Typhlosion forms
          if pokemon.form==0
            if PBDayNight.isNight?(pbGetTimeNow)
              pokemon.form=1 # Hisuian
            elsif tanzanMaps.include?($game_map.map_id)
              pokemon.form=3 # Rebornian
            else
              pokemon.form=0
            end
          end
        when 502             # Samurott Hisuian
          if PBDayNight.isNight?(pbGetTimeNow)
            pokemon.form=1
          else
            pokemon.form=0
          end
        when 723             # Decidueye Hisuian
          if pokemon.form==0
        ...
        when 848             # Toxel -> Toxtricity Low Key vs Amped Form - Gen 8
          if [PBNatures::HARDY, PBNatures::BRAVE, PBNatures::ADAMANT, PBNatures::NAUGHTY, PBNatures::DOCILE, PBNatures::IMPISH, PBNatures::LAX, PBNatures::HASTY, PBNatures::JOLLY, PBNatures::NAIVE, PBNatures::RASH, PBNatures::SASSY, PBNatures::QUIRKY].include?(pokemon.nature)
            pokemon.form=1
          end
      end
    ...
    when PBEvolution::DefenseGreater # Hitmonchan
      return poke if pokemon.level>=level && pokemon.attack<pokemon.defense
    when PBEvolution::SpeGreaterAtk # Alpico
      return poke if pokemon.level>=level && pokemon.speed>pokemon.attack
    when PBEvolution::AtkGreaterSpe # Anderind
      return poke if pokemon.level>=level && pokemon.speed<pokemon.attack
    ...
    when PBEvolution::Location
      if pokemon.species==PBSpecies::CRABRAWLER
        return poke if crabominableMaps.include?($game_map.map_id)
      elsif pokemon.species==PBSpecies::EEVEE
        if poke==PBSpecies::MANEON
          return poke if maneonMaps.include?($game_map.map_id)
        elsif poke==PBSpecies::ACCRETEON
          return poke if accreteonMaps.include?($game_map.map_id)
        else #Leafeon and Glaceon
    ...
    when PBEvolution::NuclearEvo
      for i in $Trainer.party
        return poke if !i.egg? && (i.type1==19 || i.type2==19) && pokemon.level>=level
      end
    ...
    when PBEvolution::LevelWind
      return poke if pokemon.level>=level && $game_screen.weather_type == 6
    when PBEvolution::LevelSand
      return poke if pokemon.level>=level && $game_screen.weather_type == 4
    when PBEvolution::LandCritical
      return poke if pokemon.landCritical>=level && pokemon.form == 1
    when PBEvolution::Gimmi
      return poke if $PokemonBag.pbQuantity(PBItems::GIMMIGHOULCOIN) >= 999 && pokemon.level >=level
  end
  return -1
end
...
def pbMiniCheckEvolutionItem(pokemon,evonib,level,poke,item)
  ...
  when PBEvolution::Item
    ...
    when 166            # Ledyba -> Ledian forms
      if rebornledianMaps.include?($game_map.map_id)
        pokemon.form=1
      else
        pokemon.form=0
      end
    ...
    when 891            # Kubfu - Gen 8
      if level == PBItems::PARCHMENTY
        pokemon.form=1
      else 
        pokemon.form=0
      end
    when 217            # BloodMoon/Ursaluna
      if  PBDayNight.isNight?(pbGetTimeNow) && moonMaps.include?($game_map.map_id)
        if pokemon.form==0
          pokemon.form=2
        else
          pokemon.form=3
  ...
  when PBEvolution::TradeItem || PBEvolution::Trade
    return poke if level==item
  when PBEvolution::Alcremie
    return poke if pokemon.item == PBItems::STRAWBIC && level==item
  end
  return -1
end
