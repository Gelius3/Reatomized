...
def pbDayCareGetCompat
  if pbDayCareDeposited==2
    ...
    if ((compat10==compat20 && compat10!=nil) || (compat11==compat20 && compat11!=nil) ||
       (compat10==compat21 && compat10!=nil) || (compat11==compat21 && compat11!=nil) ||
...
def pbDayCareWithdraw(index)
  if !$PokemonGlobal.daycare[index][0]
    ...
    pbAddPokemonSilent($PokemonGlobal.daycare[index][0],nil,true,$PokemonGlobal.daycare[index][0].timeReceived)
...
def pbGetNonIncenseLowestSpecies(baby)
  ...
  elsif (baby == PBSpecies::CALFPINT) && hasConst?(PBSpecies,:TAUROS)
    return PBSpecies::TAUROS
...
def pbDayCareGenerateEgg
 ...
 #   raise _INTL("Can't store the egg")
  ...
  if (babyspecies == PBSpecies::NIDORANfE) && hasConst?(PBSpecies,:NIDORANmA)
    ...
    babyspecies=pbGetNonIncenseLowestSpecies(babyspecies)
  elsif (babyspecies == PBSpecies::CALFPINT) && !(mother.item == PBItems::LAXINCENSE) && !(father.item == PBItems::LAXINCENSE)
    babyspecies=pbGetNonIncenseLowestSpecies(babyspecies)
    ...
    babyspecies=pbGetNonIncenseLowestSpecies(babyspecies)
  elsif (babyspecies == PBSpecies::AZURILL) && !(mother.item == (PBItems::SEAINCENSE || PBItems::ROCKINCENSE)) && !(father.item == (PBItems::SEAINCENSE || PBItems::ROCKINCENSE))
    babyspecies=pbGetNonIncenseLowestSpecies(babyspecies)
  ...
  egg.form = mainparent.form unless egg.species == PBSpecies::ROTOM || egg.species == PBSpecies::TOXEL # New form inheriting
    if egg.species == PBSpecies::TAUROS
      if mainparent.item == PBItems::BLACKBELT
        egg.form = 1 
      elsif mainparent.item == PBItems::CHARCOAL
        egg.form = 2
      elsif mainparent.item == PBItems::MYSTICWATER
        egg.form = 3 
      else 
...
# Destiny Knot and Inheriting IVs
  ...
  shinyretries+=2 if hasConst?(PBItems,:SHINYCHARM) && $PokemonBag.pbQuantity(:SHINYCHARM)>0
  shinyretries+=1 if hasConst?(PBItems,:SHINYCHARMP) && $PokemonBag.pbQuantity(:SHINYCHARMP)>0
  shinyretries+=2 if hasConst?(PBItems,:SHINYCHARMPP) && $PokemonBag.pbQuantity(:SHINYCHARMPP)>0
  shinyretries+=50 if hasConst?(PBItems,:DEVCHARM) && $PokemonBag.pbQuantity(:DEVCHARM)>0
  if shinyretries>0
