...
class PokemonGlobalMetadata
  ...
  PASSWORD_HASH = {
    ...
    "wtfisafont" => 2036,
    "eeveeplease" => 1366, "eevee" => 1366, "bestgamemode" => 1366, "eeveepls" => 1366,
    "pikaplease" => 2212, "ashpika" => 2212, "pikachupls" => 2212, "pikapls" => 2212, 
    "vulpixpls" => 2147, "vulpixplease" => 2147,
    "earlyphione" => 2213, "phioneplease" => 2213,
    "justvulpix" => :Just_Vulpix,
    "digimon" => 2302, "butterfly" => 2302, "braveheart" => 2302, "botamon" => 2302,
    "dratiniearly" => 2138,
    "aevianmissy" => 2175,
  ...
  def addPassword(entrytext)
    ...
    $game_switches[1060] = $game_switches[2036] == true ? true : false
  end
  ...
  def pbTogglePassword(password, isGameStart=false)
    nodisable =["fullivs","31ivs","allivs","mischievous"]
    password_string=password.downcase()
    if !isGameStart && nodisable.include?(password_string) && checkPasswordActivation(password_string)
  ...
  def pbMonoRandEvents
    ...
    mixtrain=[]
    iegg=[]
    variablearray=[50,228,229,231,351,352,353,354,355,803,356,357,913] #Order of above, lists variable/switch to save to
    ...
    eventarray.push(mixpokemon,mixegg,mixonyx,dollevent,mixsnufful,mixturtmor,mixslums,mixmalchous,mixtrade,actuallypanpour,mixperidot,mixtrain,iegg)
  ...
  def pbChallengerDefense(event)
    potentialTrainers = [3,14,25,11,32,18,15,27,30,24,26,19,35]  # Shelly Adrienn Fern, Charlotte, Lumi, Saphira, Titania, Bennett, Cal, Cain, Victoria, Heather, Lin
    loop do
      ...
      case $game_variables[601]
        ...
        when "Heather"
          ...
          valid =  $game_switches[1325]
        when "Lin"
          $game_variables[602] = 150
          $game_variables[604] = "hey. i wanna do this one legit. i sorta cheated last time. promise i won't do that here."
          $game_variables[603] = "poo."
          $game_variables[608] = "okayyy i guess i'll go, but if you ever get lonely up here, you know who to call! it can just be like old times~"
          valid =  $game_switches[2011]
      end
      break if valid == true 
    end 
  ...
  def rebornCheckRemoteVersion()
    ...
    begin
      Timeout::timeout(12) do
            ...
            socket = TCPSocket.open(host,port,connect_timeout: 5)  # Connect to server
