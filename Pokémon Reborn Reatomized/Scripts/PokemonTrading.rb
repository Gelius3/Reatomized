class PokemonTradeScene
  ...
  def pbScene1
    ...
    pictureBall.moveName(picturePoke.totalDuration+2,sprintf("Graphics/Battle/Pictures/ball%02d",@pokemon.ballused))
  ...
  def pbTrade
    ...
    pbPlayDecisionSE()
    pbScene1
    if $game_switches[:Egg_Trade] == true
        if $game_switches[:Egg_Trade_2] == true
          Kernel.pbMessageDisplay(@sprites["msgwindow"],
...
def pbStartTrade(pokemonIndex,newpoke,nickname,trainerName,trainerGender=0,online=false)
  ...
  if $game_switches[:Egg_Trade] && !online
    ...
    yourPokemon.otgender=$Trainer.gender
  ...
  if $game_switches[:Egg_Trade] == true
    # Get egg steps
    yourPokemon.eggsteps=$cache.pkmn_dex[yourPokemon.species][:EggSteps]
  else
  ...
  yourPokemon.item=myPokemon.item if !online && yourPokemon.item==0
  ...
  ret=pbCheckEvolutionEx(pokemon){|pokemon,evonib,level,poke|
    case evonib
      when PBEvolution::Trade
        unless (pokemon.species==PBSpecies::SHELMET && !$Trainer.party.any? {|mon| mon.species==PBSpecies::KARRABLAST} ||
          pokemon.species==PBSpecies::KARRABLAST && !$Trainer.party.any? {|mon| mon.species==PBSpecies::SHELMET}) && pokemon2==PBItems::LINKSTONE
          next poke
        end
      when PBEvolution::TradeItem
        if pokemon.item==level && !tradestonecheck
          pokemon.setItem(0)
          case pokemon.species
          when 125 # Electabuzz -> Electivire Forms
            titaniumMaps=[197,198,281] # Shade's Gym
            if titaniumMaps.include?($game_map.map_id)
              pokemon.form=1
            end
          when 361 # Snorunt -> Glalie Forms
            pokemon.form=4
        end
          next poke
        elsif pokemon.item==level
          next poke
        end
      when PBEvolution::TradeSpecies
        next poke if (pokemon2!=520 && pokemon2.species==level && !isConst?(pokemon2.item,PBItems,:EVERSTONE))
    end
    next -1
  }
  return ret
