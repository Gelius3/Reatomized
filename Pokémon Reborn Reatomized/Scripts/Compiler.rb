...
def pbAddScript(script,sectionname)
  case GAMETITLE
    when "Pokemon Reborn","Pokemon Reborn Reatomized" then subfolder = "Reborn/"
  ...
  i=0; loop do break unless i<lines.length
    ...
    for j in 0...numpoke
      ...
      if !poke[TPABILITY] || poke[TPABILITY]==""
        ...
        raise _INTL("Bad abilityflag: {1} (must be 0 or 1 or 2-5)\n{2}",poke[TPABILITY],FileLineData.linereport) if poke[TPABILITY]<0 || poke[TPABILITY]>999
      end
      # Gender
...
def pbCompileFields
	fields = []
	for i in 0...(FIELDEFFECTS.length)
