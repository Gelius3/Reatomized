...
################################################################################
  def initialize(battle,battler,move,crystal,simplechoice=false)
    ...
    if user.hasWorkingAbility(:MOLDBREAKER) || user.hasWorkingAbility(:TERAVOLT) || user.hasWorkingAbility(:TURBOBLAZE) || (user.hasWorkingAbility(:MYCELIUMMIGHT) && @basedamage == 0) || # Gen 9
    ...
    stellarprotype=@type
    if battler.ability == PBAbilities::STELLAREAN
      prot1 = battler.type1
      prot2 = battler.type2
      if !battler.pbHasType?(stellarprotype) || (defined?(prot2) && prot1 != prot2)
        battler.type2=stellarprotype
        stellartypename=PBTypes.getName(stellarprotype)
        @battle.pbDisplay(_INTL("{1} had its type changed to {3}!",battler.pbThis,PBAbilities.getName(battler.ability),stellartypename))
      end
    end
    if isConst?(battler.ability, PBAbilities, :STANCECHANGE) || isConst?(battler.ability, PBAbilities, :ASTRALRECKON)
    ...
    if ztargets.length==0
      if @thismove.target==PBTargets::SingleNonUser ||
        ...
        zchoice[2].name = @name
        battler.pbUseMove(zchoice, {zmove: true})
...
################################################################################
  def pbModifyDamage(damagemult,attacker,opponent)
    if !opponent.effects[PBEffects::ProtectNegation] && (opponent.pbOwnSide.effects[PBEffects::MatBlock] ||
      opponent.effects[PBEffects::Protect] || opponent.effects[PBEffects::KingsShield] || opponent.effects[PBEffects::SilkTrap] ||
  ...
  def pbZStatus(move,attacker)
    z_effect_hash = pbHashConverter(PBMoves,{
      [PBStats::ATTACK,1] => [:BULKUP,:HONECLAWS,:HOWL,:LASERFOCUS,:LEER,:MEDITATE,:ODORSLEUTH,:POWERTRICK,:ROTOTILLER,:SCREECH,:SHARPEN,
        :TAILWHIP, :TAUNT,:TOPSYTURVY,:WILLOWISP,:WORKUP,:POWERSHIFT],
      ...
      [PBStats::DEFENSE,2] =>   [:DECRYSTALLIZATION],
      ...
      [PBStats::SPDEF,1] => [:CHARGE,:CONFIDE,:COSMICPOWER,:CRAFTYSHIELD,:EERIEIMPULSE,:ENTRAINMENT,:FLATTER,:GLARE,:INGRAIN,
        :LIGHTSCREEN,:MAGICROOM,:MAGNETICFLUX,:MEANLOOK,:MISTYTERRAIN,:MUDSPORT,:SPOTLIGHT,:STUNSPORE,:THUNDERWAVE,:DOODLE,
        :WATERSPORT,:WHIRLWIND,:WISH,:WONDERROOM],
      [PBStats::SPDEF,2] => [:AROMATICMIST,:CAPTIVATE,:IMPRISON,:MAGICCOAT,:POWDER,:STUFFCHEEKS],
      [PBStats::SPEED,1] => [:AFTERYOU,:AURORAVEIL,:SOLARARRAY,:CORROSIVEGAS,:ELECTRICTERRAIN,:ENCORE,:GASTROACID,:GRASSWHISTLE,:GUARDSPLIT,:GUARDSWAP,
        :HAIL,:HYPNOSIS,:LOCKON,:LOVELYKISS,:POWERSPLIT,:POWERSWAP,:QUASH,:RAINDANCE,:ROLEPLAY,:SAFEGUARD,:TARSHOT,
        :SANDSTORM,:SCARYFACE,:SING,:SKILLSWAP,:SLEEPPOWDER,:SPEEDSWAP,:STICKYWEB,:STRINGSHOT,:SUNNYDAY,:SNOWSCAPE,
      ...
      [:crit1]  => [:ACUPRESSURE,:FORESIGHT,:HEARTSWAP,:SLEEPTALK,:TAILWIND,:COACHING],
      [:reset]  => [:ACIDARMOR,:AGILITY,:AMNESIA,:ATTRACT,:AUTOTOMIZE,:BARRIER,:BATONPASS,:CALMMIND,:COIL,:COTTONGUARD,
        ...
        :SHIFTGEAR,:SHOREUP,:SHELLSMASH,:SHIFTGEAR,:SHOREUP,:SLACKOFF,:SOFTBOILED,:SPORE,:SUBSTITUTE,:SWAGGER,:TEATIME,
        :SWALLOW,:SWORDSDANCE,:SYNTHESIS,:TAILGLOW,:COURTCHANGE,:JUNGLEHEALING,:LIFEDEW,:LUNARBLESSING,
      :NORETREAT,:OBSTRUCT,:SHELTER,:TAKEHEART,:VICTORYDANCE,:SILKTRAP,:SPICYEXTRACT,:TIDYUP],
      [:heal]   => [:AROMATHERAPY,:BELLYDRUM,(:CONVERSION2),:CLANGOROUSSOUL,:HAZE,:HEALBELL,:MIST,:PSYCHUP,:REFRESH,:SPITE,:STOCKPILE,
        :TELEPORT,:TRANSFORM,:FILLETAWAY,:SHEDTAIL],
