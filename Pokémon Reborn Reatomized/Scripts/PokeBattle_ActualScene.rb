...
class FightMenuButtons < BitmapSprite
  ...
  def pbFieldNotesBattle(move)
    ...
    return 1 if move.flags&0x400!=0 && battle.FE == PBFields::BIGTOPA
    ...
    when PBFields::DARKCRYSTALC
      moveBoost = 0 if (move.id == PBMoves::SOLARBEAM || move.id == PBMoves::SOLARBLADE) && !self.ability == PBAbilities::HEATGENERATOR
...
class PokemonDataBox < SpriteWrapper
  ...
  def refresh (loopstop=false)
    ...
    if @battler.pokemon.isPulse?
      imagepos.push(["Graphics/Pictures/Battle/battlePulseEvoBox.png",@spritebaseX+megaX-4,@spritebaseY+megaY+2,0,0,-1,-1])
    elsif @battler.pokemon.isGmax?
      imagepos.push(["Graphics/Pictures/Battle/battleGmaxEvoBox.png",@spritebaseX+megaX-10,@spritebaseY+megaY,0,0,-1,-1])
    elsif @battler.isMega? && @battler.effects[PBEffects::Illusion] == nil
      ...
      imagepos.push(["Graphics/Pictures/Battle/battleMegaEvoBox.png",@spritebaseX+megaX,@spritebaseY+megaY,0,0,-1,-1])
    elsif @battler.pokemon.isDelta?
      imagepos.push(["Graphics/Pictures/Battle/battleDeltaEvoBox.png",@spritebaseX+megaX-8,@spritebaseY+megaY,0,0,-1,-1])
    elsif @battler.pokemon.isCelestial?
      imagepos.push(["Graphics/Pictures/Battle/battleCelestialEvoBox.png",@spritebaseX+megaX-8,@spritebaseY+megaY,0,0,-1,-1])
    elsif @battler.pokemon.isCosmic?
      imagepos.push(["Graphics/Pictures/Battle/battleCosmicEvoBox.png",@spritebaseX+megaX-8,@spritebaseY+megaY,0,0,-1,-1])
    end
...
# The return value is the item chosen, or 0 if the choice was canceled.
  def pbItemMenu(index)
    ...
    loop do
      ...
      if cmdUse>=0 && command==cmdUse
        ...
        #if (($game_switches[:No_Items_Password]==true) || opponent.ability == PBAbilities::ANCIENTPRESSURE) && (usetype==1 || usetype==3 || $cache.items[item][ITEMPOCKET] == 7)
        #  if ($game_switches[:No_Items_Password]==true)
        #    Kernel.pbMessage("The 'No Items' password is on, so items can't be used in battle.")
        #  elsif (opponent.ability == PBAbilities::ANCIENTPRESSURE) 
        #    Kernel.pbMessage("The opponent's ability forbids you from using items.")
        #  end
  ...
  def pbCanUseBattleItem(pkmnid, item, pkmnscreen)
    ...
    if battler && (battler.effects[PBEffects::SkyDrop] || battler.effects[PBEffects::Commander])
      return false
    end
    if battler && (battler.effects[PBEffects::Embargo]>0 || battler.underAncientPressure?)
      return false
    end
...
# This method can change the battle's music for example.
  ...
  def pbChangePokemon(attacker,pokemon)
    return if attacker.effects[PBEffects::Substitute]>0
    pkmn=@sprites["pokemon#{attacker.index}"]
    shadow=@sprites["shadow#{attacker.index}"]
    back=!@battle.pbIsOpposing?(attacker.index)
  ...
  def pbAnimation(moveid,user,target,hitnum=0)
    ...
    pbSaveShadows {
       if animid[1] # On opposing side and using OppMove animation
         ...
         pbAnimationCore($cache.animations[anim],user,target,autoopp)
       end
    }
  ...
  def pbChooseRevivalBlessingRecipient(index)
    party=@battle.pbParty(index)
    partypos=@battle.partyorder
    ret=nil
    pbShowWindow(BLANK)
