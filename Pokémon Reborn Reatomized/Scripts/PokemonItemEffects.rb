...
ItemHandlers::UseOnPokemon.copy(:FIRESTONE,
   ...
   :GALARICAWREATH,:GALARICACUFF,:PARCHMENTX,:PARCHMENTY,
   :BLACKAUGURITE,:PEATBLOCK,:ALLOYCANDY,:METALALLOY,
   :LEADERSCREST,:SYRUPYAPPLE,:UNREMARKABLETEACUP,:MASTERPIECETEACUP,
   :COCONUTMILK,:AROMATICHERB,:ROYALJELLY,:CARROTWINE,
   :ASTRALSTONE,:VARIASTONE)

ItemHandlers::UseOnPokemon.add(:HANDMIXER,proc{|item,pokemon,scene|
  if (pokemon.isShadow? rescue false)
    scene.pbDisplay(_INTL("It won't have any effect."))
    next false
  end
  if pokemon.species == PBSpecies::MILCERY && pokemon.item == PBItems::STRAWBIC
    newspecies=pbCheckEvolution(pokemon,item)
    if newspecies<=0
      scene.pbDisplay(_INTL("It won't have any effect."))
      next false
    else
      scene.pbDisplay(_INTL("{1} was whipped up!",pokemon.name))
      pbFadeOutInWithMusic(99999){
        evo=PokemonEvolutionScene.new
        evo.pbStartScreen(pokemon,newspecies)
        evo.pbEvolution(false)
...
ItemHandlers::UseOnPokemon.add(:ABILITYCAPSULE,proc{|item,pokemon,scene|
  ...
  for i in abillist.keys
    ...
    commands.push((i < 8 ? "" : "(H) ") + PBAbilities.getName(abillist[i]))
...
ItemHandlers::UseOnPokemon.add(:REVEALGLASS,proc{|item,pokemon,scene|
   if ((pokemon.species == PBSpecies::TORNADUS) ||
      ...
      (pokemon.species == PBSpecies::ENAMORUS)) && pokemon.hp>=0
...
ItemHandlers::UseOnPokemon.add(:REINSOFUNITY,proc{|item,pokemon,scene|
   if (pokemon.species == PBSpecies::CALYREX) && pokemon.hp>=0
     if pokemon.fused!=nil
       if $Trainer.party.length>=6
         scene.pbDisplay(_INTL("Your party is full! You can't unfuse {1}.",pokemon.name))
         next false
       ...
       if chosen>=0
         ...
         if (isConst?(poke2.species,PBSpecies,:GLASTRIER) ||
            isConst?(poke2.species,PBSpecies,:SPECTRIER)) && poke2.hp>=0
...
ItemHandlers::UseOnPokemon.add(:PARTNERRIBBON,proc{|item,pokemon,scene|
  if ((pokemon.species == PBSpecies::PIKACHU && [0,1].include?(pokemon.form)) || (pokemon.species == PBSpecies::EEVEE && pokemon.form==0)) && pokemon.happiness>=220
    pokemon.form=2
    scene.pbRefresh
    scene.pbDisplay(_INTL("{1} accepted the Ribbon!",pokemon.name))
    next true
  else
    scene.pbDisplay(_INTL("It had no effect."))
    next false
...
ItemHandlers::UseOnPokemon.add(:GALARIANFEATHER,proc{|item,pokemon,scene|
  if [PBSpecies::ARTICUNO,PBSpecies::ZAPDOS,PBSpecies::MOLTRES, PBSpecies::THUFIZER].include?(pokemon.species) && pokemon.form==0 && pokemon.hp>=0
    ...
    pokemon.resetMoves
    scene.pbRefresh
    scene.pbDisplay(_INTL("{1} returned to its Original forme!",pokemon.name))
    next true
  else
    scene.pbDisplay(_INTL("It had no effect."))
    next false
...
ItemHandlers::UseOnPokemon.add(:ENERGYCORE,proc{|item,pokemon,scene|
  ...
  if formnames != nil
    ...
    if nuclearFormNumber != nil && pokemon.form != nuclearFormNumber
      ...
      scene.pbRefresh
      scene.pbDisplay(_INTL("{1} was infused with nuclear energy!",pokemon.name))
      next true
    end
  end
  scene.pbDisplay(_INTL("It had no effect."))
