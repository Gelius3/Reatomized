class RandomizerSettings
  ...
  def initialize
    ...
    @selfmon = false
    @oppmon = false
    @monexclusions = [PBSpecies::ARCEUS] + [0] + (1004..PBSpecies.maxValue).to_a
...
DummyMoves = [
    ...
    PBMoves::MULTIATTACKROCK,PBMoves::MULTIATTACKSTEEL,PBMoves::MULTIATTACKWATER,PBMoves::MULTIATTACKNUKE,
    ...
    PBMoves::JUDGMENTROCK,PBMoves::JUDGMENTSTEEL,PBMoves::JUDGMENTWATER,PBMoves::JUDGMENTNUKE
]
...
=end 
  ...
  def randomizeTypeChart(exclusions=[])
    chart = Array.new(20) {|index| Array.new(20)}
  ...
  def buildRandomizer
    ...
    if @settings.selfabil || @settings.oppabil
      ...
      if !@settings.abilchaos
        for mon in 1..1004 #hard coded to avoid gen 8
...
def pbLoadTrainer(trainerid,trainername,partyid=0)
  begin
    ...
    for poke in trainer[0]
      ...
      if defined?(SplitTrainers)
        if SplitTrainers.species == false
...
def pbCheckEvolutionEx(pokemon)
  ...
  return -1 if pokemon.species == PBSpecies::FARFETCHD && pokemon.form == 0
  return -1 if pokemon.species == PBSpecies::EEVEE && pokemon.form == 2
  ret=-1
