...
################################################################################
  ...
  def pbEffectsOnDealingDamage(move,user,target,damage,innards)
    ...
    if damage>0
      ...
      # Thrust
      if user.hasWorkingAbility(:THRUST) && @battle.pbRandom(10)<2 && !target.damagestate.substitute && !user.isFainted? && !target.isFainted?
        choices = []
...
################################################################################
  ...
  def pbSuccessCheck(thismove,user,target,flags,accuracy=true)
    ...
    if target.hasWorkingAbility(:PARRY) && thismove.isContactMove? && !(user.hasWorkingAbility(:LONGREACH)) && !(user.hasWorkingItem(:PROTECTIVEPADS)) && !(thismove.basedamage==0) && !thismove.zmove && @battle.pbRandom(10)<3
      ...
      #@battle.pbDisplay(_INTL("{1}",parrydamage)) #for easy access to see the damage being dealt
      user.hp-=parrydamage
      return false
  ...
  def pbTryUseMove(choice,thismove,flags={passedtrying: false, instructed: false})
    ...
    if @effects[PBEffects::Flinch]
      @effects[PBEffects::Flinch]=false
      @effects[PBEffects::Tenacity]=false if @effects[PBEffects::Tenacity]==true
