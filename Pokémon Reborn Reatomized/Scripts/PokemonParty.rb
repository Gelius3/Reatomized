...
class PokemonScreen_Scene
  ...
  def pbChoosePokemon(switching=false,allow_party_switch=false,canswitch=0)
    ...
    loop do
      ...
      if allow_party_switch && @activecmd >= 6 && (Input.trigger?(Input::C) || Input.trigger?(Input::X))
        return -1
      end
...
class PokemonScreen
  ...
  def pbGiveMail(item,pkmn,pkmnid=0)
    ...
    if thisitemname == "Blue Orb" && pkmn.species != PBSpecies::KYOGRE
      pbDisplay(_INTL("The Blue Orb can't be held!"))
      return false
    end
    if thisitemname == "Red Orb" && pkmn.species != PBSpecies::GROUDON
  ...
  def pbPokemonDebug(pkmn,pkmnid)
    ...
    loop do
      ...
      case command
        ...
        ### Mark ###
        when 20
          if pkmn.mark!=false || pkmn.mark!=nil
            pbDisplay(_INTL("This Pokèmon has marker number {1}!",pkmn.mark))
          end
          params=ChooseNumberParams.new
          params.setRange(0,10)
          params.setDefaultValue(0)
          h=Kernel.pbMessageChooseNumber(
            _INTL("Choose the new Marker's ID (max. 10)."),params) { @scene.update }
            if h!=pkmn.mark
              pkmn.mark=h
              pbDisplay(_INTL("{1}'s Marker has been set to ID {2}.",pkmn.name,pkmn.mark))
              pbRefreshSingle(pkmnid)
            end
