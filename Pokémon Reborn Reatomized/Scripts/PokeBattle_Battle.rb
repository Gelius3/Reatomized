  ...
  def pbThrowPokeBall(idxPokemon,ball,rareness=nil,showplayer=false)
    ...
    if @opponent && (!pbIsSnagBall?(ball) || !battler.isShadow?)
      ...
      if $DEBUG && Input.press?(Input::CTRL)
        ...
        rareness +=12 if Reborn && $PokemonBag.pbQuantity(PBItems::DEVCHARM)>0
        ...
        c=0
        if $Trainer
          mod = -3 #? Proposed change: -2
          mod +=0.5 if $Trainer.pokedexOwned>1000 #! Proposed change: +=1
          mod +=0.5 if $Trainer.pokedexOwned>900 #! Proposed change: +=1
          mod +=0.5 if $Trainer.pokedexOwned>800 #* Proposed change: removed
          mod +=0.5 if $Trainer.pokedexOwned>650
          mod +=0.5 if $Trainer.pokedexOwned>500 #! Proposed change: +=1
          mod +=0.5 if $Trainer.pokedexOwned>400
          mod +=0.5 if $Trainer.pokedexOwned>300
          mod +=0.5 if $Trainer.pokedexOwned>200 #* Proposed change: removed
          mod +=0.5 if $Trainer.pokedexOwned>100 #! Proposed change: +=1
...
################################################################################
  ...
  def initialize(scene,p1,p2,player,opponent,recorded=false)
    ...
    @rageFist        = [Array.new(p1.length, 0), Array.new(p2.length, 0)] # Rage Fist
...
################################################################################
  ...
  def seedCheck
    for battler in pbPriority
      ...
      boostlevel = ["","","sharply ", "drastically ", "exponentially "]

      # Stat boost from seed
      statupanimplayed=false
      statdownanimplayed=false
      seeddata[:stats].each_pair {|stat,statval|
        statval *= -1 if battler.ability == PBAbilities::CONTRARY || battler.ability == PBAbilities::ALOLANTECHNIQUESB
      ...
      case @field.effect
        ...
        when PBFields::WASTELAND
          battler.pbDisposeItem(false)
          battler.pbOwnSide.effects[PBEffects::Decrystals]=true
          battler.pbOpposingSide.effects[PBEffects::Decrystals]=true
          pbDisplay(_INTL("{1} laid Stealth Rocks everywhere!", battler.pbThis))
          return

        when PBFields::FALLOUT # Uranium
          battler.pbDisposeItem(false)
  ...
  def pbIsUnlosableItem(pkmn,item)
    ...
    if (pkmn.species == PBSpecies::ARCEUS)
      if [PBItems::FISTPLATE,   PBItems::SKYPLATE,    PBItems::TOXICPLATE,  PBItems::EARTHPLATE,  PBItems::STONEPLATE,
          ...
          PBItems::PIXIEPLATE,  PBItems::DREADPLATE,  PBItems::ATOMICPLATE, PBItems::COSMICPLATE].include?(item)
        return true
      end
    end
    if (pkmn.species == PBSpecies::SILVALLY)
      if [PBItems::FIGHTINGMEMORY,  PBItems::FLYINGMEMORY,    PBItems::POISONMEMORY,  PBItems::GROUNDMEMORY,  PBItems::ROCKMEMORY,
          ...
          PBItems::FAIRYMEMORY,     PBItems::DARKMEMORY,      PBItems::ATOMICMEMORY,  PBItems::COSMICMEMORY,  PBItems::QMARKSMEMORY].include?(item)
        return true
      end
    end
    ...
    return true if (pkmn.species == PBSpecies::DIALGA) && (item == PBItems::ADAMANTCRYSTAL)
    return true if (pkmn.species == PBSpecies::PALKIA) && (item == PBItems::LUSTROUSGLOBE)
    return true if (pkmn.species == PBSpecies::ZACIAN) && (item == PBItems::RUSTEDSWORD)
    return true if (pkmn.species == PBSpecies::ZAMAZENTA) && (item == PBItems::RUSTEDSHIELD)
    return true if (pkmn.ability == PBAbilities::PROTOSYNTHESIS || pkmn.ability == PBAbilities::QUARKDRIVE) && item == PBItems::BOOSTERENERGY
    return false
  end
    ...
    case trainer.trainertype
      ...
      when PBTrainers::LIN
        case $game_variables[:Battle_Text_of_Opponent]
          ...
          when 4 then ace_text = _INTL("Two stand in the way of doom... This means nothing.")
          when 5 then ace_text = _INTL("Two stand in the way of doom... The end is meaningless.")
        end
...
################################################################################
  ...
  def pbCanChooseMove?(idxPokemon,idxMove,showMessages,flags={sleeptalk: false, instructed: false, zmove: false})
    ...
    if thispkmn.effects[PBEffects::ChoiceBand]>=0 && ((thispkmn.itemWorks? && [PBItems::CHOICEBAND, PBItems::CHOICESPECS, PBItems::CHOICESCARF].include?(thispkmn.item)) || thispkmn.ability == PBAbilities::GORILLATACTICS)
      if thispkmn.moves.any? {|moveloop| moveloop.id==thispkmn.effects[PBEffects::ChoiceBand]} && (thismove.id!=thispkmn.effects[PBEffects::ChoiceBand] && sleeptalk == false)
        if showMessages
          if thispkmn.ability == PBAbilities::GORILLATACTICS
            thing = PBAbilities.getName(thispkmn.ability)
          else
            thing = PBItems.getName(thispkmn.item)
          end
          pbDisplayPaused(_INTL("{1} allows the use of only {2}!",
             thing, PBMoves.getName(thispkmn.effects[PBEffects::ChoiceBand])))
        end
        return false
      end
    end
    ...
    if (thispkmn.effects[PBEffects::GigatonHammer]>0 && thismove.id == PBMoves::GIGATONHAMMER) || (thispkmn.effects[PBEffects::BloodMoon]>0 && thismove.id == PBMoves::BLOODMOON)
      if showMessages
        pbDisplayPaused(_INTL("{1} can't use {2} twice in a row!",thispkmn.pbThis,thismove.name))
      end
      return false
    end
    return true
  ...
  def pbPriority(ignorequickclaw = true,megacalc = false)
    ...
    for i in 0..3
      ...
      if @choices[i][0] == 1 # Is a move
        ...
        pri += 1 if @field.effect == PBFields::GRASSYT && @choices[i][2].function == 0x205 # Grassy Glide
        pri += 1 if @battlers[i].ability == PBAbilities::GALEWINGS && @choices[i][2].type==PBTypes::FLYING && ((@battlers[i].hp >= (@battlers[i].totalhp*0.75).floor) || ((@field.effect == PBFields::MOUNTAIN || @field.effect == PBFields::SNOWYM) && @weather == PBWeather::STRONGWINDS))
      ...
      if !ignorequickclaw && @choices[i][0] == 1 # Is a move
        priorityarray[i][1] = 1 if @battlers[i].custap || (@battlers[i].itemWorks? && @battlers[i].item == PBItems::QUICKCLAW && (pbRandom(100)<20)) || (@battlers[i].ability == PBAbilities::QUICKDRAW && (pbRandom(100)<30)) # Gen 8
      end
    ...
    for i in 0..3
      ...
      elsif (@battlers[i].ability == PBAbilities::QUICKDRAW)
        pbDisplayBrief(_INTL("{1}'s Quick Draw let it move first!",@priority[i].pbThis)) if priorityarray[i][1] == 1 && !ignorequickclaw
      end
    end
...
################################################################################
  def pbCanSwitchLax?(idxPokemon,pkmnidxTo,showMessages)
    if pkmnidxTo>=0
      ...
      if @battlers[idxPokemon].effects[PBEffects::Commandee]
  ...
  def pbCanSwitch?(idxPokemon,pkmnidxTo,showMessages,ai_phase=false,running: false)
    ...
    if thispkmn.effects[PBEffects::MultiTurn]>0 || thispkmn.effects[PBEffects::MeanLook]>=0 || thispkmn.effects[PBEffects::Octolock]>=0 || @state.effects[PBEffects::FairyLock]==1
      pbDisplayPaused(_INTL("{1} can't be switched out!",thispkmn.pbThis)) if showMessages
      return false
    end
    # Ingrain
  ...
  def pbMessagesOnReplace(index,newpoke)
    ...
    obtainedmarker = pbGetMarker(index,newpoke)
    markdescription = nil
    case obtainedmarker
    when 3
      markdescription="Unrivaled"
    when 9
      markdescription="Chosen One"
    when 10
      markdescription = "Reliable Partner"
    end
    if (markdescription == nil)
      markdescription = ""
    end
    if pbOwnedByPlayer?(index)
      ...
      if opposing.hp<=0 || opposing.hp==opposing.totalhp
        ...
        #pbDisplayBrief(_INTL("Go! {1} the {2}!",newname,markdescription))
      elsif opposing.hp>=(opposing.totalhp/2.0)
        pbDisplayBrief(_INTL("Do it! {1}!",newname))
        #pbDisplayBrief(_INTL("Do it! {1} the {2}!",newname,markdescription))
      elsif opposing.hp>=(opposing.totalhp/4.0)
        pbDisplayBrief(_INTL("Go for it, {1}!",newname))
        #pbDisplayBrief(_INTL("Go for it, {1} the {2}!",newname,markdescription))
      else
        pbDisplayBrief(_INTL("Your foe's weak!\nGet 'em, {1}!",newname))
        #pbDisplayBrief(_INTL("Your foe's weak!\nGet 'em, {1} the {2}!",newname,markdescription))
      end
    else
  ...
  def pbGetMarker(index,newpoke) # +Illusion
    partynumber = pbParty(index)
    party=pbPartySingleOwner(index)
    gottenmarker=nil
    if (partynumber[newpoke].ability == PBAbilities::ILLUSION)
    ...
    if pbIsOpposing?(index)
      gottenmarker = illusionpoke != nil ? illusionpoke.name : PBSpecies.getName(partynumber[newpoke].species)
    else
      gottenmarker = illusionpoke != nil ? illusionpoke.name : partynumber[newpoke].name
    end
    return gottenmarker
  end
...
################################################################################
  def pbCanMegaEvolve?(index)
    ...
    return false if @megaEvolution[side][owner]!=-1 && @battlers[index].species != PBSpecies::OGERPON
    return true
  end
  ...
  def pbMegaEvolve(index)
    ...
    if @battlers[index].effects[PBEffects::Illusion]
      #do nothing
    elsif @battlers[index].item == PBItems::PULSEHOLD
      pbDisplay(_INTL("{1}'s {2} is reacting to the PULSE machine!", @battlers[index].pbThis, PBItems.getName(@battlers[index].item)))
    elsif @battlers[index].species == PBSpecies::RAYQUAZA
      pbDisplay(_INTL("{1}'s fervent wish has reached {2}!", ownername, @battlers[index].pbThis(true)))
    elsif @battlers[index].species == PBSpecies::ETERNATUS
      pbDisplay(_INTL("The Darkest Day is about to unfold again!"))
    elsif @battlers[index].species == PBSpecies::TERAPAGOS
      pbDisplay(_INTL("Terapagos' terastal energy broke lose!"))
    elsif @battlers[index].item == PBItems::VOIDSTONE
      pbDisplay(_INTL("The power of the void radiates towards {1}!", @battlers[index].pbThis(true)))
    elsif [PBItems::TEALMASK, PBItems::WELLSPRINGMASK, PBItems::HEARTHFLAMEMASK, PBItems::CORNERSTONEMASK].include?(@battlers[index].item)
    ...
    if @battlers[index].effects[PBEffects::Illusion]
      #do nothing
    elsif @battlers[index].item == PBItems::PULSEHOLD || @battlers[index].species == PBSpecies::ETERNATUS || @battlers[index].item == PBItems::VOIDSTONE
    ...
    @scene.pbChangePokemon(@battlers[index],@battlers[index].pokemon) if @battlers[index].effects[PBEffects::Substitute] == 0 && @battlers[index].effects[PBEffects::Illusion] == nil
    ...
    if @battlers[index].effects[PBEffects::Illusion]
      ...
      if @battlers[index].species == PBSpecies::DEOXYS
        pbDisplay(_INTL("{1} mutated into its Amalgamate Form!",@battlers[index].pbThis))
      else
        pbDisplay(_INTL("{1} mutated into PULSE {2}!", @battlers[index].pbThis, PBSpecies.getName(@battlers[index].pokemon.species)))
      end
    elsif @battlers[index].item == PBItems::VOIDSTONE
      pbDisplay(_INTL("{1} embraced the darkness!",@battlers[index].pbThis))
    else
    ...
    @megaEvolution[side][owner]=-1 if @battlers[index].species == PBSpecies::OGERPON
    ...
    if !onlyabilities
      ...
      if pkmn.pbOwnSide.effects[PBEffects::StealthRock]
        if (pkmn.ability != PBAbilities::MAGICGUARD || pkmn.ability != PBAbilities::MOUNTAINEER || pkmn.ability != PBAbilities::OMNIPOTENT) && !pkmn.hasWorkingItem(:HEAVYDUTYBOOTS) && @field.effect != PBFields::WASTELAND
          ...
          if eff>0
            if @field.effect == PBFields::ROCKYF || @field.effect == PBFields::CAVE || @field.effect == PBFields::FALLOUT
              eff = eff*2
            end
            @scene.pbDamageAnimation(pkmn,0)
            ...
            elsif @field.effect == PBFields::FALLOUT
              pbDisplay(_INTL("{1} was hurt by the nuclearized stealth rocks!",pkmn.pbThis))
            else
      ...
      if pkmn.pbOwnSide.effects[PBEffects::Decrystals]
        if pkmn.ability != PBAbilities::MAGICGUARD && !pkmn.hasWorkingItem(:HEAVYDUTYBOOTS) && @field.effect != PBFields::WASTELAND
          ...
          if eff>0
            ...
            pkmn.pbReduceHP([(pkmn.totalhp*eff/32).floor,1].max)
            if @field.effect == PBFields::CRYSTALC
              pbDisplay(_INTL("{1} was hurt by the charged up crystals!",pkmn.pbThis))
            elsif @field.effect == PBFields::FALLOUT
              pbDisplay(_INTL("{1} was hurt by the nuclearized crystals!",pkmn.pbThis))
            else
              pbDisplay(_INTL("{1} was hurt by Crystals!",pkmn.pbThis))
            end
          end
        end
      end
      ...
      if @field.effect == PBFields::CORROSIVEF
        if !(pkmn.ability == PBAbilities::MAGICGUARD || pkmn.ability == PBAbilities::POISONHEAL || pkmn.ability == PBAbilities::IMMUNITY || pkmn.ability == PBAbilities::WONDERGUARD || pkmn.ability == PBAbilities::MULTICORE || 
...
################################################################################
  ...
  def pbStartBattleCore(canlose)
    ...
    if @doublebattle
      if @player.is_a?(Array)
        ...
        if sendout2>-1
          ...
          obtainedmarker = @battlers[0].mark
          case obtainedmarker
          when 3
            markdescription="Unrivaled"
          when 9
          ...
          #pbDisplayBrief(_INTL("Go! {1} the {2}!",@battlers[0].name,markdescription)) 
        end
      end
    ...
    elsif @weather==PBWeather::NUCLEARFALLOUT
      pbCommonAnimation("NuclearFallout")
      pbDisplay(_INTL("The toxic air lingers."))
    ...
    priority.each {|i| i.paradoxBoostCheck}
    @turncount=1
            ...
            if pbCanShowFightMenu?(i)
              ...
              until commandDone
                ...
                if @doublebattle
                  ...
                  if target==PBTargets::SingleNonUser || target==PBTargets::SingleOpposing# single non-user
...
################################################################################
  def pbEndOfRoundPhase
    ...
    for i in 0...4
      ...
      @battlers[i].effects[PBEffects::GlaiveRush]-=1    if @battlers[i].effects[PBEffects::GlaiveRush]>0
      @battlers[i].effects[PBEffects::GigatonHammer]-=1 if @battlers[i].effects[PBEffects::GigatonHammer]>0
      @battlers[i].effects[PBEffects::BloodMoon]-=1     if @battlers[i].effects[PBEffects::BloodMoon]>0
      @battlers[i].effects[PBEffects::Syrup]-=1         if @battlers[i].effects[PBEffects::Syrup]>0
      ...
      if @field.effect==PBFields::ELECTRICT && @battlers[i].effects[PBEffects::DoubleShock] # Electric Terrain
        @battlers[i].type1= @battlers[i].pokemon.type1
        @battlers[i].type2= @battlers[i].pokemon.type2
        @battlers[i].effects[PBEffects::DoubleShock]    =false
      end
    ...
    for i in priority
      ...
      case @field.effect
        ...
        when PBFields::FALLOUT # Uranium
          next if i.hp<=0
          if i.falloutPassiveDamage?
            eff=PBTypes.getCombinedEffectiveness(PBTypes::NUCLEAR,i.type1,i.type2)
            if eff>0
              @scene.pbDamageAnimation(i,0)
              pbDisplay(_INTL("The Pokémon were damaged by the radiation!",i.pbThis)) if endmessage == false
              endmessage=true
              i.pbReduceHP([(i.totalhp*eff/80).floor,1].max)
              if i.hp<=0
                return if !i.pbFaint
              end
            end
        ...
        when PBFields::DESERTF # Desert
          next if i.hp<=0
          if i.ability == PBAbilities::EARTHEATER && i.effects[PBEffects::HealBlock]==0 && !i.isAirborne?
            hpgain=(i.totalhp/16.0).floor
            hpgain=i.pbRecoverHP(hpgain,true)
    ...
    case @weather
      when PBWeather::SUNNYDAY
        ...
        if @weatherduration==0
          ...
          if pbWeather==PBWeather::SUNNYDAY
            endmessage=false
            extrasundamager = pbCheckGlobalAbility(:UBERSTRAT) # Nova Burner+
            extrasundamagerM = pbCheckGlobalAbility(:MULTICORE) # Multicore Nova Burner+
            #pbDisplay(_INTL("{1} has {2}!", extrasundamager.pbThis, PBAbilities.getName(extrasundamager.ability))) if extrasundamager
            for i in priority
              next if i.isFainted?
              if i.ability == PBAbilities::SOLARPOWER
                ...
                @scene.pbDamageAnimation(i,0)
                i.pbReduceHP((i.totalhp/8.0).floor)
              elsif extrasundamager || extrasundamagerM
                if !i.pbHasType?(:FIRE) && !i.pbHasType?(:DRAGON) && i.ability != PBAbilities::FLASHFIRE && i.ability != PBAbilities::UBERSTRAT && i.ability != PBAbilities::MULTICORE && i.ability != PBAbilities::MAGICGUARD && ![0xCA,0xCB].include?($cache.pkmn_move[i.effects[PBEffects::TwoTurnAttack]][PBMoveData::FUNCTION]) # Dig, Dive
                  pbDisplay(_INTL("The Pokémon were hurt by the sunlight!")) if endmessage==false
                  endmessage=true
                  @scene.pbDamageAnimation(i,0)
      ...
      when PBWeather::RAINDANCE
        ...
        if @weatherduration==0
          ...
          if pbWeather==PBWeather::RAINDANCE && @state.effects[PBEffects::Storm]
            endmessage=false
            for i in priority
              next if i.isFainted?
              if !i.pbHasType?(:ELECTRIC) && !i.pbHasType?(:GROUND) && !(i.ability == PBAbilities::MOTORDRIVE || i.ability == PBAbilities::VOLTABSORB ||
              i.ability == PBAbilities::LIGHTNINGROD || i.ability == PBAbilities::MAGICGUARD) &&
              ![0xCA,0xCB].include?($cache.pkmn_move[i.effects[PBEffects::TwoTurnAttack]][PBMoveData::FUNCTION]) # Dig, Dive
                pbDisplay(_INTL("The Pokémon were struck by strong lightning!")) if endmessage==false
                endmessage=true
                @scene.pbDamageAnimation(i,0)
                i.pbReduceHP((i.totalhp/8.0).floor)
                if i.isFainted?
        ...
        if @weatherduration==0
          ...
          if pbWeather==PBWeather::SANDSTORM
            ...
            for i in priority
              ...
              !i.hasWorkingItem(:SAFETYGOGGLES) && ![0xCA,0xCB].include?($cache.pkmn_move[i.effects[PBEffects::TwoTurnAttack]][PBMoveData::FUNCTION]) # Dig, Dive
                pbDisplay(_INTL("The Pokémon were buffeted by the sandstorm!")) if endmessage==false
                endmessage=true
                @scene.pbDamageAnimation(i,0)
                i.pbReduceHP((i.totalhp/16.0).floor)
                if i.isFainted?
      ...
      when PBWeather::HAIL
        ...
        if @weatherduration==0
          ...
          if pbWeather==PBWeather::HAIL
            endmessage=false
            extrahaildamager = pbCheckGlobalAbility(:ILLEGAL) # Snow Blazer+
            #pbDisplay(_INTL("{1} has {2}!", extrahaildamager.pbThis, PBAbilities.getName(extrahaildamager.ability))) if extrahaildamager
            for i in priority
              next if i.isFainted?
              if !i.pbHasType?(:ICE) && !(extrahaildamager && i.pbHasType?(:FIRE)) && i.ability != PBAbilities::SNOWBLAZER && i.ability != PBAbilities::ILLEGAL && i.ability != PBAbilities::ICEBODY && i.ability != PBAbilities::SNOWCLOAK && i.ability != PBAbilities::MAGICGUARD &&
              !i.hasWorkingItem(:SAFETYGOGGLES) && i.ability != PBAbilities::OVERCOAT && ![0xCA,0xCB].include?($cache.pkmn_move[i.effects[PBEffects::TwoTurnAttack]][PBMoveData::FUNCTION]) # Dig, Dive
                pbDisplay(_INTL("The Pokémon were buffeted by the hail!")) if endmessage==false
                endmessage=true
                @scene.pbDamageAnimation(i,0)
                divisor = extrahaildamager ? 8.0 : 16.0
      ...
      when PBWeather::NUCLEARFALLOUT #! NUCLEAR STARTS
        @weatherduration=@weatherduration-1 if @weatherduration>0
        if @weatherduration==0
          pbDisplay(_INTL("The fallout subsided."))
          pbDisplay(_INTL("The starry sky shone through!")) if @field.effect == PBFields::STARLIGHTA
          @weather=0
        else
          pbCommonAnimation("NuclearFallout")
          if @field.effect == PBFields::SUPERHEATEDF
            @field.counter+=1
            if @field.counter == 3
              setField(PBFields::BURNINGF)
              pbDisplay(_INTL("The debris provoked an explosion!"))
            end
          end
          if @field.effect == PBFields::RAINBOWF
            breakField if @field.duration == 0
            endTempField if @field.duration > 0
            @field.duration = 0
            pbDisplay(_INTL("There's no room for happiness..."))
          end
          if pbWeather==PBWeather::NUCLEARFALLOUT
            endmessage=false
            for i in priority
              next if i.isFainted?
              eff = PBTypes.getCombinedEffectiveness(PBTypes::NUCLEAR, i.type1, i.type2)
              if !i.pbHasType?(:NUCLEAR) && !i.pbHasType?(:STEEL) && !i.pbHasType?(:COSMIC) && !(i.ability == PBAbilities::MAGICGUARD || i.ability == PBAbilities::BULLETPROOF || i.ability == PBAbilities::LEADSKIN) &&
              !i.hasWorkingItem(:SAFETYGOGGLES) && eff>0
                pbDisplay(_INTL("The nuclear end is near...")) if endmessage==false
                endmessage=true
                if @field.effect == PBFields::FALLOUT
                  eff = eff*2
                end
                @scene.pbDamageAnimation(i,0)
                #i.pbReduceHP([(i.totalhp*eff/32).floor,1].max) #! this makes it so if you are quad weak in fallout field you instantly die at the end of the turn
                #i.pbReduceHP([(i.totalhp*eff/64).floor,1].max) #? this makes it so you survive at least with half HPs
                i.pbReduceHP([(i.totalhp*eff/128).floor,1].max) #todo this makes it so you only lose a quarter of HPs [BALANCED]
              end
              if i.isFainted?
                return if !i.pbFaint
              end
    ...
    # Leech Seed (Decrystallization)
    for i in priority
      if i.abilityWorks?(true) && i.ability == PBAbilities::LIQUIDOOZE && i.effects[PBEffects::Decrystalseed]>=0
        recipient=@battlers[i.effects[PBEffects::Decrystalseed]]
        if recipient && !recipient.isFainted?
          hploss=(i.totalhp/8.0).floor
          hploss= hploss * 2 if @field.effect == PBFields::WASTELAND 
          pbCommonAnimation("LeechSeed",recipient,i)
      ...
      next if i.isFainted?
      if i.effects[PBEffects::Decrystalseed]>=0
        recipient=@battlers[i.effects[PBEffects::Decrystalseed]]
        if recipient && !recipient.isFainted? && i.ability != PBAbilities::MAGICGUARD # if recipient exists
          pbCommonAnimation("Decrystalseed",recipient,i)
          hploss=i.pbReduceHP((i.totalhp/8.0).floor,true)
          hploss= hploss * 2 if @field.effect == PBFields::WASTELAND
          if recipient.effects[PBEffects::HealBlock]==0
            hploss=(hploss*1.3).floor if recipient.hasWorkingItem(:BIGROOT)
            recipient.pbRecoverHP(hploss,true)
            pbDisplay(_INTL("{1}'s health was sapped by the Crystals!",i.pbThis))
          end
          if i.isFainted?
            return if !i.pbFaint
          end
    ...
    for i in priority
      ...
      # Salt Cure
      if i.effects[PBEffects::SaltCure] && i.ability != PBAbilities::MAGICGUARD
        @scene.pbDamageAnimation(i,0)
        divisor = i.pbHasType?(:STEEL) || i.pbHasType?(:WATER) ? 4.0 : 8.0
        i.pbReduceHP((i.totalhp/divisor).floor)
    ...
    for i in priority
      ...
      if i.effects[PBEffects::MultiTurn]>0
        ...
        if i.effects[PBEffects::MultiTurn]==0
          ...
          case i.effects[PBEffects::MultiTurnAttack]
          when PBMoves::BIND, PBMoves::THUNDERCAGE, PBMoves::METALWHIP
            pbCommonAnimation("Bind",i,nil)
          when PBMoves::CLAMP
            pbCommonAnimation("Clamp",i,nil)
          when PBMoves::FIRESPIN, PBMoves::INFERNALFIREBLAST
            pbCommonAnimation("FireSpin",i,nil)
          when PBMoves::MAGMASTORM
            pbCommonAnimation("Magma Storm",i,nil)
          when PBMoves::SANDTOMB
            pbCommonAnimation("SandTomb",i,nil)
          when PBMoves::INFESTATION, PBMoves::MIDNIGHTPHOTOSYNTHESIS
            pbCommonAnimation("Infestation",i,nil)
          when PBMoves::WHIRLPOOL, PBMoves::OCEANSWRATH, PBMoves::INFERNALWATERBLAST
    ...
    # Solar Array
    for i in 0...2
      if sides[i].effects[PBEffects::SolarArray]>0
        sides[i].effects[PBEffects::SolarArray]-=1
        if sides[i].effects[PBEffects::SolarArray]==0
          pbDisplay(_INTL("Your team's Solar Array faded!")) if i==0
          pbDisplay(_INTL("The opposing team's Solar Array faded!")) if i==1
        end
      end
    end
    # Safeguard
    ...
    for i in priority
      next if i.isFainted?
      if i.ability==PBAbilities::SLOWSTART && i.turncount==3
        pbDisplay(_INTL("{1} finally got its act together!",i.pbThis))
      end
    end

    # Quick Start's end message
    for i in priority
      next if i.isFainted?
      if i.ability==PBAbilities::QUICKSTART && i.turncount==2
        pbDisplay(_INTL("{1} cannot get it going anymore!",i.pbThis))
      end
    end
    ...
    if @field.effect == PBFields::WASTELAND
      for i in priority
        ...
        # Decrystallization
        if i.pbOwnSide.effects[PBEffects::Decrystals]==true
          pbDisplay(_INTL("The waste swallowed up the pointed crystals!"))
          i.pbOwnSide.effects[PBEffects::Decrystals]=false
          pbDisplay(_INTL("...Crystals spewed out from the ground below!"))
          for mon in [i, i.pbPartner]
            next if mon.isFainted? || PBStuff::TWOTURNMOVE.include?(mon.effects[PBEffects::TwoTurnAttack])
            eff=PBTypes.getCombinedEffectiveness(PBTypes::COSMIC,mon.type1,mon.type2)
            next if eff <=0
            @scene.pbDamageAnimation(mon,0)
            mon.pbReduceHP([(mon.totalhp*eff/16).floor,1].max)
          end
    ...
    for i in priority
      ...
      i.ability != PBAbilities::WONDERGUARD && i.ability != PBAbilities::PLOTARMOR && !i.isAirborne? && !i.pbHasType?(:STEEL) && !i.pbHasType?(:POISON) && @field.effect == PBFields::CORROSIVEF
        hploss=i.pbReduceHP((i.totalhp/16.0).floor,true)
      ...
      # Octolock
      if i.effects[PBEffects::Octolock] >= 0
        #pbDisplay(_INTL("{1} is caught in {2}'s Octolock!", i.pbThis, @battlers[i.effects[PBEffects::Octolock]].pbThis(true)))
        if !i.pbTooLow?(PBStats::DEFENSE)
          i.pbReduceStat(PBStats::DEFENSE,1, abilitymessage:false, mirrorable:false)
        end
        if !i.pbTooLow?(PBStats::SPDEF)
          i.pbReduceStat(PBStats::SPDEF,1, abilitymessage:false, mirrorable:false)
        end
      end
      # Syrup Bomb
      if i.effects[PBEffects::Syrup]>0 && !i.pbTooLow?(PBStats::SPEED)
        #pbDisplay(_INTL("{1} is covered in syrup!", i.pbThis))
        i.pbReduceStat(PBStats::SPEED,1, abilitymessage:false, mirrorable:false)
      end
      ...
      # Well-Baked Body on Fire Fields
      if i.ability == PBAbilities::WELLBAKEDBODY
        if [PBFields::BURNINGF,PBFields::SUPERHEATEDF,PBFields::DRAGONSD].include?(@field.effect)
          if !i.pbTooHigh?(PBStats::DEFENSE)
            i.pbIncreaseStatBasic(PBStats::DEFENSE,2)
            pbCommonAnimation("StatUp",i,nil)
            pbDisplay(_INTL("{1}'s Well-Baked Body sharply raised its defense!", i.pbThis))
          end
        end
      end
      #Thermal Exchange on Fire Fields
      if i.ability == PBAbilities::THERMALEXCHANGE
        if [PBFields::BURNINGF,PBFields::SUPERHEATEDF,PBFields::DRAGONSD].include?(@field.effect)
          if !i.pbTooHigh?(PBStats::ATTACK)
            i.pbIncreaseStatBasic(PBStats::ATTACK,1)
            pbCommonAnimation("StatUp",i,nil)
            pbDisplay(_INTL("{1}'s Thermal Exchange raised its attack!", i.pbThis))
          end
        end
      end
      # Bad Dreams
      ...
      # Cud Chew
      if !i.effects[PBEffects::CudChew].empty? && @turncount > i.effects[PBEffects::CudChew][1]
        if i.ability == PBAbilities::CUDCHEW
          pbDisplay(_INTL("{1} regurgitated its {2} to eat it again!", i.pbThis, PBItems.getName(i.effects[PBEffects::CudChew][0])))
          i.pbUseBerry(i.effects[PBEffects::CudChew][0],true)
        end
        i.effects[PBEffects::CudChew].clear
      end
      # Ball Fetch
      ...
      if i.ability == PBAbilities::CLOUDNINE && @field.effect==PBFields::RAINBOWF
        ...
        if failsafe!=1000
          i.pbIncreaseStatBasic(randoms[0],1)
          pbCommonAnimation("StatUp",i,nil)
    ...
    for i in priority
      ...
      # Alolan Techniques
      if (i.ability == PBAbilities::ALOLANTECHNIQUESA || i.ability == PBAbilities::ALOLANTECHNIQUESB) && (i.species == PBSpecies::LILLIGANT)
        i.form = (i.form==3) ? 4 : 3
        i.pbUpdate(true)
        scene.pbChangePokemon(i,i.pokemon)
        pbDisplay(_INTL("{1} transformed!",i.pbThis))
...
################################################################################
  def pbEndOfBattle(canlose=false)
    ...
    for i in @party1
      ...
      i.makeUnultra(i.initialform) if i.isUltra? && i.initialform
      i.form=0 if i.species == PBSpecies::MIMIKYU && i.form == 1
      i.form=2 if i.species == PBSpecies::MIMIKYU && i.form == 3
