...
class PokemonStorage
  ...
  def pbStoreCaught(pkmn)
    ...
    pkmn.makeUnultra(pkmn.initialform) if pkmn.isUltra? && pkmn.initialform
    pkmn.form=0 if pkmn.species == PBSpecies::MIMIKYU && pkmn.form == 1
    pkmn.form=2 if pkmn.species == PBSpecies::MIMIKYU && pkmn.form == 3
...
class PokemonStorageScene
  ...
  def pbHardRefresh
    ...
    @sprites["tooltip"].dispose if @sprites["tooltip"]
    @sprites["box"]=PokemonBoxSprite.new(@storage,@storage.currentBox,@boxviewport)
    @sprites["boxparty"]=PokemonBoxPartySprite.new(@storage.party,@boxsidesviewport)
